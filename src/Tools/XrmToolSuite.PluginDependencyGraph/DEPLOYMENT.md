...
> ClosedXML/PdfSharp-MigraDoc-GDI dependency DLLs) into that tool's per-tool subfolder (Plugins\XrmToolSuite.PluginDependencyGraph\) automatically; the tool DLL itself sits in the Plugins root. For a **manual**
> copy to another machine, copy **every** DLL from the tool's `bin\Release\net48\` folder (the tool DLL into the Plugins root, its dependency DLLs into the subfolder Plugins\XrmToolSuite.PluginDependencyGraph\) - see the **"Export tools only"** section of

...
namespace XrmToolSuite.PluginDependencyGraph
{
    ...
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "Plugin Dependency Graph"),
     ExportMetadata("Description", "Read-only dependency graph of Dataverse plugin registrations (assembly → type → step → image → message/table → custom API → solution/config) with high-impact, duplicate-step and unmanaged-registration detection, exportable as PNG/SVG/PDF/Excel/JSON/GraphML/HTML."),
    ...
    public class PluginDependencyGraphPlugin : PluginBase
    {
        ...
        static PluginDependencyGraphPlugin()
        {
        ...
        private static Assembly ResolveDependency(object sender, ResolveEventArgs args)
        {
            ...
            // version differs — this papers over the facade version mismatch.
        ...
        {
            return new PluginDependencyGraphControl();
        }
    }
}

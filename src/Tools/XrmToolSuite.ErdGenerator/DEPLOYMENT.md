# ERD Generator - Build & Install

Deployment guide for the **ERD Generator** XrmToolBox plugin. For the suite-wide guide
...
Then **restart XrmToolBox** and open **ERD Generator**. Files written by your own build are not
...
2. Copy the **single** output DLL
   ...
   ```
   src\Tools\XrmToolSuite.ErdGenerator\bin\Release\net48\XrmToolSuite.ErdGenerator.dll
   ```
...
4. **Restart XrmToolBox**, search for **ERD Generator**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.ErdGenerator.dll`. |

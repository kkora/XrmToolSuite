# Attribute Auditor — Build & Install

Deployment guide for the **Attribute Auditor** XrmToolBox plugin. For the suite-wide guide
...
Then **restart XrmToolBox** and open **Attribute Auditor**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.AttributeAuditor\XrmToolSuite.AttributeAuditor.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.AttributeAuditor\bin\Release\net48\XrmToolSuite.AttributeAuditor.dll
   ```
...
4. **Restart XrmToolBox**, search for **Attribute Auditor**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships — delete it and keep only `XrmToolSuite.AttributeAuditor.dll`. |

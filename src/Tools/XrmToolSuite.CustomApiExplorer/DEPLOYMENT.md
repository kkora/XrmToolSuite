# Custom API Explorer - Build & Install

Deployment guide for the **Custom API Explorer** XrmToolBox plugin. For the suite-wide guide
...
Then **restart XrmToolBox** and open **Custom API Explorer**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.CustomApiExplorer\XrmToolSuite.CustomApiExplorer.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.CustomApiExplorer\bin\Release\net48\XrmToolSuite.CustomApiExplorer.dll
   ```
...
4. **Restart XrmToolBox**, search for **Custom API Explorer**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.CustomApiExplorer.dll`. |

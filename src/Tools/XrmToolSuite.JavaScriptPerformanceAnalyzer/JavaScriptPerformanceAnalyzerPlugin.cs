...
namespace XrmToolSuite.JavaScriptPerformanceAnalyzer
{
    ...
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "JavaScript Performance Analyzer"),
     ExportMetadata("Description", "Statically analyzes JavaScript web resources and their form/event usage for performance and deprecation risks (Xrm.Page, synchronous XHR, blocking alerts, excessive logging, repeated retrieves, hardcoded GUIDs/URLs, unsupported DOM access, script size), with a per-script risk score and Excel/PDF/JSON/HTML/Markdown/CSV exports."),
    ...
    public class JavaScriptPerformanceAnalyzerPlugin : PluginBase
    {
        // The Excel-export (ClosedXML) and native-PDF (PdfSharp/MigraDoc) chains ship in a per-tool
        // SUBFOLDER named after this assembly (Plugins\XrmToolSuite.JavaScriptPerformanceAnalyzer\), the
        ...
        private static readonly string PluginDirectory =
            Path.GetDirectoryName(typeof(JavaScriptPerformanceAnalyzerPlugin).Assembly.Location) ?? string.Empty;
        ...
        {
            Path.Combine(PluginDirectory, typeof(JavaScriptPerformanceAnalyzerPlugin).Assembly.GetName().Name),
            PluginDirectory,
        };
        ...
        static JavaScriptPerformanceAnalyzerPlugin()
        {
        ...
        {
            return new JavaScriptPerformanceAnalyzerControl();
        }
    }
}

# JavaScript Performance Analyzer - Build & Install

Deployment guide for the **JavaScript Performance Analyzer** XrmToolBox plugin. For the suite-wide guide
...
> ClosedXML/PdfSharp-MigraDoc-GDI dependency DLLs) into that tool's per-tool subfolder (Plugins\XrmToolSuite.JavaScriptPerformanceAnalyzer\) automatically; the tool DLL itself sits in the Plugins root. For a **manual**
> copy to another machine, copy **every** DLL from the tool's `bin\Release\net48\` folder (the tool DLL into the Plugins root, its dependency DLLs into the subfolder Plugins\XrmToolSuite.JavaScriptPerformanceAnalyzer\) - see the **"Export tools only"** section of
...
Then **restart XrmToolBox** and open **JavaScript Performance Analyzer**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.JavaScriptPerformanceAnalyzer\XrmToolSuite.JavaScriptPerformanceAnalyzer.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.JavaScriptPerformanceAnalyzer\bin\Release\net48\XrmToolSuite.JavaScriptPerformanceAnalyzer.dll
   ```
...
4. **Restart XrmToolBox**, search for **JavaScript Performance Analyzer**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.JavaScriptPerformanceAnalyzer.dll`. |

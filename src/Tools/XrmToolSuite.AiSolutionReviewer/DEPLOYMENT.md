# AI Solution Reviewer - Build & Install

Deployment guide for the **AI Solution Reviewer** XrmToolBox plugin. For the suite-wide guide
...
> ClosedXML/PdfSharp-MigraDoc-GDI dependency DLLs) into that tool's per-tool subfolder (Plugins\XrmToolSuite.AiSolutionReviewer\) automatically; the tool DLL itself sits in the Plugins root. For a **manual**
> copy to another machine, copy **every** DLL from the tool's `bin\Release\net48\` folder (the tool DLL into the Plugins root, its dependency DLLs into the subfolder Plugins\XrmToolSuite.AiSolutionReviewer\) - see the **"Export tools only"** section of
...
Then **restart XrmToolBox** and open **AI Solution Reviewer**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.AiSolutionReviewer\XrmToolSuite.AiSolutionReviewer.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.AiSolutionReviewer\bin\Release\net48\XrmToolSuite.AiSolutionReviewer.dll
   ```
...
4. **Restart XrmToolBox**, search for **AI Solution Reviewer**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.AiSolutionReviewer.dll`. |

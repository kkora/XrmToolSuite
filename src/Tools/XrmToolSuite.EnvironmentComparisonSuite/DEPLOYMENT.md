# Environment Comparison Suite - Build & Install

Deployment guide for the **Environment Comparison Suite** XrmToolBox plugin. For the suite-wide guide
...
Then **restart XrmToolBox** and open **Environment Comparison Suite**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.EnvironmentComparisonSuite\XrmToolSuite.EnvironmentComparisonSuite.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.EnvironmentComparisonSuite\bin\Release\net48\XrmToolSuite.EnvironmentComparisonSuite.dll
   ```
...
4. **Restart XrmToolBox**, search for **Environment Comparison Suite**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.EnvironmentComparisonSuite.dll`. |

...
{
    /// <summary>
    /// Measures a single solution's complexity: counts every component type, computes a 0–100 complexity
    /// score plus maintainability and upgrade/migration/testing effort estimates, and surfaces hotspots —
    /// with Excel/PDF/HTML/JSON exports and an offline/AI executive summary. Follows the suite patterns.
    /// </summary>
    public partial class SolutionComplexityScoreControl : BaseToolControl, IGitHubPlugin, IHelpPlugin
    {
        ...
        private DataGridView _gridMetrics, _gridFindings;
        ...
        public SolutionComplexityScoreControl()
        {
            BuildUi();
            Load += OnLoad;
        }
        ...
        private void BuildUi()
        {
            ...
            _btnAnalyze = new ToolStripButton("▶ Score complexity") { DisplayStyle = ToolStripItemDisplayStyle.Text };
            ...
            _btnExport.DropDownItems.Add("Checklist (.md)", null, (s, e) => Export("md"));
            ...
            _lblScore = new Label { Text = "Load a solution and score its complexity.", AutoSize = true, Location = new Point(8, 34) };
            summaryPanel.Controls.AddRange(new Control[] { _lblBand, _lblScore });

            _gridMetrics = MakeGrid();
            _gridMetrics.Columns.Add(new DataGridViewTextBoxColumn { Name = "Metric", HeaderText = "Metric", FillWeight = 60 });
            _gridMetrics.Columns.Add(new DataGridViewTextBoxColumn { Name = "Value", HeaderText = "Value", FillWeight = 40 });

            _gridFindings = MakeGrid();
            _gridFindings.Columns.Add(new DataGridViewTextBoxColumn { Name = "Severity", HeaderText = "Severity", FillWeight = 16 });
            _gridFindings.Columns.Add(new DataGridViewTextBoxColumn { Name = "Hotspot", HeaderText = "Hotspot", FillWeight = 34 });
            _gridFindings.Columns.Add(new DataGridViewTextBoxColumn { Name = "Detail", HeaderText = "Detail", FillWeight = 50 });
            ...
            leftPanel.Controls.Add(_gridMetrics);
            leftPanel.Controls.Add(new Label { Text = "Metrics", Dock = DockStyle.Top, Height = 22, Font = new Font(Font, FontStyle.Bold), Padding = new Padding(4, 4, 0, 0) });
        ...
        {
            _settings = LoadSettings<ComplexitySettings>();
        ...
        private void RunAnalysis()
        {
            ...
            {
                MessageBox.Show(this, "Load and select a solution first.", "Solution Complexity Score",
            ...
            RunAsync(
                "Scoring solution complexity…",
                worker =>
                {
                    var ctx = new ComplexityContext(service, solution);
                    ctx.LoadComponents();
                    var counts = ComplexityCollector.Collect(ctx, msg => worker.ReportProgress(0, msg));
                    return ComplexityReport.Build(counts, ctx.SolutionFriendlyName, ctx.SolutionUniqueName,
                ...
                model =>
                {
                    ...
                    SetStatusMessage($"Complexity {model.Score}/100 ({model.Band}).");
                });
        }

        private void BindModel(ReportModel model)
        {
            _gridMetrics.Rows.Clear();
            _gridMetrics.Rows.Add("Complexity score", $"{model.Score}/100");
            foreach (var m in model.Metrics)
                _gridMetrics.Rows.Add(m.Label, string.IsNullOrEmpty(m.Hint) ? m.Value : $"{m.Value} {m.Hint}");

            _gridFindings.Rows.Clear();
            foreach (var f in model.Findings.OrderByDescending(x => x.Severity))
            {
                int i = _gridFindings.Rows.Add(f.Severity.ToString(), f.Title, f.Description);
        ...
        private void Export(string kind)
        {
            ...
            string defaultName = $"SolutionComplexity_{DateTime.Now:yyyyMMdd_HHmm}.{kind}";
            using (var dlg = new SaveFileDialog { Filter = filter, FileName = defaultName })
            {
                ...
                try
                {
                    ...
                    if (MessageBox.Show(this, "Report exported. Open it now?", "Solution Complexity Score",
        ...
        private void ProduceSummary(bool interactive)
        {
            ...
            {
                if (interactive)
                    MessageBox.Show(this, "Score a solution first.", "Solution Complexity Score",
                        MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            ...
            RunAsync(
                ...
                summary =>
                {
                    ...
                    SetStatusMessage(summary.FromAi
                        ...
                        : summary.Error != null ? $"Offline summary (AI unavailable: {summary.Error})"
        ...
        private SummaryOptions TryBuildAiOptions(bool interactive, ReportModel model)
        {
            ...
            return new SummaryOptions
            {
                ...
                SystemPrompt = ComplexityReport.AiSystemPrompt
            };
        }

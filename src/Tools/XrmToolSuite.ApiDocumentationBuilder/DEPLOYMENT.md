# API Documentation Builder - Build & Install

Deployment guide for the **API Documentation Builder** XrmToolBox plugin. For the suite-wide guide
...
Then **restart XrmToolBox** and open **API Documentation Builder**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.ApiDocumentationBuilder\XrmToolSuite.ApiDocumentationBuilder.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.ApiDocumentationBuilder\bin\Release\net48\XrmToolSuite.ApiDocumentationBuilder.dll
   ```
...
4. **Restart XrmToolBox**, search for **API Documentation Builder**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.ApiDocumentationBuilder.dll`. |

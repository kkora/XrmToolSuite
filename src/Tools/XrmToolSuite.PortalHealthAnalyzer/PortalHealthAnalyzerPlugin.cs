...
namespace XrmToolSuite.PortalHealthAnalyzer
{
    ...
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "Portal Health Analyzer"),
     ExportMetadata("Description", "Read-only health scoring of a Power Pages website across the dual adx_/mspp_ schemas: inventories pages, templates, snippets, site settings, roles, table permissions, forms, lists and web files, then runs structural, site-settings and security-surface rules to produce a 0–100 health score and a categorized issue grid. Exports Excel/PDF/Word/JSON/HTML/CSV."),
    ...
    public class PortalHealthAnalyzerPlugin : PluginBase
    {
        // The Excel-export (ClosedXML) and native-PDF/Word (PdfSharp/MigraDoc) chains ship in a per-tool
        // SUBFOLDER named after this assembly (Plugins\XrmToolSuite.PortalHealthAnalyzer\), the
        ...
        private static readonly string PluginDirectory =
            Path.GetDirectoryName(typeof(PortalHealthAnalyzerPlugin).Assembly.Location) ?? string.Empty;
        ...
        {
            Path.Combine(PluginDirectory, typeof(PortalHealthAnalyzerPlugin).Assembly.GetName().Name),
            PluginDirectory,
        };
        ...
        private static readonly HashSet<string> OwnedDependencies = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // Native-PDF/Word chain (PdfSharp/MigraDoc, GDI build — assemblies carry a -gdi suffix)
        ...
        static PortalHealthAnalyzerPlugin()
        {
        ...
        {
            return new PortalHealthAnalyzerControl();
        }
    }
}

# Architecture Diagram Generator - Build & Install

Deployment guide for the **Architecture Diagram Generator** XrmToolBox plugin. For the suite-wide guide
...
Then **restart XrmToolBox** and open **Architecture Diagram Generator**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.ArchitectureDiagramGenerator\XrmToolSuite.ArchitectureDiagramGenerator.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.ArchitectureDiagramGenerator\bin\Release\net48\XrmToolSuite.ArchitectureDiagramGenerator.dll
   ```
...
4. **Restart XrmToolBox**, search for **Architecture Diagram Generator**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.ArchitectureDiagramGenerator.dll`. |

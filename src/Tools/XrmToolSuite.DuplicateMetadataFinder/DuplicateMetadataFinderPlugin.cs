...
namespace XrmToolSuite.DuplicateMetadataFinder
{
    ...
    // no "Tools not loaded" entry). PluginIcons supplies suite defaults; never remove the keys.
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "Duplicate Metadata Finder"),
     ExportMetadata("Description", "Groups duplicate and near-duplicate Dataverse metadata (columns, option sets, tables, forms, views, business rules, web resources, plugin steps, relationships) with a 0-100 similarity score per pair and a recommended primary to keep. Read-only; recommends only, never merges or deletes. Excel/PDF/JSON/HTML export."),
    ...
    public class DuplicateMetadataFinderPlugin : PluginBase
    {
        // The Excel-export (ClosedXML) and native-PDF (PdfSharp/MigraDoc) chains ship in a per-tool
        // SUBFOLDER named after this assembly (Plugins\XrmToolSuite.DuplicateMetadataFinder\), the
        ...
        // System.Memory 4.0.1.1 but its NuGet deps demand higher). Current XrmToolBox ships those
        // facades in its app dir with binding redirects that cover the range, so the mismatch resolves
        // for free at runtime. This handler is a scoped safety net for hosts whose redirects don't
        // cover the range: for OUR shipped dependencies only, it satisfies a failed bind with whatever
        // compatible version sits next to us. It deliberately does nothing for any other assembly so it
        // can never interfere with the other tools in the same AppDomain.
        private static readonly string PluginDirectory =
            Path.GetDirectoryName(typeof(DuplicateMetadataFinderPlugin).Assembly.Location) ?? string.Empty;
        ...
        {
            Path.Combine(PluginDirectory, typeof(DuplicateMetadataFinderPlugin).Assembly.GetName().Name),
            PluginDirectory,
        };
        ...
        static DuplicateMetadataFinderPlugin()
        {
        ...
        {
            return new DuplicateMetadataFinderControl();
        }
    }
}

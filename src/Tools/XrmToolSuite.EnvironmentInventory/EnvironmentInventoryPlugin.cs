...
namespace XrmToolSuite.EnvironmentInventory
{
    ...
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "Environment Inventory"),
     ExportMetadata("Description", "Inventories a Dataverse environment (solutions, tables, security roles, users/teams/BUs, plugins, workflows, web resources, custom APIs, environment variables and connection references) into one searchable, filterable catalog with CSV/JSON/Markdown/HTML export. Read-only; never exports secrets."),
    ...
    public class EnvironmentInventoryPlugin : PluginBase
    {
        // The Excel-export (ClosedXML) and native-PDF (PdfSharp/MigraDoc) chains ship in a per-tool
        // SUBFOLDER named after this assembly (Plugins\XrmToolSuite.EnvironmentInventory\), the
        ...
        private static readonly string PluginDirectory =
            Path.GetDirectoryName(typeof(EnvironmentInventoryPlugin).Assembly.Location) ?? string.Empty;
        ...
        {
            Path.Combine(PluginDirectory, typeof(EnvironmentInventoryPlugin).Assembly.GetName().Name),
            PluginDirectory,
        };
        ...
        static EnvironmentInventoryPlugin()
        {
        ...
        {
            return new EnvironmentInventoryControl();
        }
    }
}

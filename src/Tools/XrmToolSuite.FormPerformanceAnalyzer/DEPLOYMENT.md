# Form Performance Analyzer - Build & Install

Deployment guide for the **Form Performance Analyzer** XrmToolBox plugin. For the suite-wide guide
...
Then **restart XrmToolBox** and open **Form Performance Analyzer**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.FormPerformanceAnalyzer\XrmToolSuite.FormPerformanceAnalyzer.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.FormPerformanceAnalyzer\bin\Release\net48\XrmToolSuite.FormPerformanceAnalyzer.dll
   ```
...
4. **Restart XrmToolBox**, search for **Form Performance Analyzer**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.FormPerformanceAnalyzer.dll`. |

...
namespace XrmToolSuite.FetchXmlPerformanceAnalyzer
{
    ...
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "FetchXML Performance Analyzer"),
     ExportMetadata("Description", "Analyzes FetchXML for performance risks (all-attributes, missing filters, excessive joins, sort/aggregation cost) and estimates query cost with optional live timing."),
    ...
    public class FetchXmlPerformanceAnalyzerPlugin : PluginBase
    {
        // The Excel-export (ClosedXML) and native-PDF (PdfSharp/MigraDoc) chains ship in a per-tool
        // SUBFOLDER named after this assembly (Plugins\XrmToolSuite.FetchXmlPerformanceAnalyzer\), the
        ...
        private static readonly string PluginDirectory =
            Path.GetDirectoryName(typeof(FetchXmlPerformanceAnalyzerPlugin).Assembly.Location) ?? string.Empty;
        ...
        {
            Path.Combine(PluginDirectory, typeof(FetchXmlPerformanceAnalyzerPlugin).Assembly.GetName().Name),
            PluginDirectory,
        };
        ...
        static FetchXmlPerformanceAnalyzerPlugin()
        {
        ...
        {
            return new FetchXmlPerformanceAnalyzerControl();
        }
    }
}

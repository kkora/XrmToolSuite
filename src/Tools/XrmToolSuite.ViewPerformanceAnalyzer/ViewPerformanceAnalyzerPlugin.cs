...
namespace XrmToolSuite.ViewPerformanceAnalyzer
{
    ...
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "View Performance Analyzer"),
     ExportMetadata("Description", "Batch-analyzes every system and personal view's FetchXML and layout for performance risks (all-attributes, missing filters, excessive joins, over-wide grids), ranks the slowest/riskiest views by a heuristic cost score, and exports Excel/PDF/JSON/HTML/Markdown/CSV."),
    ...
    public class ViewPerformanceAnalyzerPlugin : PluginBase
    {
        // The Excel-export (ClosedXML) and native-PDF (PdfSharp/MigraDoc) chains ship in a per-tool
        // SUBFOLDER named after this assembly (Plugins\XrmToolSuite.ViewPerformanceAnalyzer\), the
        ...
        private static readonly string PluginDirectory =
            Path.GetDirectoryName(typeof(ViewPerformanceAnalyzerPlugin).Assembly.Location) ?? string.Empty;
        ...
        {
            Path.Combine(PluginDirectory, typeof(ViewPerformanceAnalyzerPlugin).Assembly.GetName().Name),
            PluginDirectory,
        };
        ...
        static ViewPerformanceAnalyzerPlugin()
        {
        ...
        {
            return new ViewPerformanceAnalyzerControl();
        }
    }
}

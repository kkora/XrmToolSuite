...
namespace XrmToolSuite.PrivilegeGapAnalyzer
{
    ...
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "Privilege Gap Analyzer"),
     ExportMetadata("Description", "Diagnoses exactly why a user, team, or role can or cannot perform a Dataverse operation on a table: computes effective privileges (direct + team-inherited), pinpoints the missing privilege or insufficient scope, flags Append/AppendTo mismatches, and recommends a read-only fix."),
    ...
    public class PrivilegeGapAnalyzerPlugin : PluginBase
    {
        // The Excel-export (ClosedXML) and native-PDF (PdfSharp/MigraDoc) chains ship in a per-tool
        // SUBFOLDER named after this assembly (Plugins\XrmToolSuite.PrivilegeGapAnalyzer\), the
        ...
        private static readonly string PluginDirectory =
            Path.GetDirectoryName(typeof(PrivilegeGapAnalyzerPlugin).Assembly.Location) ?? string.Empty;
        ...
        {
            Path.Combine(PluginDirectory, typeof(PrivilegeGapAnalyzerPlugin).Assembly.GetName().Name),
            PluginDirectory,
        };
        ...
        static PrivilegeGapAnalyzerPlugin()
        {
        ...
        {
            return new PrivilegeGapAnalyzerControl();
        }
    }
}

# Solution Knowledge Graph - Build & Install

Deployment guide for the **Solution Knowledge Graph** XrmToolBox plugin. For the suite-wide guide
...
Then **restart XrmToolBox** and open **Solution Knowledge Graph**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.SolutionKnowledgeGraph\XrmToolSuite.SolutionKnowledgeGraph.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.SolutionKnowledgeGraph\bin\Release\net48\XrmToolSuite.SolutionKnowledgeGraph.dll
   ```
...
4. **Restart XrmToolBox**, search for **Solution Knowledge Graph**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.SolutionKnowledgeGraph.dll`. |

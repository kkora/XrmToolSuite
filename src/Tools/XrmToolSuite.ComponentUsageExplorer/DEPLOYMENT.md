# Component Usage Explorer - Build & Install

Deployment guide for the **Component Usage Explorer** XrmToolBox plugin. For the suite-wide guide
...
> ClosedXML/PdfSharp-MigraDoc-GDI dependency DLLs) into that tool's per-tool subfolder (Plugins\XrmToolSuite.ComponentUsageExplorer\) automatically; the tool DLL itself sits in the Plugins root. For a **manual**
> copy to another machine, copy **every** DLL from the tool's `bin\Release\net48\` folder (the tool DLL into the Plugins root, its dependency DLLs into the subfolder Plugins\XrmToolSuite.ComponentUsageExplorer\) - see the **"Export tools only"** section of
...
Then **restart XrmToolBox** and open **Component Usage Explorer**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.ComponentUsageExplorer\XrmToolSuite.ComponentUsageExplorer.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.ComponentUsageExplorer\bin\Release\net48\XrmToolSuite.ComponentUsageExplorer.dll
   ```
...
4. **Restart XrmToolBox**, search for **Component Usage Explorer**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.ComponentUsageExplorer.dll`. |

...
namespace XrmToolSuite.ComponentUsageExplorer
{
    ...
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "Component Usage Explorer"),
     ExportMetadata("Description", "Pick one Dataverse component and see its full where-used footprint (required + dependent components, per-type usage) plus a change-safety verdict (Safe to change / Change with caution / High impact / Do not delete / Requires dependency review / Requires ALM review). Read-only; exports Excel/PDF/JSON/HTML."),
    ...
    public class ComponentUsageExplorerPlugin : PluginBase
    {
        // The Excel-export (ClosedXML) and native-PDF (PdfSharp/MigraDoc) chains ship in a per-tool
        // SUBFOLDER named after this assembly (Plugins\XrmToolSuite.ComponentUsageExplorer\), the
        ...
        private static readonly string PluginDirectory =
            Path.GetDirectoryName(typeof(ComponentUsageExplorerPlugin).Assembly.Location) ?? string.Empty;
        ...
        {
            Path.Combine(PluginDirectory, typeof(ComponentUsageExplorerPlugin).Assembly.GetName().Name),
            PluginDirectory,
        };
        ...
        static ComponentUsageExplorerPlugin()
        {
        ...
        {
            return new ComponentUsageExplorerControl();
        }
    }
}

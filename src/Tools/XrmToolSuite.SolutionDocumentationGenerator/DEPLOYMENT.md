# Solution Documentation Generator - Build & Install

Deployment guide for the **Solution Documentation Generator** XrmToolBox plugin. For the suite-wide guide
...
> ClosedXML/PdfSharp-MigraDoc-GDI dependency DLLs) into that tool's per-tool subfolder (Plugins\XrmToolSuite.SolutionDocumentationGenerator\) automatically; the tool DLL itself sits in the Plugins root. For a **manual**
> copy to another machine, copy **every** DLL from the tool's `bin\Release\net48\` folder (the tool DLL into the Plugins root, its dependency DLLs into the subfolder Plugins\XrmToolSuite.SolutionDocumentationGenerator\) - see the **"Export tools only"** section of
...
Then **restart XrmToolBox** and open **Solution Documentation Generator**. Files written by your own build are not
...
2. Copy the **single** output DLL
   ...
   ```
   src\Tools\XrmToolSuite.SolutionDocumentationGenerator\bin\Release\net48\XrmToolSuite.SolutionDocumentationGenerator.dll
   ```
...
4. **Restart XrmToolBox**, search for **Solution Documentation Generator**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.SolutionDocumentationGenerator.dll`. |

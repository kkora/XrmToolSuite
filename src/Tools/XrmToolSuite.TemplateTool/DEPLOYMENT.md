# Template Tool - Build & Install

Deployment guide for the **Template Tool** XrmToolBox plugin. For the suite-wide guide
...
Then **restart XrmToolBox** and open **Template Tool**. Files written by your own build are not
...
1. Build in Release:
   ...
   ```powershell
   dotnet build src\Tools\XrmToolSuite.TemplateTool\XrmToolSuite.TemplateTool.csproj -c Release
   ```

2. Copy the **single** output DLL

   ```
   src\Tools\XrmToolSuite.TemplateTool\bin\Release\net48\XrmToolSuite.TemplateTool.dll
   ```
...
4. **Restart XrmToolBox**, search for **Template Tool**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.TemplateTool.dll`. |

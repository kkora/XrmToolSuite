...
namespace XrmToolSuite.ManagedSolutionImpactChecker
{
    ...
    [Export(typeof(IXrmToolBoxPlugin)),
     ExportMetadata("Name", "Managed Solution Impact Checker"),
     ExportMetadata("Description", "Read-only impact analysis for importing/upgrading/patching/deleting a managed solution: solution layering (unmanaged over managed), overwrite/deletion risk (path-aware — Upgrade deletes, Update/Patch do not), missing dependencies, publisher and managed-property restrictions. Produces an impact score, a pre-upgrade checklist, rollback guidance, and a CAB-ready Excel/PDF/JSON/HTML report."),
    ...
    public class ManagedSolutionImpactCheckerPlugin : PluginBase
    {
        // The Excel-export (ClosedXML) and native-PDF (PdfSharp/MigraDoc) chains ship in a per-tool
        // SUBFOLDER named after this assembly (Plugins\XrmToolSuite.ManagedSolutionImpactChecker\), the
        ...
        private static readonly string PluginDirectory =
            Path.GetDirectoryName(typeof(ManagedSolutionImpactCheckerPlugin).Assembly.Location) ?? string.Empty;
        ...
        {
            Path.Combine(PluginDirectory, typeof(ManagedSolutionImpactCheckerPlugin).Assembly.GetName().Name),
            PluginDirectory,
        };
        ...
        static ManagedSolutionImpactCheckerPlugin()
        {
        ...
        {
            return new ManagedSolutionImpactCheckerControl();
        }
    }
}

# Solution Merge Assistant - Build & Install

Deployment guide for the **Solution Merge Assistant** XrmToolBox plugin. For the suite-wide guide
...
> ClosedXML/PdfSharp-MigraDoc-GDI dependency DLLs) into that tool's per-tool subfolder (Plugins\XrmToolSuite.SolutionMergeAssistant\) automatically; the tool DLL itself sits in the Plugins root. For a **manual**
> copy to another machine, copy **every** DLL from the tool's `bin\Release\net48\` folder (the tool DLL into the Plugins root, its dependency DLLs into the subfolder Plugins\XrmToolSuite.SolutionMergeAssistant\) - see the **"Export tools only"** section of
...
Then **restart XrmToolBox** and open **Solution Merge Assistant**. Files written by your own build are not
...
2. Copy the **single** output DLL
   ...
   ```
   src\Tools\XrmToolSuite.SolutionMergeAssistant\bin\Release\net48\XrmToolSuite.SolutionMergeAssistant.dll
   ```
...
4. **Restart XrmToolBox**, search for **Solution Merge Assistant**, and connect to your environment.
...
| "Could not load file or assembly" for `Microsoft.Xrm.*` or `Newtonsoft.Json` | You copied a DLL XrmToolBox already ships: delete it and keep only `XrmToolSuite.SolutionMergeAssistant.dll`. |

...
4. **Execute + screenshots** — run `dotnet test`, perform the manual/GUI cases, and save evidence (screenshots) under `testing/<Tool>/screenshots/`.
...
7. Testing artifacts exist and are current under `testing/<Tool>/` (TEST_PLAN, TEST_CASES, TEST_SUMMARY, screenshots), user stories under `docs/user-stories/<Tool>.md`, and `dotnet test testing/UnitTests/UnitTests.csproj` passes. State plainly which cases were executed vs. pending-manual — never report a manual GUI case as passed if it wasn't run.

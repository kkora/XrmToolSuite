...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Audit Compliance Checker (opt-in, LOCAL only), following
    /// the operator script: launch XrmToolBox if needed, connect DEV, open the tool, "Check audit settings"
    /// (reads org/table/column audit config for the WHOLE environment — no solution, no target env), then
    /// "Analyze activity" over the default date range, browse the result tabs, export EACH format alongside the
    /// screenshots and open it, then open Help and close the tab. A screenshot of the XrmToolBox window ONLY is
    /// captured after every step (via PrintWindow, so the IDE/desktop never appear), under
    /// screenshots/&lt;yyyyMMdd&gt;/audit-compliance-checker/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class AuditComplianceCheckerE2ETest : IDisposable
    {
        ...
        private const string Tool = "Audit Compliance Checker";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension.
        // Order mirrors the Designer's tsddExport.DropDownItems: Excel, PDF, JSON, HTML, CSV.
        ...
        public AuditComplianceCheckerE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\audit-compliance-checker\  (created if missing).
            ...
            //    "Check audit settings" toolbar button (a tool-only control) rather than a tab caption.
            //    Retry the open if needed.
            var toolOpen = false;
            for (var i = 0; i < 3 && !toolOpen; i++)
            {
                if (i > 0) _host.HardReset();
                if (!_host.WaitForClickable("Check audit settings", TimeSpan.FromSeconds(2)))
                    _host.OpenTool(Tool);
                toolOpen = _host.WaitForClickable("Check audit settings", TimeSpan.FromSeconds(25));
            }
            Shot("03-tool-open");
            Check(toolOpen, $"'{Tool}' did not open (no 'Check audit settings' toolbar after retries).");

            // 3b) Validation guard — exporting BEFORE any check has run must pop the "Run \"Check audit settings\"…"
            //     info dialog rather than a Save dialog (Export returns early while the report is null). Drive the
            //     Export dropdown and pick the first format; assert a dialog (not a Save As) appears.
            if (_host.OpenExportMenu())
            {
                _host.SelectExportItem("Excel", 1);
                Thread.Sleep(1200);
                var guard = _host.DialogHwnd();
                ShotHwnd("03b-guard-no-report", guard);
                Check(guard != IntPtr.Zero, "Export with no report did not show the validation dialog.");
                _host.ClickProcessDialogButton("OK", TimeSpan.FromSeconds(5));
            }
            else
            {
                Check(false, "Could not open the Export dropdown for the validation-guard check.");
            }
            _host.HardReset();

            // 4) Click Check audit settings — reads org/table/column audit config for the whole environment.
            Check(_host.ClickByPartialName("Check audit settings"), "Could not find 'Check audit settings'.");
            Thread.Sleep(6000);   // let the async read run
            _host.HardReset();    // it mutates the tree heavily
            var hasCoverage = _host.WaitForGridRows(TimeSpan.FromSeconds(120));
            Shot("04-audit-settings-read");
            _output.WriteLine(hasCoverage ? "Audit-settings read produced coverage rows." : "Coverage grid rows not detected via UIA (still exportable).");
            _host.HardReset();

            // 5) Analyze activity — tallies audit records over the default (last-30-day) range. Heavy; give it time.
            Check(_host.ClickByPartialName("Analyze activity"), "Could not find 'Analyze activity'.");
            Thread.Sleep(6000);   // let the async activity tally run
            _host.HardReset();
            var hasActivity = _host.WaitForGridRows(TimeSpan.FromSeconds(120));
            Shot("05-activity-analyzed");
            _output.WriteLine(hasActivity ? "Activity analysis produced rows." : "Activity grid rows not detected via UIA (range may be empty; still exportable).");
            _host.HardReset();

            // 5b) Browse the Compliance score tab (populated once the settings read scored the environment).
            _host.ClickByName("Compliance score");
            Thread.Sleep(1200);
            Shot("05b-compliance-score");
            _host.HardReset();

            // 5c) Recommendations tab — select the first finding row so the walkthrough exercises the grid.
            //     This tool has no separate detail pane, so we only confirm a row can be selected (soft-logged).
            _host.ClickByName("Recommendations");
            Thread.Sleep(1000);
            var selected = _host.SelectFirstFinding();
            Thread.Sleep(600);
            Shot("05c-recommendations");
            _output.WriteLine(selected ? "Selected the first recommendation row." : "No recommendation row available to select (still exportable).");
            _host.HardReset();
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"AuditCompliance.{ext}";
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "audit-compliance-checker");

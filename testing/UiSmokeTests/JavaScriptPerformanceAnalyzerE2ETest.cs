...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the JavaScript Performance Analyzer tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, Analyze web
    /// resources (static scan of every JScript web resource), select the worst script, read its code/detail pane,
    /// export EACH format alongside the screenshots and open it, then open Help and close the tab. Like the
    /// Solution Complexity Score this tool needs NO target environment. A screenshot of the XrmToolBox window
    /// ONLY is captured after every step (via PrintWindow, so the IDE/desktop never appear), under
    /// screenshots/&lt;yyyyMMdd&gt;/javascript-performance-analyzer/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class JavaScriptPerformanceAnalyzerE2ETest : IDisposable
    {
        ...
        private const string Tool = "JavaScript Performance Analyzer";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension,
        // in Designer order: Excel, PDF, JSON, HTML, Markdown, CSV.
        ...
        public JavaScriptPerformanceAnalyzerE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\javascript-performance-analyzer\  (created if missing).
            ...
            // 3) Find the tool and double-click to open. Verify it REALLY opened by finding its
            //    "Analyze web resources" toolbar button (a tool-only control) rather than a tab caption.
            //    Retry the open if needed.
            var toolOpen = false;
            for (var i = 0; i < 3 && !toolOpen; i++)
            {
                if (i > 0) _host.HardReset();
                if (!_host.WaitForClickable("Analyze web resources", TimeSpan.FromSeconds(2)))
                    _host.OpenTool(Tool);
                toolOpen = _host.WaitForClickable("Analyze web resources", TimeSpan.FromSeconds(25));
            }
            Shot("03-tool-open");
            Check(toolOpen, $"'{Tool}' did not open (no 'Analyze web resources' toolbar after retries).");

            // 4) Analyze web resources — the primary flow: retrieve + statically scan every JScript web resource.
            //    Retry: the click or the async scan can transiently no-op on this flaky host.
            var hasRows = false;
            for (var i = 0; i < 3 && !hasRows; i++)
            {
                if (i > 0) { _host.HardReset(); Thread.Sleep(2000); }
                _host.ClickByPartialName("Analyze web resources");
            ...
            Shot("04-analyzed");
            Check(hasRows, "Analysis never produced script rows (no JScript web resources, or the scan failed).");
            _host.HardReset();

            // 5) Select the first (worst-ranked) script row so the findings grid + code/detail panes fill.
            var selected = _host.SelectFirstFinding();
            Thread.Sleep(800);
            Shot("05-script-selected");
            Check(selected, "Could not select the first script row.");

            // 6) Read the code / detail pane for the selected script (confirms the panes populated).
            var detail = _host.ReadDetailPane();
            Shot("06-code-detail");
            Check(!string.IsNullOrWhiteSpace(detail), "Selecting a script did not populate the code/detail pane.");
            _host.HardReset();
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"javascript-performance.{ext}";
            ...
            if (ext == "xlsx")
            {
                // The shared ReportModel Excel workbook has three worksheets — capture each (Ctrl+PageDown between them).
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "javascript-performance-analyzer");

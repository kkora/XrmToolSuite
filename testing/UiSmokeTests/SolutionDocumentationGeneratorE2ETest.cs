...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Solution Documentation Generator tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, Load solutions,
    /// use the first (auto-selected) solution, Generate the document, read the preview, export EACH format
    /// alongside the screenshots and open it, then open Help and close the tab. Like the Solution Complexity
    /// Score this tool needs NO target environment. A screenshot of the XrmToolBox window ONLY is captured after
    /// every step (via PrintWindow, so the IDE/desktop never appear), under
    /// screenshots/&lt;yyyyMMdd&gt;/solution-documentation-generator/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class SolutionDocumentationGeneratorE2ETest : IDisposable
    {
        ...
        private const string Tool = "Solution Documentation Generator";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension,
        // in the SAME order as the Export dropdown is built in the control's ctor (Word, PDF, Excel, Markdown,
        // HTML, HTML Portal, JSON). "HTML (" and "Portal" disambiguate the two .html formats.
        ...
        public SolutionDocumentationGeneratorE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\solution-documentation-generator\  (created if missing).
            ...
            // 3b) Validation guard — Generate with NO solution loaded must pop the "Load and select a solution
            //     first." dialog rather than build a document. Do this before any solution is loaded.
            _host.ClickByPartialName("Generate");
            Thread.Sleep(1200);
            var g = _host.DialogHwnd();
            ShotHwnd("03b-guard-no-solution", g);
            Check(g != IntPtr.Zero, "Generate with no solution did not show the validation dialog.");
            ...
            // 5) Confirm the first solution is auto-selected. "Load solutions" auto-selects index 0; the list is
            ...
            // 6) Generate — scans the solution and builds the document, then fills the preview textbox.
            Check(_host.ClickByPartialName("Generate"), "Could not find 'Generate'.");
            Thread.Sleep(6000);   // let the async scan/build run
            _host.HardReset();    // it mutates the tree heavily
            var preview = WaitForPreview(TimeSpan.FromSeconds(120));
            Shot("06-generated");
            Check(!string.IsNullOrWhiteSpace(preview), "Generate did not fill the document preview.");
            _output.WriteLine(string.IsNullOrWhiteSpace(preview)
                ? "Preview text not detected via UIA (still exportable)."
                : $"Preview produced {preview.Length} chars of documentation.");
            _host.HardReset();
        ...
        /// <summary>Poll (rebuilding the UIA tree) until the preview textbox holds a non-trivial document, or timeout.</summary>
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"SolutionDocumentationGenerator.{ext}";
            ...
            if (ext == "xlsx")
            {
                // The Excel workbook has an Overview sheet plus one worksheet per documented section — capture the
                // first few (Ctrl+PageDown between them) to evidence the multi-sheet export.
                for (var s = 0; s < 3; s++)
                {
                    ShotHwnd($"07-export-xlsx-3-report-{s + 1}", _host.ForegroundHwnd());
                    if (s < 2) { _host.NextExcelSheet(); Thread.Sleep(1500); }
                }
            }
            else
            {
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "solution-documentation-generator");

...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Sharing Analyzer tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, exercise the
    /// "pick tables or enable full scan first" validation guard, enable the Full-environment scan, run
    /// "Scan sharing", inspect the shares/findings grids, export EACH format alongside the screenshots and open
    ...
    /// the IDE/desktop never appear), under screenshots/&lt;yyyyMMdd&gt;/sharing-analyzer/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class SharingAnalyzerE2ETest : IDisposable
    {
        ...
        private const string Tool = "Sharing Analyzer";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension,
        // in the exact order they appear in SharingAnalyzerControl.Designer.cs.
        ...
        public SharingAnalyzerE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\sharing-analyzer\  (created if missing).
            ...
            // 3) Find the tool and double-click to open. Verify it REALLY opened by finding its "Scan sharing"
            ...
            for (var i = 0; i < 3 && !toolOpen; i++)
            {
                ...
                if (!_host.WaitForClickable("Scan sharing", TimeSpan.FromSeconds(2)))
                    _host.OpenTool(Tool);
                toolOpen = _host.WaitForClickable("Scan sharing", TimeSpan.FromSeconds(25));
            }
            Shot("03-tool-open");
            Check(toolOpen, $"'{Tool}' did not open (no 'Scan sharing' toolbar after retries).");

            // 3b) Validation guard — "Scan sharing" with NO tables picked and full-scan OFF must pop the
            //     "Pick one or more tables … or enable the full-environment scan first." dialog rather than scan.
            _host.ClickByPartialName("Scan sharing");
            Thread.Sleep(1200);
            var g = _host.DialogHwnd();
            ShotHwnd("03b-guard-no-tables", g);
            Check(g != IntPtr.Zero, "Scan with no tables/full-scan did not show the validation dialog.");
            _host.ClickProcessDialogButton("OK", TimeSpan.FromSeconds(5));
            _host.HardReset();

            // 4) Enable the Full-environment scan toggle (avoids driving the modal table picker). Toggling it on
            //    raises a Yes/No warning about POA volume — confirm with Yes. Retry: the click can transiently no-op.
            var fullScanOn = false;
            for (var i = 0; i < 3 && !fullScanOn; i++)
            {
                if (i > 0) { _host.HardReset(); Thread.Sleep(1500); }
                if (!_host.ClickByPartialName("Full-environment scan")) continue;
                Thread.Sleep(1200);
                var warn = _host.DialogHwnd();
                if (warn != IntPtr.Zero)
                {
                    ShotHwnd("04-fullscan-warning", warn);
                    fullScanOn = _host.ClickProcessDialogButton("Yes", TimeSpan.FromSeconds(5));
                }
            }
            _host.HardReset();
            Shot("04b-fullscan-enabled");
            Check(fullScanOn, "Could not enable the Full-environment scan (warning dialog not confirmed).");

            // 5) Scan sharing
            Check(_host.ClickByPartialName("Scan sharing"), "Could not find 'Scan sharing'.");
            Thread.Sleep(6000);   // let the async POA scan run
            _host.HardReset();    // it mutates the tree heavily
            var hasRows = _host.WaitForGridRows(TimeSpan.FromSeconds(120));
            Shot("05-scanned");
            _output.WriteLine(hasRows ? "Scan produced share/finding rows." : "Scan grid rows not detected via UIA (still exportable).");
            _host.HardReset();

            // 5b) Select the first share row so the selection is visible in the grid screenshot. (This tool has no
            //     read-only detail pane — the findings/intensity/recommendations live in their own tab grids.)
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"SharingAnalysis.{ext}";
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "sharing-analyzer");

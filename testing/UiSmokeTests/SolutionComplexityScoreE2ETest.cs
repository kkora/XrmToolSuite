...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Solution Complexity Score tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, Load solutions,
    /// use the first (auto-selected) solution, Score complexity, open the Executive summary and AI settings
    /// dialogs, export EACH format alongside the screenshots and open it, then open Help and close the tab.
    ...
    /// under screenshots/&lt;yyyyMMdd&gt;/solution-complexity-score/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class SolutionComplexityScoreE2ETest : IDisposable
    {
        ...
        private const string Tool = "Solution Complexity Score";
        ...
        public SolutionComplexityScoreE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\solution-complexity-score\  (created if missing).
            ...
            // 6) Score complexity
            Check(_host.ClickByPartialName("Score complexity"), "Could not find 'Score complexity'.");
            Thread.Sleep(6000);   // let the async scoring run
            _host.HardReset();    // it mutates the tree heavily
            var hasRows = _host.WaitForGridRows(TimeSpan.FromSeconds(120));
            Shot("06-scored");
            _output.WriteLine(hasRows ? "Scoring produced metric/hotspot rows." : "Scoring grid rows not detected via UIA (still exportable).");
            _host.HardReset();

            // 7) Executive summary — enabled after scoring; opens a summary dialog (offline when no API key).
            _host.ClickByPartialName("Executive summary");
            Thread.Sleep(3500);
            var aiHwnd = _host.DialogHwnd();
            ShotHwnd("07-exec-summary", aiHwnd);
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "solution-complexity-score");

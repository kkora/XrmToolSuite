...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Team Permission Explorer tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, exercise the
    /// "load teams and select the first team, then click Compare" validation guard, Load teams, select the first
    /// team so its members/roles/effective-privileges/owned-records/findings tabs bind, export EACH format
    /// alongside the screenshots and open it, then open Help and close the tab. Like the Sharing Analyzer this
    /// tool needs NO target environment. A screenshot of the XrmToolBox window ONLY is captured after every step
    /// (via PrintWindow, so the IDE/desktop never appear), under
    /// screenshots/&lt;yyyyMMdd&gt;/team-permission-explorer/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class TeamPermissionExplorerE2ETest : IDisposable
    {
        ...
        private const string Tool = "Team Permission Explorer";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension,
        // in the exact order they appear in TeamPermissionExplorerControl.Designer.cs.
        ...
        public TeamPermissionExplorerE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\team-permission-explorer\  (created if missing).
            ...
            // 3) Find the tool and double-click to open. Verify it REALLY opened by finding its "Load teams"
            ...
            for (var i = 0; i < 3 && !toolOpen; i++)
            {
                ...
                if (!_host.WaitForClickable("Load teams", TimeSpan.FromSeconds(2)))
                    _host.OpenTool(Tool);
                toolOpen = _host.WaitForClickable("Load teams", TimeSpan.FromSeconds(25));
            }
            Shot("03-tool-open");
            Check(toolOpen, $"'{Tool}' did not open (no 'Load teams' toolbar after retries).");

            // 3b) Validation guard — "Compare…" with NO teams loaded / no team selected must pop the
            //     "Load teams and select the first team, then click Compare." dialog rather than compare.
            _host.ClickByPartialName("Compare");
            Thread.Sleep(1200);
            var g = _host.DialogHwnd();
            ShotHwnd("03b-guard-no-team", g);
            Check(g != IntPtr.Zero, "Compare with no team selected did not show the validation dialog.");
            _host.ClickProcessDialogButton("OK", TimeSpan.FromSeconds(5));
            _host.HardReset();

            // 4) Click Load teams — retry: the click or the async load can transiently no-op on this flaky host.
            var hasRows = false;
            for (var i = 0; i < 3 && !hasRows; i++)
            {
                if (i > 0) { _host.HardReset(); Thread.Sleep(2000); }
                _host.ClickByPartialName("Load teams");
                hasRows = _host.WaitForGridRows(TimeSpan.FromSeconds(120));
            }
            _host.HardReset();    // loading teams mutates the tree heavily / poisons the UIA cache
            Shot("04-teams-loaded");
            _output.WriteLine(hasRows ? "Load produced team rows." : "Team grid rows not detected via UIA (still exportable).");

            // 5) Select the first team row so its detail tabs (Members / Roles / Effective privileges /
            //    Owned records / Findings) bind, and the selection is visible in the grid screenshot.
            _host.SelectFirstFinding();
            Thread.Sleep(1200);   // let the on-demand member/detail bind run
            Shot("05-team-selected");
            _host.HardReset();
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"TeamPermissions.{ext}";
            ...
            if (ext == "xlsx")
            {
                // Excel workbook (shared ReportModel exporter) has three worksheets (Summary, Findings, Fix
                // Checklist) — capture each (Ctrl+PageDown between them).
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "team-permission-explorer");

...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Environment Comparison Suite (opt-in, LOCAL only), following
    /// the operator script: launch XrmToolBox if needed, connect the SOURCE env, open the tool, connect a
    /// SECOND (TARGET) env via the connection selector, Compare, select a difference, then export EACH format
    /// alongside the screenshots and open it, then open Help and close the tab. Like the Deployment Risk
    /// Analyzer this tool needs a TARGET environment (a second, additional connection raised via
    /// RaiseRequestConnectionEvent) — connecting it must NOT drop the SOURCE connection. A screenshot of the
    /// XrmToolBox window ONLY is captured after every step (via PrintWindow, so the IDE/desktop never appear),
    /// under screenshots/&lt;yyyyMMdd&gt;/environment-comparison-suite/NN-step.png.
    ///
    /// Constraints (see README Tier-3b): interactive connections can't authenticate unattended, so BOTH envs
    /// must have a warm token; desktop must stay UNLOCKED. Gated behind XTB_E2E=1.
    ///
    /// Env vars: XTB_E2E=1 to run; XTB_EXE; XTB_SOURCE (default "XTS-CI-DEV"); XTB_TARGET (default "XTS-CI-TEST");
    ///           UISMOKE_SCREENSHOT_DIR.
    /// </summary>
    public sealed class EnvironmentComparisonSuiteE2ETest : IDisposable
    {
        ...
        private const string Tool = "Environment Comparison Suite";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension.
        // Order MUST match the toolbar dropdown build order in EnvironmentComparisonSuiteControl.BuildUi().
        private static readonly (string Menu, int Index, string Ext)[] Exports =
        {
            ("Excel workbook", 1, "xlsx"),
            ("PDF report", 2, "pdf"),
            ("JSON", 3, "json"),
            ("HTML report", 4, "html"),
        };

        public EnvironmentComparisonSuiteE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            {
                _output.WriteLine("Skipped: set XTB_E2E=1 (desktop unlocked, both env tokens warm) to run. See README Tier-3b.");
                return;
            }
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\environment-comparison-suite\  (created if missing).
            ...
            // 2) Connect to the SOURCE env (select it -> Connect if not connected). This is the PRIMARY connection.
            var connected = _host.EnsureConnected(source, TimeSpan.FromSeconds(90));
            Shot("02-connect-source");
            ...
            // 3) Find the tool and double-click to open. Verify it REALLY opened by finding its "Connect target
            //    env…" toolbar button (a tool-only control) rather than a tab caption. Retry the open if needed.
            var toolOpen = false;
            for (var i = 0; i < 3 && !toolOpen; i++)
            {
                if (i > 0) _host.HardReset();
                if (!_host.WaitForClickable("Connect target env", TimeSpan.FromSeconds(2)))
                    _host.OpenTool(Tool);
                toolOpen = _host.WaitForClickable("Connect target env", TimeSpan.FromSeconds(25));
            }
            Shot("03-tool-open");
            Check(toolOpen, $"'{Tool}' did not open (no 'Connect target env' toolbar after retries).");

            // 3b) VALIDATION GUARD — Compare with NO target connected must pop the "Connect a target environment
            //     first." dialog rather than compare. Do this before connecting the target env.
            _host.ClickByPartialName("Compare");
            Thread.Sleep(1200);
            var noTargetHwnd = _host.DialogHwnd();
            ShotHwnd("03b-guard-no-target", noTargetHwnd);
            Check(noTargetHwnd != IntPtr.Zero, "Compare without a target env did not show the validation dialog.");
            _host.ClickProcessDialogButton("OK", TimeSpan.FromSeconds(5));
            _host.HardReset();

            // 4) Click Connect target env… (best-effort: the real verification is step 5's Target label — the
            ...
            Thread.Sleep(800);
            ShotDialogOrMain("04-target-dialog");

            // 5) Select the TARGET connection and double-click it -> the "Target: <name>" label turns green.
            ...
            Shot("05-target-connected");
            Check(targetLabel != null, $"Target env '{target}' was not connected (label stayed '(none)').");

            // 5b) Dual-connection: connecting the target must NOT drop the SOURCE connection (status bar still live).
            Check(_host.IsConnectedTo(""), "Source connection was lost after connecting the target env.");
            _host.HardReset();

            // 6) Click Compare
            Check(_host.ClickByPartialName("Compare"), "Could not find 'Compare'.");
            Thread.Sleep(6000);   // let the async comparison run
            _host.HardReset();    // it mutates the tree heavily
            var hasRows = _host.WaitForGridRows(TimeSpan.FromSeconds(120));
            Shot("06-comparison-complete");
            _output.WriteLine(hasRows ? "Comparison produced difference rows." : "Comparison grid rows not detected via UIA (still exportable).");
            _host.HardReset();

            // 6b) Score / band is shown in the summary banner (e.g. "HIGH DIFFERENCE — score NN/100").
            var risk = _host.ReadRiskSummary();
            Shot("06b-score-band");
            Check(!string.IsNullOrWhiteSpace(risk), "Difference summary banner (score/band) was not found.");
            _output.WriteLine($"Score summary: \"{risk}\".");

            // 6c) Select the first difference row -> the side-by-side (property | source | target) detail grid fills.
            var selected = _host.SelectFirstFinding();
            Thread.Sleep(800);
            Shot("06c-difference-selected");
            Check(selected, "Could not select a difference row in the results grid.");
            _host.HardReset();
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"EnvironmentComparison.{ext}";
            ...
            if (ext == "xlsx")
            {
                // Excel workbook has three worksheets (Summary, Findings, Fix Checklist) — capture each (Ctrl+PageDown between them).
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "environment-comparison-suite");

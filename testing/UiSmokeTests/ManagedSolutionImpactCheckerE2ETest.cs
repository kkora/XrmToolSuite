...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Managed Solution Impact Checker tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, Refresh solutions,
    /// use the first (auto-selected) managed solution on the default (Upgrade) deployment path, Analyze impact,
    /// select the first finding, then export EACH format alongside the screenshots and open it, then open Help
    /// and close the tab. Unlike the Deployment Risk Analyzer this tool needs NO target environment. A screenshot
    /// of the XrmToolBox window ONLY is captured after every step (via PrintWindow, so the IDE/desktop never
    /// appear), under screenshots/&lt;yyyyMMdd&gt;/managed-solution-impact-checker/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class ManagedSolutionImpactCheckerE2ETest : IDisposable
    {
        ...
        private const string Tool = "Managed Solution Impact Checker";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension.
        // Order MUST match the Designer's tsbExport.DropDownItems: Excel, PDF, JSON, HTML.
        ...
        public ManagedSolutionImpactCheckerE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\managed-solution-impact-checker\  (created if missing).
            ...
            // 3) Find the tool and double-click to open. Verify it REALLY opened by finding its "Refresh solutions"
            ...
            for (var i = 0; i < 3 && !toolOpen; i++)
            {
                ...
                if (!_host.WaitForClickable("Refresh solutions", TimeSpan.FromSeconds(2)))
                    _host.OpenTool(Tool);
                toolOpen = _host.WaitForClickable("Refresh solutions", TimeSpan.FromSeconds(25));
            }
            Shot("03-tool-open");
            Check(toolOpen, $"'{Tool}' did not open (no 'Refresh solutions' toolbar after retries).");

            // 3b) Validation guard — Analyze impact with NO solution selected must pop the "No solution selected"
            //     dialog rather than analyze. Do this before any solution is loaded.
            _host.ClickByPartialName("Analyze impact");
            Thread.Sleep(1200);
            var g = _host.DialogHwnd();
            ShotHwnd("03b-guard-no-solution", g);
            Check(g != IntPtr.Zero, "Analyze with no solution did not show the validation dialog.");
            _host.ClickProcessDialogButton("OK", TimeSpan.FromSeconds(5));
            _host.HardReset();

            // 4) Click Refresh solutions — retry: the click or the async load can transiently no-op on this flaky host.
            string loaded = null;
            for (var i = 0; i < 3 && string.IsNullOrWhiteSpace(loaded); i++)
            {
                if (i > 0) { _host.HardReset(); Thread.Sleep(2000); }
                _host.ClickByPartialName("Refresh solutions");
            ...
            // 5) Confirm the first managed solution is auto-selected. "Refresh solutions" auto-selects index 0; the
            // list is UIA-virtualized so we confirm the auto-selection rather than driving the dropdown (which,
            // left open, blocks every later click). The deployment path combo defaults to "Upgrade".
            var chosen = _host.ReadComboValue();
            Shot("05-solution-selected");
            Check(!string.IsNullOrWhiteSpace(chosen), "No solution is selected after Refresh solutions.");
            _output.WriteLine($"Step 5 - using first managed solution: \"{chosen}\" on the default (Upgrade) path.");

            // 6) Analyze impact (read-only layering / upgrade / delete analysis on the default Upgrade path).
            Check(_host.ClickByPartialName("Analyze impact"), "Could not find 'Analyze impact'.");
            ...
            Shot("06-analyzed");
            _output.WriteLine(hasRows ? "Analysis produced finding rows." : "Findings grid rows not detected via UIA (still exportable).");
            _host.HardReset();

            // 6b) Finding row -> summary pane: select the first finding and confirm the impact summary is populated.
            _host.SelectFirstFinding();
            Thread.Sleep(800);
            var detail = _host.ReadDetailPane();
            Shot("06b-finding-detail");
            Check(!string.IsNullOrWhiteSpace(detail), "Selecting a finding did not populate the summary pane.");
            _host.HardReset();
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"ManagedSolutionImpactChecker.{ext}";
            ...
            if (ext == "xlsx")
            {
                // Excel workbook (shared ExcelReportExporter) has three worksheets — capture each (Ctrl+PageDown between them).
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "managed-solution-impact-checker");

...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Duplicate Metadata Finder tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, Scan for
    /// duplicates, select the first duplicate group (detail pane fills), export EACH format alongside the
    /// screenshots and open it, then open Help and close the tab. Like the Solution Complexity Score this
    /// tool needs NO target environment — it scans the connected org's metadata. A screenshot of the
    /// XrmToolBox window ONLY is captured after every step (via PrintWindow, so the IDE/desktop never appear),
    /// under screenshots/&lt;yyyyMMdd&gt;/duplicate-metadata-finder/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class DuplicateMetadataFinderE2ETest : IDisposable
    {
        ...
        private const string Tool = "Duplicate Metadata Finder";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension.
        // Order matches the Designer's tsddExport.DropDownItems: Excel, PDF, JSON, HTML.
        ...
        public DuplicateMetadataFinderE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\duplicate-metadata-finder\  (created if missing).
            ...
            // 3) Find the tool and double-click to open. Verify it REALLY opened by finding its "Scan for
            //    duplicates" toolbar button (a tool-only control) rather than a tab caption. Retry the open.
            var toolOpen = false;
            for (var i = 0; i < 3 && !toolOpen; i++)
            {
                if (i > 0) _host.HardReset();
                if (!_host.WaitForClickable("Scan for duplicates", TimeSpan.FromSeconds(2)))
                    _host.OpenTool(Tool);
                toolOpen = _host.WaitForClickable("Scan for duplicates", TimeSpan.FromSeconds(25));
            }
            Shot("03-tool-open");
            Check(toolOpen, $"'{Tool}' did not open (no 'Scan for duplicates' toolbar after retries).");

            // 4) Scan for duplicates — retry: the click or the async scan can transiently no-op on this flaky host.
            Check(_host.ClickByPartialName("Scan for duplicates"), "Could not find 'Scan for duplicates'.");
            Thread.Sleep(6000);   // let the async metadata scan run
            _host.HardReset();    // it mutates the tree heavily
            var hasRows = _host.WaitForGridRows(TimeSpan.FromSeconds(180));
            Shot("04-scanned");
            _output.WriteLine(hasRows ? "Scan produced duplicate-group rows." : "Scan grid rows not detected via UIA (still exportable).");
            _host.HardReset();

            // 5) Select the first duplicate group -> detail pane: confirm the read-only detail box fills.
            _host.SelectFirstFinding();
            Thread.Sleep(800);
            var detail = _host.ReadDetailPane();
            Shot("05-group-detail");
            Check(!string.IsNullOrWhiteSpace(detail), "Selecting a duplicate group did not populate the detail pane.");
            _host.HardReset();
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"duplicate-metadata.{ext}";
            ...
            if (ext == "xlsx")
            {
                // The shared ExcelReportExporter writes three worksheets — capture each (Ctrl+PageDown between them).
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "duplicate-metadata-finder");

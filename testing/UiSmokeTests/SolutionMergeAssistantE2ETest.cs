...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Solution Merge Assistant tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, Load solutions,
    /// check TWO solutions in the list, Compare (a READ-ONLY pre-merge comparison — this tool never imports or
    /// writes solutions, so there is no destructive step to guard), select the first conflict to fill the detail
    /// pane, export EACH format alongside the screenshots and open it, then open Help and close the tab.
    ...
    /// under screenshots/&lt;yyyyMMdd&gt;/solution-merge-assistant/NN-step.png.
    ///
    ...
    /// XTB_MERGE_A / XTB_MERGE_B — two DISTINCT solution-name fragments in the source org to check for the
    /// comparison (a CheckedListBox row is matched by substring). Empty picks the first row; set BOTH to two
    /// different solutions so exactly two DISTINCT rows get checked.
    /// </summary>
    public sealed class SolutionMergeAssistantE2ETest : IDisposable
    {
        ...
        private const string Tool = "Solution Merge Assistant";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension.
        // Order mirrors the Designer's DropDownItems.AddRange: Excel, PDF, JSON, HTML.
        ...
        public SolutionMergeAssistantE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            var hintA = EnvOr("XTB_MERGE_A", "");   // solution-name fragment for the FIRST solution to check
            var hintB = EnvOr("XTB_MERGE_B", "");   // …and the SECOND (must differ from A)
            // Exports are saved alongside the screenshots, in the same per-date tool folder:
            // …\UiSmokeTests\screenshots\<yyyymmdd>\solution-merge-assistant\  (created if missing).
            ...
            // 3b) Validation guard — Compare with NOTHING checked must pop the "Check at least two solutions to
            //     compare" dialog rather than analyze. Do this before any solution is checked.
            _host.ClickByPartialName("Compare");
            Thread.Sleep(1200);
            var g = _host.DialogHwnd();
            ShotHwnd("03b-guard-need-two", g);
            Check(g != IntPtr.Zero, "Compare with nothing checked did not show the validation dialog.");
            ...
            for (var i = 0; i < 3; i++)
            {
                ...
                Thread.Sleep(8000);   // let the async solution load finish
            }
            _host.HardReset(); // loading solutions poisons the UIA cache
            Shot("04-solutions-loaded");

            // 5) Check TWO solutions in the list to compare (read-only). ToggleAnalyzer flips a CheckedListBox row
            //    by name-substring; two DISTINCT fragments check two DISTINCT rows. Retry re-loading if the rows
            //    aren't in the tree yet (the list can populate slowly on this host).
            var twoChecked = false;
            for (var i = 0; i < 3 && !twoChecked; i++)
            {
                if (i > 0) { _host.HardReset(); _host.ClickByPartialName("Load solutions"); Thread.Sleep(6000); _host.HardReset(); }
                var a = _host.ToggleAnalyzer(hintA);
                Thread.Sleep(700);
                var b = _host.ToggleAnalyzer(hintB);
                twoChecked = a && b;
            }
            Shot("05-solutions-checked");
            Check(twoChecked, "Could not check two solutions in the list " +
                              "(set XTB_MERGE_A / XTB_MERGE_B to two DISTINCT solution-name fragments in the source org).");
            _host.HardReset();

            // 6) Compare — the read-only pre-merge comparison. Populates the verdict banner, conflict grid,
            //    recommended-strategy and merged-component checklist. This never writes/imports a solution.
            ...
            Shot("06-compared");
            _output.WriteLine(hasRows ? "Comparison produced conflict rows." : "No conflict rows (a clean merge is a valid outcome; report still exportable).");
            _host.HardReset();

            // 7) Conflict row -> detail: select the first conflict finding and read the detail (strategy/checklist)
            //    pane. Best-effort — a clean merge has no conflict rows, so don't fail the run when none exist.
            if (hasRows && _host.SelectFirstFinding())
            {
                Thread.Sleep(800);
                var detail = _host.ReadDetailPane();
                Shot("07-conflict-detail");
                _output.WriteLine(string.IsNullOrWhiteSpace(detail) ? "Conflict selected but detail pane was empty." : "Conflict detail pane populated.");
            }
            else
            {
                Shot("07-conflict-detail");
                _output.WriteLine("No conflict row to select (clean comparison).");
            }
            _host.HardReset();

            // 8) Export each option -> menu shot -> Save dialog shot -> save to the screenshots folder -> Yes -> report shot.
            //    Export is enabled only after a Compare. Retry once per format: the menu-item selection can
            //    transiently miss on this flaky host.
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"solution-merge.{ext}";
            ...
            if (ext == "xlsx")
            {
                // Excel workbook has three worksheets (Summary, Findings, Checklist) — capture each (Ctrl+PageDown between them).
                var sheets = new[] { "summary", "findings", "checklist" };
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "solution-merge-assistant");

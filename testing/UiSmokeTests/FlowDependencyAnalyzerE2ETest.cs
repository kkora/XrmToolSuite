...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Flow Dependency Analyzer tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, Analyze flows
    /// (retrieve every cloud flow and map its dependencies), select the first flow to render its dependency
    /// tree, then export EACH format alongside the screenshots and open it, then open Help and close the tab.
    /// This tool needs NO target environment and loads flows directly (no "Load solutions" step). A screenshot
    /// of the XrmToolBox window ONLY is captured after every step (via PrintWindow, so the IDE/desktop never
    /// appear), under screenshots/&lt;yyyyMMdd&gt;/flow-dependency-analyzer/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class FlowDependencyAnalyzerE2ETest : IDisposable
    {
        ...
        private const string Tool = "Flow Dependency Analyzer";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension.
        // Order matches the Designer's DropDownItems: Excel (.xlsx), PDF (.pdf), JSON (.json), HTML (.html).
        ...
        public FlowDependencyAnalyzerE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\flow-dependency-analyzer\  (created if missing).
            ...
            // 3) Find the tool and double-click to open. Verify it REALLY opened by finding its "Analyze flows"
            ...
            for (var i = 0; i < 3 && !toolOpen; i++)
            {
                ...
                if (!_host.WaitForClickable("Analyze flows", TimeSpan.FromSeconds(2)))
                    _host.OpenTool(Tool);
                toolOpen = _host.WaitForClickable("Analyze flows", TimeSpan.FromSeconds(25));
            }
            Shot("03-tool-open");
            Check(toolOpen, $"'{Tool}' did not open (no 'Analyze flows' toolbar after retries).");

            // 4) Analyze flows — the primary read-only flow: retrieve every cloud flow and map dependencies.
            //    Retry: the click or the async collect can transiently no-op on this flaky host.
            var hasRows = false;
            for (var i = 0; i < 3 && !hasRows; i++)
            {
                if (i > 0) { _host.HardReset(); Thread.Sleep(2000); }
                _host.ClickByPartialName("Analyze flows");
                Thread.Sleep(6000);   // let the async collect run
                _host.HardReset();    // it mutates the grid/tree heavily
                hasRows = _host.WaitForGridRows(TimeSpan.FromSeconds(120));
            }
            Shot("04-analyzed");
            _output.WriteLine(hasRows ? "Analysis produced flow rows." : "Flow grid rows not detected via UIA (still exportable).");
            _host.HardReset();

            // 5) Select the first flow row so its dependency tree renders in the detail pane.
            _host.SelectFirstFinding();
            Thread.Sleep(800);
            var detail = _host.ReadDetailPane(); // best-effort: the detail here is a TreeView, may read empty
            Shot("05-flow-selected");
            _output.WriteLine(string.IsNullOrWhiteSpace(detail)
                ? "Selected first flow (dependency tree rendered; no read-only Edit pane to read)."
                : "Selected first flow; detail pane text captured.");
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"flow-dependencies.{ext}";
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "flow-dependency-analyzer");

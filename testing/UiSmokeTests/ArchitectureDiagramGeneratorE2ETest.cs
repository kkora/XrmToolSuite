...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Architecture Diagram Generator tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, Load solutions,
    /// use the first (auto-selected) solution, Generate the diagram, confirm the preview populates, flip the
    /// "Hide orphans" toggle, export EACH format alongside the screenshots and open it, then open Help and close
    /// the tab. Like the Solution Complexity Score this tool needs NO target environment. A screenshot of the
    /// XrmToolBox window ONLY is captured after every step (via PrintWindow, so the IDE/desktop never appear),
    /// under screenshots/&lt;yyyyMMdd&gt;/architecture-diagram-generator/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class ArchitectureDiagramGeneratorE2ETest : IDisposable
    {
        ...
        private const string Tool = "Architecture Diagram Generator";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension.
        // All six are single-file, BCL-only formats — none is a multi-sheet workbook, so there is no xlsx branch.
        ...
        public ArchitectureDiagramGeneratorE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\architecture-diagram-generator\  (created if missing).
            ...
            //     first." dialog rather than build. Do this before any solution is loaded.
            ...
            // 5) Confirm the first solution is auto-selected. "Load solutions" already auto-selects index 0; the
            // list is UIA-virtualized so we confirm the auto-selection rather than driving the dropdown (which,
            // left open, blocks every later click).
            ...
            // Results render into the read-only multiline preview textbox (not a grid) — confirm it filled.
            var preview = _host.ReadDetailPane();
            Shot("06-generated");
            _output.WriteLine(!string.IsNullOrWhiteSpace(preview)
                ? "Generate produced diagram source in the preview pane."
                : "Preview text not detected via UIA (still exportable).");
            _host.HardReset();

            // 6b) "Hide orphans" toggle: flip the toolbar toggle (re-renders the preview), screenshot, restore.
            var tog = _host.ClickByPartialName("Hide orphans");
            Thread.Sleep(700);
            Shot("06b-hide-orphans-toggled");
            Check(tog, "Could not toggle 'Hide orphans'.");
            _host.ClickByPartialName("Hide orphans");
            _host.HardReset();
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"ArchitectureDiagramGenerator.{ext}";
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "architecture-diagram-generator");

...
{
    /// <summary>
    /// TIER-3c — full END-TO-END walkthrough of the Environment Inventory tool (opt-in, LOCAL only),
    /// following the operator script: launch XrmToolBox if needed, connect DEV, open the tool, Collect
    /// inventory of the connected org, select the first inventoried component and read its detail pane,
    /// then export EACH format alongside the screenshots and open it, then open Help and close the tab.
    /// Unlike the Deployment Risk Analyzer this tool needs NO target environment, and unlike the Solution
    /// Complexity Score it needs NO solution selection — it inventories the whole connected org. A
    /// screenshot of the XrmToolBox window ONLY is captured after every step (via PrintWindow, so the
    /// IDE/desktop never appear), under screenshots/&lt;yyyyMMdd&gt;/environment-inventory/NN-step.png.
    ///
    ...
    /// </summary>
    public sealed class EnvironmentInventoryE2ETest : IDisposable
    {
        ...
        private const string Tool = "Environment Inventory";

        // Export menu items (popup text fragment, 1-based position in the dropdown) -> saved file extension.
        // Order matches the tsdExport.DropDownItems.AddRange sequence in the Designer.
        ...
        public EnvironmentInventoryE2ETest(ITestOutputHelper output) => _output = output;
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // …\UiSmokeTests\screenshots\<yyyymmdd>\environment-inventory\  (created if missing).
            ...
            // 3) Find the tool and double-click to open. Verify it REALLY opened by finding its "Collect
            //    inventory" toolbar button (a tool-only control) rather than a tab caption. Retry the open if needed.
            var toolOpen = false;
            ...
            Shot("05-component-detail");
            Check(!string.IsNullOrWhiteSpace(detail), "Selecting a component did not populate the detail pane.");
            _host.HardReset();
        ...
        private bool ExportOne(string menuText, int index, string ext, string saveDir)
        {
            ...
            if (string.IsNullOrWhiteSpace(defaultName)) defaultName = $"environment-inventory.{ext}";
            ...
            if (ext == "xlsx")
            {
                // Excel workbook has two worksheets — capture each (Ctrl+PageDown between them).
                var sheets = new[] { "summary", "items" };
        ...
        private string RunDir() =>
            Path.Combine(ScreenshotRoot(), _dateStamp, "environment-inventory");

...
namespace XrmToolSuite.UiSmokeTests
{
    ...
    public sealed class DeploymentRiskAnalyzerE2ETest : IDisposable
    {
        ...
        public void FullOperatorWalkthrough()
        {
            ...
            // 11) Close the tool via its own Close button (tears down the tool tab -> back to the Tools list).
            _host.ForceForeground();
            _host.HardReset();
            var closed = _host.ClickToolClose();
            Thread.Sleep(1500);
            Shot("11-tool-closed");
            Check(closed, "Could not click the tool's Close button.");

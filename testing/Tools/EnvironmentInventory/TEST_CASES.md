...
| TC-M2 | Collect inventory off-thread with progress | US-ADMIN07.1.1 / .2.2 | Click "Load inventory" | Spinner shows progress messages; UI stays responsive; grid fills; status shows total count | Manual | Pending |
...
| TC-M12 | Export Word (catalog) | US-ADMIN07.5.1 | Filter, then Export ▼ Word (*.docx) | .docx opens in Word; title + environment + collected date, Key-metrics counts, then the filtered records grouped by category in a table per category (Type/Name/Schema/Managed/Modified); NO score gauge / severity grid; no secrets | Manual | Pending |
| TC-M13 | Export PDF (catalog) | US-ADMIN07.5.1 | Filter, then Export ▼ PDF (*.pdf) | .pdf opens in a viewer; native PdfSharp/MigraDoc catalog — title + key-metric counts + filtered records grouped by category (Type/Name/Schema/Managed/Modified); NO score gauge / severity grid (unlike the analyzer report); no secrets | Manual | Pending |
...
| TC-M15 | Column-header sort stays responsive on large env | US-ADMIN07.4.1 | On a 100k+ component org, click each column header; click a header twice | Grid re-sorts by that column (asc, then desc on second click) with a sort glyph on the active column; tool does NOT hang / go "not responding" | Manual | Pending |
| TC-M16 | Processing indicator on filter/search | US-ADMIN07.4.1 | Type in Search; change a dropdown; sort a large result set | Search re-filters after typing pauses (not per keystroke); a "Filtering…" badge + wait cursor appear over the grid during the filter/sort/re-bind; status shows "Showing N of M" | Manual | Pending |
| TC-M17 | Export honors the active filter | US-ADMIN07.5.1 | Filter to a category (e.g. Configuration showing "163 of 134064"); Export ▼ each format | Every file (CSV/JSON/MD/HTML/Excel/Word/PDF) contains only the 163 filtered rows and their summary counts — not the full 134064; status shows "Exported 163 component(s)". Clearing filters exports the full set | Manual | Pending |

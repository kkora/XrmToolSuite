...
> **Suggested tag:** `PERF02` · **Suggested project:** `XrmToolSuite.ApiLatencyAnalyzer`
...
## EPIC-PERF02 — Measure Dataverse API latency to localize performance problems
...
## FEAT-PERF02-1 — Test configuration `[Planned]`
- **US-PERF02.1.1** `[Planned]` **As** a PERF engineer, **I want** to configure which operations, tables, and iteration counts to test, **so that** I control scope and cost.
  - **AC:** A config panel drives operation/table/iteration selection; configuration persists via settings.
- **US-PERF02.1.2** `[Planned]` **As** a PERF engineer, **I want** read-only mode selected by default, **so that** I can never accidentally write to production.
...
## FEAT-PERF02-2 — Read-operation latency tests `[Planned]`
- **US-PERF02.2.1** `[Planned]` **As** a PERF engineer, **I want** timed Retrieve and RetrieveMultiple tests, **so that** I can baseline core read latency.
  - **AC:** Each test runs N iterations off the UI thread with progress/cancellation and records per-call latency.
- **US-PERF02.2.2** `[Planned]` **As** a PERF engineer, **I want** FetchXML and QueryExpression latency tested, **so that** I can compare query styles.
  - **AC:** Both query paths are timed against the same table for a like-for-like comparison.
- **US-PERF02.2.3** `[Planned]` **As** a PERF engineer, **I want** selected-columns vs all-columns and with vs without related entities compared, **so that** I can quantify payload cost.
  - **AC:** Results grid shows paired latencies for each comparison dimension.

## FEAT-PERF02-3 — Safe write-operation tests `[Planned]`
- **US-PERF02.3.1** `[Planned]` **As** a PERF engineer, **I want** Create/Update/Delete latency tested only after an explicit confirmation dialog, **so that** writes never happen by surprise.
...
- **US-PERF02.3.2** `[Planned]` **As** a PERF engineer, **I want** every test record auto-cleaned, **so that** the environment is left as found.
  - **AC:** Created record IDs are tracked and deleted in a `finally`; the summary reports created vs cleaned counts.

## FEAT-PERF02-4 — Latency metrics & slow-op detection `[Planned]`
- **US-PERF02.4.1** `[Planned]` **As** a PERF engineer, **I want** min/max/average/median/P95 per operation, **so that** I see distribution, not just an average.
  - **AC:** The dashboard shows all five statistics per operation/table.
- **US-PERF02.4.2** `[Planned]` **As** a PERF engineer, **I want** slow operations detected and network-vs-server patterns hinted, **so that** I can localize the bottleneck.
...
## FEAT-PERF02-5 — Baseline, comparison & export `[Planned]`
- **US-PERF02.5.1** `[Planned]` **As** a PERF engineer, **I want** to save a baseline and compare the current scan to it, **so that** I can detect regressions over time.
  - **AC:** Baseline saves locally (no credentials); a comparison view shows deltas per operation.
- **US-PERF02.5.2** `[Planned]` **As** a MGR, **I want** Excel/PDF/JSON/HTML/CSV latency reports, **so that** I can share findings.

...
> **Suggested tag:** `PERF3` · **Suggested project:** `XrmToolSuite.FetchXmlPerformanceAnalyzer`
> **Overlaps:** None as a standalone tool — but its core parser/rule engine should become a **shared FetchXML analyzer** in `src/Shared/Core/` reused by View Performance Analyzer (PERF4), Dashboard Performance Checker (PERF5), and any future report/portal analyzer. Build the analyzer UI-free first, wrap it here.
...
- **Design as a shared, UI-free analyzer:** a pure parser + rule engine operating on a FetchXML string, returning findings. The WinForms tool is a thin host; PERF4/PERF5 consume the same engine.
...
## FEAT-PERF3-1 — Input & load `[Planned]`
- **US-PERF3.1.1** `[Planned]` **As** a PERF engineer, **I want** to paste FetchXML into an editor and validate its syntax, **so that** I can analyze ad-hoc queries.
  - **AC:** Invalid FetchXML shows a clear parse error; valid FetchXML enables analysis.
- **US-PERF3.1.2** `[Planned]` **As** an ADM, **I want** to load FetchXML from a system view, **so that** I can analyze what users actually run.
  - **AC:** A view picker loads `savedquery`/`userquery` FetchXML via `Service.RetrieveAll` off the UI thread.

## FEAT-PERF3-2 — Query parsing & summary `[Planned]`
- **US-PERF3.2.1** `[Planned]` **As** a PERF engineer, **I want** the query broken into entities, attributes, filters, links, aliases, orders, and aggregations, **so that** I understand its shape.
  - **AC:** A summary panel lists each element with counts.
- **US-PERF3.2.2** `[Planned]` **As** a TOOLDEV, **I want** the parser/rule engine to be UI-free and shared, **so that** View and Dashboard analyzers reuse it.
...
## FEAT-PERF3-3 — Performance rule engine `[Planned]`
- **US-PERF3.3.1** `[Planned]` **As** a PERF engineer, **I want** all-attributes usage and excessive selected columns flagged, **so that** I can trim payload.
  - **AC:** `<all-attributes/>` → High; column count over threshold → Medium.
- **US-PERF3.3.2** `[Planned]` **As** a PERF engineer, **I want** missing filters and broad date ranges flagged, **so that** I catch unbounded scans.
  - **AC:** No filter on the root entity → High; date filter spanning beyond a configurable window → Medium.
- **US-PERF3.3.3** `[Planned]` **As** a PERF engineer, **I want** excessive/outer joins and risky sorts flagged, **so that** I can restructure the query.
...
- **US-PERF3.3.4** `[Planned]` **As** a PERF engineer, **I want** aggregation and paging risks flagged, **so that** large result issues surface.
  - **AC:** Aggregations without adequate filtering and missing/incorrect paging → Medium/High findings.

## FEAT-PERF3-4 — Cost estimate & optional timing `[Planned]`
- **US-PERF3.4.1** `[Planned]` **As** a PERF engineer, **I want** an estimated query cost score, **so that** I can compare queries without running them.
  - **AC:** A composite cost score derives from the rule findings and is labeled an estimate.
- **US-PERF3.4.2** `[Planned]` **As** a PERF engineer, **I want** to optionally execute the query with timing, **so that** I can ground the estimate.
  - **AC:** Execution is opt-in, runs off the UI thread with cancellation, and reports elapsed time and row count.

## FEAT-PERF3-5 — Recommendations & export `[Planned]`
- **US-PERF3.5.1** `[Planned]` **As** a PERF engineer, **I want** a suggested optimized FetchXML, **so that** I have a concrete fix to apply.
  - **AC:** A suggested-FetchXML panel proposes safe improvements (explicit columns, added filters) with an explanation.
- **US-PERF3.5.2** `[Planned]` **As** a MGR, **I want** Excel/PDF/JSON/HTML/CSV exports of the analysis, **so that** I can share it.

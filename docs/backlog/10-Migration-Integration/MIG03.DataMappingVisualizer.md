...
> **Suggested tag:** `MIG03` · **Suggested project:** `XrmToolSuite.DataMappingVisualizer`
...
## EPIC-MIG03 — Validate and visualize source-to-target data mappings against live metadata
...
## FEAT-MIG03-1 — Import and parse mapping files `[Planned]`
- **US-MIG03.1.1** `[Planned]` **As** a migration CUST, **I want** to import a mapping file from Excel, CSV, or JSON, **so that** I can reuse the mapping my project already maintains.
...
- **US-MIG03.1.2** `[Planned]` **As** a migration CUST, **I want** to select which target Dataverse tables the mapping covers, **so that** only relevant metadata is retrieved.
...
## FEAT-MIG03-2 — Validate table and column mappings `[Planned]`
- **US-MIG03.2.1** `[Planned]` **As** a migration CUST, **I want** source→target table and column mappings validated against metadata, **so that** I know which mappings resolve to real schema.
...
- **US-MIG03.2.2** `[Planned]` **As** a migration CUST, **I want** datatype mismatches detected, **so that** I avoid load failures from incompatible types.
...
## FEAT-MIG03-3 — Detect completeness and special-field gaps `[Planned]`
- **US-MIG03.3.1** `[Planned]` **As** a migration CUST, **I want** required Dataverse fields that are not mapped detected, **so that** I don't miss mandatory columns.
  - **AC:** Target attributes with required level = ApplicationRequired that have no incoming mapping are findings.
- **US-MIG03.3.2** `[Planned]` **As** a migration CUST, **I want** choice/option-set, lookup, and alternate-key mapping gaps detected, **so that** reference and key mappings are complete.
...
- **US-MIG03.3.3** `[Planned]` **As** a migration CUST, **I want** duplicate mappings and unmapped source fields detected, **so that** the mapping is unambiguous and complete.
  - **AC:** Two source fields mapped to the same target (and vice versa) and source fields with no target are findings.

## FEAT-MIG03-4 — Visualize, score, and export `[Planned]`
- **US-MIG03.4.1** `[Planned]` **As** a migration CUST, **I want** the mapping drawn as a table/field diagram, **so that** I can see coverage and gaps at a glance.
...
- **US-MIG03.4.2** `[Planned]` **As** an **MGR**, **I want** a mapping-completeness score, **so that** I get a readiness signal for the load.
  - **AC:** Score is a UI-free roll-up of mapped/required/valid ratios with severities Critical/High/Medium/Low/Info.
- **US-MIG03.4.3** `[Planned]` **As** a migration CUST, **I want** an exported validated-mapping report, **so that** I can hand a corrected mapping back to the project.

...
## EPIC-MIG7 — Visualize integration dependencies as a component-to-endpoint graph
...
**Outcome:** an interactive dependency graph (typed nodes/edges, inbound/outbound), a dependency detail panel, a risk-findings panel (single points of failure, non-prod endpoints in prod, stale/undocumented integrations), and exportable graph + integration report — built on the discovery engine shared with MIG4.

---

## FEAT-MIG7-1 — Discover and build the graph model `[Planned]`
- **US-MIG7.1.1** `[Planned]` **As** an ARCH, **I want** integration touchpoints discovered and turned into typed graph nodes, **so that** every integration component is represented.
  - **AC:** Nodes cover table, plugin, custom API, flow, connector, connection reference, environment variable, web resource, Power Pages component, external endpoint, and external system; discovery reuses the MIG4 shared engine and runs off the UI thread.
- **US-MIG7.1.2** `[Planned]` **As** an ARCH, **I want** component-to-endpoint relationships and inbound vs outbound direction mapped as edges, **so that** dependency direction is explicit.
...
## FEAT-MIG7-2 — Explore the graph interactively `[Planned]`
- **US-MIG7.2.1** `[Planned]` **As** an ARCH, **I want** a graph canvas with endpoint, component-type, and environment filters, **so that** I can focus on a slice of the map.
...
- **US-MIG7.2.2** `[Planned]` **As** an ARCH, **I want** a dependency detail panel for a selected node, **so that** I can see everything connected to it.
  - **AC:** Selecting a node lists its upstream/downstream neighbors and endpoint details; secrets are masked.

## FEAT-MIG7-3 — Detect integration risks in the graph `[Planned]`
- **US-MIG7.3.1** `[Planned]` **As** an ARCH, **I want** single points of failure and duplicate integrations detected, **so that** I know where blast radius and redundancy concentrate.
...
- **US-MIG7.3.2** `[Planned]` **As** a **SEC**, **I want** non-production endpoints in production and stale/undocumented integrations detected, **so that** I can remediate risky or unknown links.
...
## FEAT-MIG7-4 — Score and export `[Planned]`
- **US-MIG7.4.1** `[Planned]` **As** an **MGR**, **I want** an integration risk score, **so that** I get a signal on integration fragility.
...
- **US-MIG7.4.2** `[Planned]` **As** an ARCH, **I want** to export the graph and an integration report, **so that** I can share the dependency map.
...
- Integration-discovery engine is UI-free and **shared with MIG4 (API Integration Explorer)**; graph rendering reuses Solution Knowledge Graph patterns rather than duplicated.

...
> **Suggested tag:** `MIG06` · **Suggested project:** `XrmToolSuite.DuplicateRecordAnalyzer`
> **Overlaps:** Touches MIG05 'Data Quality Scanner' (duplicate candidate values) — this tool is the dedicated duplicate-grouping engine (fuzzy/normalized matching, groups, confidence, side-by-side). Share MIG05's safe row-sampling + masking helpers. Complements Dataverse's built-in duplicate detection by handling enterprise matching scenarios it doesn't.
...
## EPIC-MIG06 — Identify potential duplicate records with configurable matching (recommendations only)
...
## FEAT-MIG06-1 — Select table and build matching rules `[Planned]`
- **US-MIG06.1.1** `[Planned]` **As** an ADM, **I want** to select a table and matching columns, **so that** I control what defines a duplicate.
  - **AC:** Selection drives targeted `ColumnSet`s; a configurable sample cap applies; configuration round-trips via settings.
- **US-MIG06.1.2** `[Planned]` **As** an ADM, **I want** to configure exact, fuzzy, and normalized (phone/email/name/address) match rules with thresholds, **so that** I can tune matching to my data.
  - **AC:** Rule builder composes per-column matchers with weights/thresholds; the matching engine is UI-free and unit-testable.

## FEAT-MIG06-2 — Detect duplicate groups `[Planned]`
- **US-MIG06.2.1** `[Planned]` **As** an ADM, **I want** duplicate accounts, contacts, leads, or custom records detected, **so that** I find dupes in any table.
...
- **US-MIG06.2.2** `[Planned]` **As** an ADM, **I want** duplicates detected across active/inactive status and by alternate-key candidates, **so that** I catch dupes native detection misses.
...
## FEAT-MIG06-3 — Score confidence and compare `[Planned]`
- **US-MIG06.3.1** `[Planned]` **As** an ADM, **I want** a confidence score per duplicate group, **so that** I can triage high-confidence dupes first.
...
- **US-MIG06.3.2** `[Planned]` **As** an ADM, **I want** a side-by-side record comparison, **so that** I can verify a group is truly duplicate.
  - **AC:** Comparison panel shows matched records field-by-field; sensitive values are masked by default.

## FEAT-MIG06-4 — Recommend, safely act (opt-in), and export `[Planned]`
- **US-MIG06.4.1** `[Planned]` **As** an ADM, **I want** merge *recommendations* without any merge performed, **so that** I stay in control.
...
- **US-MIG06.4.2** `[Planned]` **As** an ADM, **I want** any optional safe-action mode to be explicitly opt-in, confirmed, and logged, **so that** destructive actions never happen silently.
...
- **US-MIG06.4.3** `[Planned]` **As** an **MGR**, **I want** a masked exported duplicate report, **so that** I can share findings safely.

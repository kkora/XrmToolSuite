...
> **Suggested tag:** `MIG5` · **Suggested project:** `XrmToolSuite.DataQualityScanner`
> **Overlaps:** Touches SEC7 'Sensitive Data Scanner' (both scan record data with masking) and MIG6 'Duplicate Record Analyzer' (duplicate values) — different focus: this is broad column-level quality (formats, blanks, orphans, stale, placeholders), MIG6 is dedicated duplicate grouping. Share the safe row-sampling + masking helpers.
...
## EPIC-MIG5 — Safely scan Dataverse data quality across selected tables and columns
...
## FEAT-MIG5-1 — Scope and configure a safe scan `[Planned]`
- **US-MIG5.1.1** `[Planned]` **As** an ADM, **I want** to select tables, columns, and which rules to run, **so that** I scan only what matters.
  - **AC:** Selection drives targeted `ColumnSet`s; disabled rules are not evaluated; configuration round-trips via settings.
- **US-MIG5.1.2** `[Planned]` **As** an ADM, **I want** a configurable row-sampling cap and a confirmation before large scans, **so that** I never accidentally run a long/heavy scan.
...
## FEAT-MIG5-2 — Detect value-level quality issues `[Planned]`
- **US-MIG5.2.1** `[Planned]` **As** an ADM, **I want** missing required values, invalid email/phone/URL formats, and invalid date ranges detected, **so that** malformed data is surfaced.
...
- **US-MIG5.2.2** `[Planned]` **As** an ADM, **I want** inconsistent casing, leading/trailing spaces, and placeholder values (test, n/a, unknown, dummy) detected, **so that** dirty text data is surfaced.
  - **AC:** Placeholder list is configurable; casing/whitespace/placeholder violations are separate finding types.
- **US-MIG5.2.3** `[Planned]` **As** an ADM, **I want** invalid choice values and duplicate candidate values detected where possible, **so that** reference-data problems are surfaced.
  - **AC:** Choice values not in the current option set and repeated candidate values are findings; masked in output.

## FEAT-MIG5-3 — Detect record-level quality issues `[Planned]`
- **US-MIG5.3.1** `[Planned]` **As** an ADM, **I want** orphan lookup references and records missing owner/business unit detected, **so that** relational and ownership gaps are surfaced.
...
- **US-MIG5.3.2** `[Planned]` **As** an ADM, **I want** inactive/stale records detected, **so that** obsolete data is flagged before migration.
  - **AC:** Stale threshold (by `modifiedon`/`statecode`) is configurable; stale records are counted per table.

## FEAT-MIG5-4 — Review masked samples, score, and export `[Planned]`
- **US-MIG5.4.1** `[Planned]` **As** an ADM, **I want** a findings grid and a masked sample-records panel, **so that** I can review issues without exposing sensitive data.
...
- **US-MIG5.4.2** `[Planned]` **As** an **MGR**, **I want** a data-quality score and cleanup recommendations, **so that** I can prioritize remediation.
...
- **US-MIG5.4.3** `[Planned]` **As** an ADM, **I want** a masked exported data-quality report, **so that** I can share findings safely.

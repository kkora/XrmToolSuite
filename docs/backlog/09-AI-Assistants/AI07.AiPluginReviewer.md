...
> **Suggested tag:** `AI07` · **Suggested project:** `XrmToolSuite.AiPluginReviewer`
> **Overlaps:** **AI Solution Reviewer (shipped)** — reuse AI client, session-only key, consent-preview, redaction, offline-fallback. Consumes the **Plugins-track** deterministic analyzers (e.g. Plugin Performance Profiler / PERF01 for trace-log and step analysis); overlaps AI Performance Advisor (AI04) on synchronous/recursion signals — share the plugin-metadata retrieval and trace-log parsing layer.
...
## EPIC-AI07 — Review plugin registration quality and produce refactoring guidance
...
## FEAT-AI07-1 — Deterministic rules-first findings `[Planned]`
- **US-AI07.1.1** `[Planned]` **As** a TOOLDEV, **I want** the full plugin analysis and score without any AI, **so that** the tool is useful and safe with AI disabled.
  - **AC:** Complete deterministic findings and score available offline; AI toggled off by default.
- **US-AI07.1.2** `[Planned]` **As** a TOOLDEV, **I want** detection of duplicate steps, update steps without filtering attributes, recursion risks, high-depth trace logs, synchronous performance risks, excessive images and outdated assemblies, **so that** the real issues surface deterministically.
  - **AC:** Reads via `RetrieveAll` off the UI thread with progress and cancellation; each finding carries severity and evidence.

## FEAT-AI07-2 — Plugin dashboard, filters & trace summary `[Planned]`
- **US-AI07.2.1** `[Planned]` **As** a TOOLDEV, **I want** a plugin review dashboard with assembly/type/step filters and a findings grid, **so that** I can focus on a specific assembly.
  - **AC:** Score deterministic and reproducible with AI off.
- **US-AI07.2.2** `[Planned]` **As** a DEVOPS, **I want** a trace-log summary panel, **so that** I can spot failing or high-depth executions.
  - **AC:** Trace messages shown with sensitive values masked; nothing leaves the tool unredacted.

## FEAT-AI07-3 — AI plugin health & refactoring advice (opt-in) `[Planned]`
- **US-AI07.3.1** `[Planned]` **As** a TOOLDEV, **I want** an AI plugin-health summary with recursion/performance explanation, **so that** I understand systemic risks.
...
- **US-AI07.3.2** `[Planned]` **As** a TOOLDEV, **I want** AI refactoring recommendations and Custom API conversion suggestions, **so that** I can modernise plugin logic.
  - **AC:** Advisory only; the tool never edits registrations.

## FEAT-AI07-4 — AI developer backlog (opt-in) `[Planned]`
- **US-AI07.4.1** `[Planned]` **As** a DEVOPS, **I want** AI to generate a developer refactoring backlog with best-practice guidance, **so that** I can schedule remediation.
  - **AC:** Backlog items trace to specific deterministic findings; offline fallback yields the deterministic list.

## FEAT-AI07-5 — Configurable prompt templates `[Planned]`
- **US-AI07.5.1** `[Planned]` **As** a TOOLDEV, **I want** editable AI prompt/system-instruction templates, **so that** advice matches our plugin standards.
  - **AC:** Templates round-trip via settings as POCOs; no key/connection details persisted.

## FEAT-AI07-6 — Export `[Planned]`
- **US-AI07.6.1** `[Planned]` **As** a TOOLDEV, **I want** to export the plugin review and backlog to Excel/PDF/Word/CSV/Markdown/HTML/JSON, **so that** I can share findings.

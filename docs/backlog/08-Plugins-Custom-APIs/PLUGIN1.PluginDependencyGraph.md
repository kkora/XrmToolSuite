...
> **Suggested tag:** `PLUGIN1` · **Suggested project:** `XrmToolSuite.PluginDependencyGraph`
...
## EPIC-PLUGIN1 — Visualize the full dependency footprint of Dataverse plugin registrations
...
## FEAT-PLUGIN1-1 — Plugin metadata retrieval `[Planned]`
- **US-PLUGIN1.1.1** `[Planned]` **As** a TOOLDEV, **I want** to load all plugin assemblies, types, and SDK steps for the connected org, **so that** I have a complete pipeline inventory.
  - **AC:** Assemblies, types, and steps load via `Service.RetrieveAll` off the UI thread with progress and cancellation.
- **US-PLUGIN1.1.2** `[Planned]` **As** an ARCH, **I want** each step's table, message, stage, mode, rank, filtering attributes, impersonating user, deployment type, and status resolved, **so that** nodes carry accurate context.
  - **AC:** Step nodes expose stage/mode/rank/message/entity/filteringattributes/impersonation/supporteddeployment/statecode.
- **US-PLUGIN1.1.3** `[Planned]` **As** a TOOLDEV, **I want** pre/post images and their attributes attached to their steps, **so that** image dependencies appear in the graph.
  - **AC:** Each `sdkmessageprocessingstepimage` is linked to its step with imagetype and attribute list.

## FEAT-PLUGIN1-2 — Graph model & rendering `[Planned]`
- **US-PLUGIN1.2.1** `[Planned]` **As** an ARCH, **I want** an interactive dependency graph canvas with node types for assembly, type, step, table, message, stage, image, custom API, solution, and configuration, **so that** I can explore relationships visually.
...
- **US-PLUGIN1.2.2** `[Planned]` **As** a TOOLDEV, **I want** to select an assembly or plugin type and see its subgraph, **so that** I can focus on one component's footprint.
  - **AC:** Selecting a node highlights its connected nodes/edges and can isolate the subgraph; selection persists via settings.
- **US-PLUGIN1.2.3** `[Planned]` **As** an ARCH, **I want** to filter the graph by table, message, stage, mode, or solution, **so that** I can narrow a large pipeline.
  - **AC:** Each filter live-updates the visible nodes/edges without re-querying Dataverse.

## FEAT-PLUGIN1-3 — Solution & custom API relationships `[Planned]`
- **US-PLUGIN1.3.1** `[Planned]` **As** an ALM lead, **I want** solution membership shown for each assembly/step, **so that** I can see which solution owns a registration.
  - **AC:** Solution nodes link to their member components via `solutioncomponent`; unmanaged registrations are visually distinct.
- **US-PLUGIN1.3.2** `[Planned]` **As** a TOOLDEV, **I want** custom API relationships drawn where detectable, **so that** I can see which plugin type backs which custom API.
  - **AC:** `customapi.plugintypeid` produces an edge from the custom API node to the plugin type; absent links degrade silently.
- **US-PLUGIN1.3.3** `[Planned]` **As** a SEC reviewer, **I want** secure/unsecure configuration usage flagged on steps without exposing secrets, **so that** I can audit config dependency safely.
  - **AC:** A "uses config" flag/edge appears; secure-config values are never displayed and unsecure values are secret-redacted.

## FEAT-PLUGIN1-4 — High-impact & risk detection `[Planned]`
- **US-PLUGIN1.4.1** `[Planned]` **As** an ARCH, **I want** high-impact assemblies (registered on many tables/messages) detected, **so that** I know what carries the most blast radius.
  - **AC:** Assemblies whose table/message fan-out exceeds a configurable threshold are flagged and ranked.
- **US-PLUGIN1.4.2** `[Planned]` **As** a DEVOPS, **I want** duplicate or overlapping steps detected, **so that** I can remove redundant registrations.
  - **AC:** Steps matching on message+entity+stage+mode are grouped and flagged Medium/High.
- **US-PLUGIN1.4.3** `[Planned]` **As** an ALM lead, **I want** unmanaged plugin registrations in production detected, **so that** I can catch out-of-process changes.
  - **AC:** Unmanaged steps/assemblies are flagged High; finding names the component and (if known) owning solution.

## FEAT-PLUGIN1-5 — Details, findings & export `[Planned]`
- **US-PLUGIN1.5.1** `[Planned]` **As** a TOOLDEV, **I want** a step details panel and a dependency grid beside the canvas, **so that** I can read exact registration values.
  - **AC:** Selecting a node populates a details panel and a dependency grid with its connected components.
- **US-PLUGIN1.5.2** `[Planned]` **As** an ARCH, **I want** a risk findings panel with severities, **so that** high-impact and duplication risks are actionable.
  - **AC:** Findings list carries Critical/High/Medium/Low/Info with a plain-language description per item.
- **US-PLUGIN1.5.3** `[Planned]` **As** a MGR, **I want** to export the graph to PNG, SVG, PDF, Excel, JSON, GraphML, and HTML, **so that** I can share or archive it.

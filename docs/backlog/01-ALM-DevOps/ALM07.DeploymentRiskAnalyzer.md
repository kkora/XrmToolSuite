...
> **Status:** DONE (shipped tool) — filed under ALM & DevOps. Kept its own area tag and story IDs.
...
## EPIC-ALM07 - Pre-deployment risk analysis for Dataverse solutions
...
## FEAT-ALM07-1 - Connect to source and target environments `[Done]`

- **US-ALM07-1.1** `[Done]` **As** an ALM engineer, **I want** to load the solutions in my source (dev)
...
- **US-ALM07-1.2** `[Done]` **As** an ALM engineer, **I want** to *optionally* connect a target
...
## FEAT-ALM07-2 - Solution dependency analysis `[Done]`

- **US-ALM07-2.1** `[Done]` **As** an ALM engineer, **I want** missing required components detected,
...
- **US-ALM07-2.2** `[Done]` **As** an ALM engineer, **I want** publisher prefix / option-value-prefix
...
- **US-ALM07-2.3** `[Done]` **As** a release manager, **I want** components in the target's installed
...
## FEAT-ALM07-3 - Environment variables & connection references `[Done]`

- **US-ALM07-3.1** `[Done]` **As** an ALM engineer, **I want** environment variables with no
...
- **US-ALM07-3.2** `[Done]` **As** an ALM engineer, **I want** values accidentally packaged into the
...
## FEAT-ALM07-4 - Flow & plugin readiness `[Done]`

- **US-ALM07-4.1** `[Done]` **As** an ALM engineer, **I want** draft (OFF) flows/processes and flows
...
- **US-ALM07-4.2** `[Done]` **As** an ALM engineer, **I want** plugin steps with missing types/assemblies,
...
- **US-ALM07-4.3** `[Done]` **As** an ALM engineer, **I want** duplicate SDK step registrations and
...
## FEAT-ALM07-5 - Security impact `[Done]`

- **US-ALM07-5.1** `[Done]` **As** an ADM, **I want** new custom tables with no role coverage and
...
- **US-ALM07-5.2** `[Done]` **As** an ADM, **I want** roles assigned to no user/team in the target
  flagged, **so that** I notice ineffective security before relying on it.

## FEAT-ALM07-6 - Data model / schema conflict analysis (target required) `[Done]`

- **US-ALM07-6.1** `[Done]` **As** an ALM engineer, **I want** import-breaking schema conflicts
...
- **US-ALM07-6.2** `[Done]` **As** an ALM engineer, **I want** solution-version-not-incremented and
  managed/unmanaged mismatch detected, **so that** I avoid downgrade and layering surprises.

## FEAT-ALM07-7 - Power Pages readiness `[Done]`

- **US-ALM07-7.1** `[Done]` **As** an ADM, **I want** Power Pages security/readiness gaps flagged
...
## FEAT-ALM07-8 - Risk scoring `[Done]`

- **US-ALM07-8.1** `[Done]` **As** an ALM engineer, **I want** findings rolled into a weighted
...
- **US-ALM07-8.2** `[Done]` **As** a release manager, **I want** an executive deployment summary with a
...
## FEAT-ALM07-9 - Report export `[Done]`

- **US-ALM07-9.1** `[Done]` **As** an ADM, **I want** to export a styled HTML **dashboard** report, **so that** I can
...
- **US-ALM07-9.2** `[Done]` **As** an ADM, **I want** an Excel workbook (Summary / Findings / Fix
  Checklist), **so that** I can track remediation.
- **US-ALM07-9.3** `[Done]` **As** a DEVOPS engineer, **I want** a JSON export with `ci.pass` and
...
- **US-ALM07-9.4** `[Done]` **As** an ADM, **I want** a Markdown fix checklist that always ends with
  rollback guidance, **so that** I have a recovery plan if an import goes wrong.
- **US-ALM07-9.5** `[Done]` **As** an ADM, **I want** a native executive PDF (rendered directly, not via
...
## FEAT-ALM07-10 - Resilience & extensibility `[Done]`

- **US-ALM07-10.1** `[Done]` **As** an ALM engineer, **I want** an analyzer whose query fails to degrade
  to an informational finding, **so that** one permission gap never aborts the whole run.
- **US-ALM07-10.2** `[Done]` **As** a TOOLDEV, **I want** to add an analyzer by implementing `IAnalyzer`
...
## FEAT-ALM07-11 - Form changes `[Done]`

- **US-ALM07-11** `[Done]` **As** an ALM engineer, **I want** forms flagged when a script/control
...
## FEAT-ALM07-12 - Ribbon changes `[Done]`

- **US-ALM07-12** `[Done]` **As** an ALM engineer, **I want** ribbon (command bar) commands flagged when
...
## FEAT-ALM07-14 - Help & Support `[Done]`

- **US-ALM07-14** `[Done]` **As** a user, **I want** an in-tool Help & Support page, **so that** I can find
...
- **US-ALM07-13.1** `[Planned]` **As** a DEVOPS engineer, **I want** a headless CLI runner for the
  analyzers (no XrmToolBox), **so that** I can run Deployment Risk Analyzer directly in CI.
- **US-ALM07-13.2** `[Planned]` **As** an ALM engineer, **I want** to save/load selected-analyzer and
  target-connection profiles, **so that** repeat analyses are one click.
- **US-ALM07-13.3** `[Done]` Native PDF export — delivered as US-ALM07-9.5.
- **US-ALM07-13.4** `[Planned]` **As** an ALM engineer, **I want** a baseline/diff mode comparing this
  run to a previous report, **so that** I see whether risk improved or regressed.
- **US-ALM07-13.5** `[Done]` AI/offline deployment summary — delivered as US-ALM07-8.2.

...
> **Suggested tag:** `ALM05` · **Suggested project:** `XrmToolSuite.EnvironmentVariableValidator`
...
## EPIC-ALM05 — Validate environment variables across solutions and environments
...
## FEAT-ALM05-1 — Inventory variables and values `[Planned]`
- **US-ALM05.1.1** `[Planned]` **As** an ALM engineer, **I want** all environment-variable definitions and their current values listed, **so that** I have the full inventory to validate.
...
- **US-ALM05.1.2** `[Planned]` **As** an ALM engineer, **I want** variables grouped/filterable by solution and type, **so that** I can focus on the ones I am shipping.
  - **AC:** Solution filter and type filter drive the grid; grouping persists via Load/SaveSettings.

## FEAT-ALM05-2 — Presence and duplication validation `[Planned]`
- **US-ALM05.2.1** `[Planned]` **As** an ALM engineer, **I want** missing current values and default-used-in-production cases detected, **so that** the target is properly configured before go-live.
...
- **US-ALM05.2.2** `[Planned]` **As** an ALM engineer, **I want** duplicate schema names and secret-not-configured cases flagged, **so that** I avoid ambiguous references and broken Key Vault bindings.
  - **AC:** Duplicate schema names and secret variables lacking a configured secret store are findings.

## FEAT-ALM05-3 — Format and type validation `[Planned]`
- **US-ALM05.3.1** `[Planned]` **As** a System Customizer, **I want** URL, JSON, boolean, and number values validated against their declared type, **so that** malformed values do not fail at runtime.
  - **AC:** Invalid URL/JSON format and boolean/number parse failures are findings tied to the variable and type.
- **US-ALM05.3.2** `[Planned]` **As** an ALM engineer, **I want** invalid data types and environment-specific values accidentally moved detected, **so that** DEV values do not leak into PROD.
  - **AC:** Type mismatches and values matching a known non-target pattern (reusing DRA's packaged-value check) are flagged.

## FEAT-ALM05-4 — Usage tracing and unused detection `[Planned]`
- **US-ALM05.4.1** `[Planned]` **As** an ALM engineer, **I want** variables referenced by flows/plugins/web resources/custom APIs traced, **so that** I know which are actually in use.
...
- **US-ALM05.4.2** `[Planned]` **As** an ALM engineer, **I want** unused variables and referenced-but-not-in-solution cases flagged, **so that** I can clean up and avoid broken references.
  - **AC:** Unused variables → Low/Info; referenced-but-not-included → higher severity per the source rules.

## FEAT-ALM05-5 — Cross-environment compare and export `[Planned]`
- **US-ALM05.5.1** `[Planned]` **As** an Enterprise Architect, **I want** variable values compared across DEV/TEST/UAT/PROD, **so that** I can spot values that differ or are missing in the target.
...
- **US-ALM05.5.2** `[Planned]` **As** a DEVOPS engineer, **I want** a deployment checklist and Excel/PDF/JSON/HTML export with a pass flag, **so that** I can gate a pipeline on unconfigured variables.

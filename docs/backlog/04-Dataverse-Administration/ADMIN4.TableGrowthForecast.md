...
> **Suggested tag:** `ADMIN4` · **Suggested project:** `XrmToolSuite.TableGrowthForecast`
> **Overlaps:** Shares data (row counts, largest tables, archive candidates) with candidate **ADMIN5 (Storage Cost Estimator)** and pack idea #19 'Dataverse Capacity Analyzer'; ADMIN5 turns rows into cost, ADMIN4 turns rows into a time-series forecast. Consider a shared snapshot store.
...
- Row counts, not bytes — this tool forecasts *rows*; storage-byte estimation is ADMIN5's job (may share the estimation helper).
...
## EPIC-ADMIN4 — Forecast Dataverse table growth from snapshot history so teams can plan archiving
...
## FEAT-ADMIN4-1 — Capture and store snapshots `[Planned]`
- **US-ADMIN4.1.1** `[Planned]` **As** an ADM, **I want** current row counts captured per table on demand, **so that** I have a data point to forecast from.
  - **AC:** Counts use `RetrieveTotalRecordCount` off the UI thread with progress/cancellation; an approximation disclaimer is shown.
- **US-ADMIN4.1.2** `[Planned]` **As** an ADM, **I want** each scan saved as a timestamped local snapshot, **so that** history accrues for trend analysis.
  - **AC:** Snapshots persist to local storage append-only (no deletes); a snapshot-history panel lists prior scans with dates.

## FEAT-ADMIN4-2 — Compute growth and forecast `[Planned]`
- **US-ADMIN4.2.1** `[Planned]` **As** an ADM, **I want** daily/weekly/monthly/yearly growth estimated from snapshot deltas, **so that** I understand each table's velocity.
...
- **US-ADMIN4.2.2** `[Planned]` **As** an MGR, **I want** 3/6/12/24-month row-count forecasts per table, **so that** I can plan capacity ahead.
...
## FEAT-ADMIN4-3 — Identify hotspots and archive candidates `[Planned]`
- **US-ADMIN4.3.1** `[Planned]` **As** an ADM, **I want** fastest-growing tables and high-create-volume tables identified, **so that** I focus on the real drivers.
  - **AC:** A fastest-growing grid ranks by growth rate; high-create-volume uses `createdon`-trend sampling.
- **US-ADMIN4.3.2** `[Planned]` **As** an ADM, **I want** archive candidates and tables with large notes/files/audit activity flagged, **so that** I know where retention will help.
  - **AC:** Archive-candidate grid combines size, growth, and age signals; annotation/file/audit-heavy tables are noted.

## FEAT-ADMIN4-4 — Scoring, assumptions, and export `[Planned]`
- **US-ADMIN4.4.1** `[Planned]` **As** an ADM, **I want** a table growth score and configurable growth assumptions, **so that** I can tune forecasts to my org's reality.
  - **AC:** Growth score is a UI-free roll-up; assumptions (growth rate overrides, horizon) round-trip via Load/SaveSettings.
- **US-ADMIN4.4.2** `[Planned]` **As** an MGR, **I want** archive/retention recommendations and a forecast report exported, **so that** I can drive a data-management decision.

...
> **Status:** DONE (shipped tool) — filed under Dataverse Administration. Kept its own area tag and story IDs.
...
> check, table multi-select / solution scoping, JSON/Excel export, and the guarded cleanup (FEAT-ADMIN10-5).
...
## EPIC-ADMIN10 - Find and safely retire unused columns across a Dataverse environment
...
## FEAT-ADMIN10-0 - Scaffold & shared wiring `[Done]`

- **US-ADMIN10-0.1** `[Done]` **As** a TOOLDEV, **I want** the tool to load in XrmToolBox with connection,
...
- **US-ADMIN10-0.2** `[Done]` **As** a TOOLDEV, **I want** the placeholder `UserName`/`HelpUrl` and
...
## FEAT-ADMIN10-1 - Select audit scope `[Partial]`

- **US-ADMIN10-1.1** `[Partial]` **As** a CUST, **I want** to pick one, several, or all tables (with a
...
- **US-ADMIN10-1.2** `[Planned]` **As** a CUST, **I want** to choose which usage signals count, **so that**
  I can tune strictness (e.g. treat "on a form" as used, or require actual data population).

## FEAT-ADMIN10-2 - Detect column usage signals `[Partial]`

Each signal marks a column as "used" with evidence. A column with **no** signals is a retirement candidate.

- **US-ADMIN10-2.1** `[Done]` **As** a CUST, **I want** columns present on any **form** detected,
  **so that** UI-referenced columns are not falsely flagged.
- **US-ADMIN10-2.2** `[Done]` **As** a CUST, **I want** columns used in any **view / saved query**
  (columns or filter) detected.
- **US-ADMIN10-2.3** `[Partial]` **As** a CUST, **I want** columns referenced by **business rules,
  workflows, flows, calculated/rollup fields, and plugin steps** detected.
- **US-ADMIN10-2.4** `[Partial]` **As** a CUST, **I want** columns referenced by **charts, dashboards,
  and field security profiles** detected.
- **US-ADMIN10-2.5** `[Planned]` **As** an ADM, **I want** an optional **data population** check
...
## FEAT-ADMIN10-3 - Results & classification `[Partial]`

- **US-ADMIN10-3.1** `[Done]` **As** a CUST, **I want** a grid of columns with table, display/logical
...
- **US-ADMIN10-3.2** `[Partial]` **As** a CUST, **I want** to sort/filter by "no usage", type, or table,
  **so that** I can focus on the safest wins first.
- **US-ADMIN10-3.3** `[Done]` **As** an ADM, **I want** managed columns clearly marked non-deletable,
  **so that** I do not target components I cannot change.

## FEAT-ADMIN10-4 - Export `[Partial]`

- **US-ADMIN10-4.1** `[Partial]` **As** an ADM, **I want** to export the audit to CSV/Excel, **so that**
  I can review with stakeholders and track decisions.
- **US-ADMIN10-4.2** `[Planned]` **As** a DEVOPS engineer, **I want** a JSON export, **so that** the audit
  can feed governance dashboards.

## FEAT-ADMIN10-5 - Guarded cleanup `[Planned]`

- **US-ADMIN10-5.1** `[Planned]` **As** an ADM, **I want** to select confirmed-unused columns and generate
...
- **US-ADMIN10-5.2** `[Planned]` **As** an ADM, **I want** an audit log of what was proposed/removed and

...
> **Suggested tag:** `ADMIN03` · **Suggested project:** `XrmToolSuite.DuplicateMetadataFinder`
> **Overlaps:** Feeds candidate **ADMIN02 (Metadata Cleanup Advisor)** — that tool should consume this tool's duplicate-group output rather than reimplement similarity matching. Partial overlap with the shipped **Attribute Auditor** (column inventory/usage) — reuse its metadata retrieval and usage evidence.
...
## EPIC-ADMIN03 — Surface duplicate and near-duplicate metadata so teams can consolidate
...
## FEAT-ADMIN03-1 — Scan configuration and metadata retrieval `[Planned]`
- **US-ADMIN03.1.1** `[Planned]` **As** an ARCH, **I want** to configure which component types and scope to scan, **so that** I control run cost and noise.
  - **AC:** Type/scope selection loads via `RetrieveAll`/`RetrieveMetadataChanges` off the UI thread with progress and cancellation.
- **US-ADMIN03.1.2** `[Planned]` **As** an ARCH, **I want** a configurable similarity threshold, **so that** I can tune false positives vs. recall.
  - **AC:** Threshold round-trips via Load/SaveSettings; only pairs at/above threshold are grouped.

## FEAT-ADMIN03-2 — Detect duplicate columns and option sets `[Planned]`
- **US-ADMIN03.2.1** `[Planned]` **As** a MAKER, **I want** duplicate column display names and similar schema names detected, **so that** I find fields created twice by different teams.
...
- **US-ADMIN03.2.2** `[Planned]` **As** a MAKER, **I want** fields with the same data type and similar purpose, and duplicate/overlapping option sets, detected, **so that** semantic duplicates surface too.
  - **AC:** Option-set duplicates compare option-value sets; overlap ratio contributes to the score.

## FEAT-ADMIN03-3 — Detect duplicate UI and logic components `[Planned]`
- **US-ADMIN03.3.1** `[Planned]` **As** an ARCH, **I want** duplicate tables, forms, views, charts, and dashboards detected, **so that** redundant UI assets are grouped.
  - **AC:** Each type is a distinct duplicate category with its own similarity basis (name + structure signals).
- **US-ADMIN03.3.2** `[Planned]` **As** a TOOLDEV, **I want** duplicate business rules, web resources, JavaScript functions (best-effort), plugin steps, and relationships detected, **so that** duplicated logic is found.
...
## FEAT-ADMIN03-4 — Group, compare, and recommend primary `[Planned]`
- **US-ADMIN03.4.1** `[Planned]` **As** an ARCH, **I want** duplicates grouped with a side-by-side comparison and usage/dependency impact, **so that** I can judge which to keep.
...
- **US-ADMIN03.4.2** `[Planned]` **As** an ARCH, **I want** a recommended primary component to keep per group, **so that** consolidation has a clear default.
...
## FEAT-ADMIN03-5 — Export `[Planned]`
- **US-ADMIN03.5.1** `[Planned]` **As** an MGR, **I want** the duplicate report exported, **so that** consolidation can be planned as a change.

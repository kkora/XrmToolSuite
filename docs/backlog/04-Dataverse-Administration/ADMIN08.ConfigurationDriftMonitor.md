...
> **Suggested tag:** `ADMIN08` · **Suggested project:** `XrmToolSuite.ConfigurationDriftMonitor`
> **Overlaps:** Overlaps a candidate Migration **'Environment Comparison Suite'** and pack idea #1 'Dataverse Environment Drift Detector' — same core (compare environments/baselines, score drift, recommend remediation); **NOTE** to consolidate rather than ship two comparison engines. Consumes candidate **ADMIN07 (Environment Inventory)** normalization as the comparable snapshot; complements ADMIN01 (health) baselines.
...
## EPIC-ADMIN08 — Detect configuration drift between environments or against a baseline
...
## FEAT-ADMIN08-1 — Source/target selection and baselines `[Planned]`
- **US-ADMIN08.1.1** `[Planned]` **As** an ALM engineer, **I want** to save the current environment as a named baseline, **so that** I can compare future state against a known-good point.
...
- **US-ADMIN08.1.2** `[Planned]` **As** an ALM engineer, **I want** to compare against a baseline or a second connected environment, **so that** I can validate DEV→PROD alignment.
...
## FEAT-ADMIN08-2 — Compare components `[Planned]`
- **US-ADMIN08.2.1** `[Planned]` **As** an ALM engineer, **I want** tables, columns, relationships, forms, views, charts, and dashboards compared, **so that** metadata drift surfaces.
  - **AC:** Each component is classified Missing/Extra/Changed; the diff engine is UI-free and unit-testable.
- **US-ADMIN08.2.2** `[Planned]` **As** a SEC, **I want** security roles, business rules, workflows, plugins, and web resources compared, **so that** logic and access drift surfaces.
  - **AC:** Per-class comparison lists changed properties; role diff is by privilege set.
- **US-ADMIN08.2.3** `[Planned]` **As** an ALM engineer, **I want** environment variables, connection references, and solution versions/publishers compared, **so that** binding and packaging drift surfaces.
  - **AC:** Env-var/connection-ref value and definition differences, and solution version/publisher mismatches, are findings.

## FEAT-ADMIN08-3 — Classify and score drift `[Planned]`
- **US-ADMIN08.3.1** `[Planned]` **As** an ALM engineer, **I want** unmanaged changes in the target, missing components, extra components, and changed properties detected, **so that** I know the nature of each drift.
  - **AC:** Unmanaged-change detection uses `ismanaged`/layering; each category has a count in the summary.
- **US-ADMIN08.3.2** `[Planned]` **As** an MGR, **I want** an overall drift score, **so that** I get an alignment signal without reading every row.
  - **AC:** Drift score is a UI-free weighted roll-up with severities Critical/High/Medium/Low/Info.

## FEAT-ADMIN08-4 — Review, remediate, and export `[Planned]`
- **US-ADMIN08.4.1** `[Planned]` **As** an ALM engineer, **I want** a component comparison grid and a difference viewer, **so that** I can inspect each drift in detail.
  - **AC:** Grid filters by category/classification/severity; the difference viewer shows changed-property before/after.
- **US-ADMIN08.4.2** `[Planned]` **As** an ALM engineer, **I want** a remediation checklist and an exported drift report, **so that** I can realign environments as a controlled change.

...
> **Suggested tag:** `RPT01` · **Suggested project:** `XrmToolSuite.ExecutiveDashboard`
> **Overlaps:** Top-level aggregator that rolls up scores from many suite tools — shipped Deployment Risk Analyzer, Technical Debt Analyzer, Solution Complexity Score, plus candidate Governance Scorecard (RPT03), ALM KPI Dashboard (RPT02), Usage Analytics (RPT06), Technical Debt Trends (RPT04). Also overlaps the candidate Admin Environment Health Dashboard — that is component/admin-facing; this is the executive/board-facing single-pane roll-up above it. NOTE the duplication before both are built.
...
## EPIC-RPT01 — Give leadership one executive pane for environment health, risk, and trend
...
## FEAT-RPT01-1 — Score aggregation & orchestration `[Planned]`
- **US-RPT01.1.1** `[Planned]` **As** an ARCH, **I want** the tool to run each contributing analyzer and collect its score and findings, **so that** the dashboard reflects real environment state.
...
- **US-RPT01.1.2** `[Planned]` **As** a TOOLDEV, **I want** domain scores computed by reusing existing analyzer engines, **so that** scoring rules live in one place and stay unit-testable.
  - **AC:** The aggregation/banding engine is UI-free with no Dataverse calls in its signatures.

## FEAT-RPT01-2 — Executive KPI dashboard & score cards `[Planned]`
- **US-RPT01.2.1** `[Planned]` **As** an MGR, **I want** an overall health score plus a card per domain (governance, ALM, security, performance, tech debt, storage, solution quality, deployment risk), **so that** I see strengths and weaknesses at a glance.
  - **AC:** Each card shows a 0-100 score and a Critical/High/Medium/Low/Info band, explainable from its contributing findings.
- **US-RPT01.2.2** `[Planned]` **As** an MGR, **I want** inventory KPI tiles (active users, teams, solutions, apps, plugins, flows, Power Pages sites, tables, custom components), **so that** I understand the environment's size.
...
## FEAT-RPT01-3 — Risk heatmap & top issues `[Planned]`
- **US-RPT01.3.1** `[Planned]` **As** an ARCH, **I want** a risk heatmap across domains and severities, **so that** I can spot concentrations of risk quickly.
  - **AC:** Heatmap renders as inline SVG/PNG and is embeddable in exports.
- **US-RPT01.3.2** `[Planned]` **As** an MGR, **I want** a top-critical-issues panel, **so that** I know the few things that most need attention.

## FEAT-RPT01-4 — Recommendation roadmap `[Planned]`
- **US-RPT01.4.1** `[Planned]` **As** an ARCH, **I want** a prioritized recommendation roadmap (highest-impact first), **so that** teams know what to tackle next.
  - **AC:** Each item links back to its finding and source analyzer; all read-only.

## FEAT-RPT01-5 — Trend snapshots `[Planned]`
- **US-RPT01.5.1** `[Planned]` **As** an MGR, **I want** prior scores saved locally and shown as trend charts, **so that** I can prove health is improving over releases.
...
## FEAT-RPT01-6 — Executive export `[Planned]`
- **US-RPT01.6.1** `[Planned]` **As** an MGR, **I want** to export an executive report (PDF / PowerPoint-style deck / Excel / HTML) with charts and KPI data, **so that** I can present to leadership.

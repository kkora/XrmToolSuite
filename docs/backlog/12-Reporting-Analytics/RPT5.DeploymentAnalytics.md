...
> **Suggested tag:** `RPT5` · **Suggested project:** `XrmToolSuite.DeploymentAnalytics`
> **Overlaps:** Overlaps the candidate ALM Deployment Timeline Visualizer (ALM track) — both build a deployment timeline from solution/component history. NOTE clearly: keep this the analytics/KPI + risk-pattern layer; if the ALM visualizer ships, reuse its timeline model rather than rebuild it. Also overlaps ALM KPI Dashboard (RPT2, readiness KPIs) and the shipped Deployment Risk Analyzer (risk rules) — reuse the risk engine; complements it with historical/frequency analysis.
...
## EPIC-RPT5 — Give release managers insight into what deployed, when, and how risky the cadence is
...
## FEAT-RPT5-1 — Deployment history collection `[Planned]`
- **US-RPT5.1.1** `[Planned]` **As** an ALM lead, **I want** solution versions, publishers, and created/modified dates collected, **so that** I have the deployment history.
  - **AC:** Retrieval uses Service.RetrieveAll off the UI thread with progress and cancellation; targeted ColumnSets only.
- **US-RPT5.1.2** `[Planned]` **As** an ARCH, **I want** import-history / failed-deployment indicators used where available and inferred otherwise, **so that** the analysis works across Dataverse versions.
  - **AC:** Unavailable native sources degrade to Info; inferred data is labeled "inferred", not presented as authoritative.

## FEAT-RPT5-2 — Deployment timeline `[Planned]`
- **US-RPT5.2.1** `[Planned]` **As** an MGR, **I want** a deployment timeline of solution imports/version changes, **so that** I can see release history at a glance.
  - **AC:** Timeline renders as inline SVG/PNG; reuses the ALM timeline model if that tool exists.
- **US-RPT5.2.2** `[Planned]` **As** an ALM lead, **I want** a solution-version grid with publisher/version filters, **so that** I can drill into specific solutions.

## FEAT-RPT5-3 — Deployment KPIs `[Planned]`
- **US-RPT5.3.1** `[Planned]` **As** an MGR, **I want** deployment-frequency and release-volume KPIs plus a frequency chart, **so that** I can judge cadence.
  - **AC:** KPI calc is deterministic from the collected history and explainable.
- **US-RPT5.3.2** `[Planned]` **As** an ALM lead, **I want** upgrade/patch/hotfix pattern detection, **so that** I can spot unstable release behavior.

## FEAT-RPT5-4 — Deployment risk panel `[Planned]`
- **US-RPT5.4.1** `[Planned]` **As** an ARCH, **I want** risk indicators (hotfix bursts, unmanaged changes after deployment), **so that** I can flag risky ALM patterns.
  - **AC:** Risk rules reuse the shipped Deployment Risk Analyzer engine; findings carry Critical/High/Medium/Low/Info severity.

## FEAT-RPT5-5 — Snapshots & trend `[Planned]`
- **US-RPT5.5.1** `[Planned]` **As** an MGR, **I want** frequency/volume saved locally and trended, **so that** I can show cadence changing over time.
  - **AC:** Snapshots persist locally with no credentials.

## FEAT-RPT5-6 — Export `[Planned]`
- **US-RPT5.6.1** `[Planned]` **As** an MGR, **I want** to export the deployment analytics report to Excel/PDF/CSV/HTML, **so that** I can share release insight.

...
## EPIC-SOLN08 - Quantify a solution's complexity and the effort to maintain it
...
## FEAT-SOLN08-0 - Scaffold & shared wiring `[Done]`

- **US-SOLN08-0.1** `[Done]` **As** a TOOLDEV, **I want** the tool to load in XrmToolBox with connection,
...
## FEAT-SOLN08-1 - Component inventory `[Done]`

- **US-SOLN08-1.1** `[Done]` **As** an architect, **I want** to pick a solution and inventory every component
...
## FEAT-SOLN08-2 - Scoring, maintainability & effort `[Done]`

- **US-SOLN08-3** `[Done]` **As** an ALM lead, **I want** a 0–100 complexity score and a maintainability
  score, **so that** I can communicate solution health at a glance.
  - **AC:** Weighted per-dimension points map to a saturating 0–100 score; maintainability = 100 − complexity. *(TC-SOLN08-METRIC-01..03)*
- **US-SOLN08-4** `[Done]` **As** a delivery manager, **I want** upgrade/migration/testing effort (in days)
  and an annual support-cost estimate, **so that** I can budget the next release.
  - **AC:** Effort/cost are transparent, documented linear functions of the tallies (exact-value tested). *(TC-SOLN08-METRIC-02)*
- **US-SOLN08-5** `[Done]` **As** an architect, **I want** hotspot findings (wide forms, high plugin counts,
  large automation/JS/data model), **so that** I know where to focus.
  - **AC:** Outliers surface as findings; an unremarkable solution reads as "no structural hotspots". *(TC-SOLN08-REPORT-06)*

## FEAT-SOLN08-3 - Dashboard, export & summary `[Done]`

- **US-SOLN08-6** `[Done]` **As** an executive, **I want** a dashboard (gauge + metric strip) and Excel/PDF/
...
- **US-SOLN08-7** `[Done]` **As** an ALM lead, **I want** an executive summary (offline default, AI opt-in),

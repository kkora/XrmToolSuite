...
> **Suggested tag:** `SOLN5` · **Suggested project:** `XrmToolSuite.SolutionDocumentationGenerator`
> **Overlaps:** Overlaps Documentation-section candidates (any tool that emits solution/metadata docs). Reuse the shipped **Solution Complexity Score** / **Solution Knowledge Graph** component inventory and dependency graph as the documentation data source and diagram input rather than re-scanning. Diagram/ERD generation overlaps Dependency Heatmap (SOLN2) rendering — share the pure-managed rendering helper.
...
## EPIC-SOLN5 — Generate complete, current solution documentation on demand
...
## FEAT-SOLN5-1 — Select solution, template, format, and sections `[Planned]`
- **US-SOLN5.1.1** `[Planned]` **As** a TOOLDEV, **I want** to pick a solution, a documentation mode, an output format, and which sections to include, **so that** I generate exactly the document I need.
...
- **US-SOLN5.1.2** `[Planned]` **As** a TOOLDEV, **I want** a branding/settings panel, **so that** generated docs carry my org's header/logo/publisher info.
  - **AC:** Branding fields round-trip in settings and appear in the rendered document header; no credentials stored.

## FEAT-SOLN5-2 — Document data, schema, and automation components `[Planned]`
- **US-SOLN5.2.1** `[Planned]` **As** a TOOLDEV, **I want** tables, columns, relationships, and choices/option sets documented, **so that** the data model is fully described.
...
- **US-SOLN5.2.2** `[Planned]` **As** a TOOLDEV, **I want** forms, views, charts, dashboards, and apps documented, **so that** the UI layer is captured.
...
- **US-SOLN5.2.3** `[Planned]` **As** a TOOLDEV, **I want** business rules, workflows/flows, plugins (assembly/type/step/image), web resources/JS, custom APIs, env vars, connection refs, and included security roles documented, **so that** automation and configuration are covered.
...
## FEAT-SOLN5-3 — Diagrams and summaries `[Planned]`
- **US-SOLN5.3.1** `[Planned]` **As** an ARCH, **I want** dependency and ERD-style relationship diagrams generated, **so that** the document shows structure visually.
...
- **US-SOLN5.3.2** `[Planned]` **As** an ARCH, **I want** a component inventory, release notes, and an architecture summary generated, **so that** the document has executive-level framing.
  - **AC:** Inventory counts by component type; release notes and architecture summary sections are produced from the scanned data.

## FEAT-SOLN5-4 — Preview and generation progress `[Planned]`
- **US-SOLN5.4.1** `[Planned]` **As** a TOOLDEV, **I want** a preview and per-section progress while generating, **so that** I can verify content before exporting a large document.
  - **AC:** Generation reports progress per section and is cancellable; a preview panel shows rendered content before final export.

## FEAT-SOLN5-5 — Multi-format export `[Planned]`
- **US-SOLN5.5.1** `[Planned]` **As** a TOOLDEV, **I want** the document exported to Word, PDF, Markdown, HTML, Excel, and JSON, **so that** I can meet audit, handoff, and machine-readable needs.

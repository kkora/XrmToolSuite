...
> **FEAT-SOLN08-4** (a 0–100 build-quality score over the same `ComponentCounts`), per the recommendation below,
> rather than as a standalone `XrmToolSuite.SolutionQualityScore` project. See
> [`../../user-stories/SOLN08.SolutionComplexityScore.md`](../../user-stories/SOLN08.SolutionComplexityScore.md) →
> FEAT-SOLN08-4 and the design at [`../../design/SOLN04-RPT04-extensions.md`](../../design/SOLN04-RPT04-extensions.md).
...
> **Suggested tag:** `SOLN04` · **Suggested project:** `XrmToolSuite.SolutionQualityScore`
...
## EPIC-SOLN04 — Score a solution's deployment and governance readiness 0–100
...
## FEAT-SOLN04-1 — Select solution and compute overall score `[Planned]`
- **US-SOLN04.1.1** `[Planned]` **As** an ALM engineer, **I want** to select one solution and get an overall quality score, **so that** I have a single readiness number.
...
- **US-SOLN04.1.2** `[Planned]` **As** an ALM engineer, **I want** the score reproducible and reusing the Complexity Score engine, **so that** results stay consistent across the suite.
  - **AC:** Scoring reuses the shared analyzer/scoring core; weighting lives in settings and round-trips via Load/SaveSettings.

## FEAT-SOLN04-2 — Governance and ALM-readiness categories `[Planned]`
- **US-SOLN04.2.1** `[Planned]` **As** an ALM engineer, **I want** ALM Readiness and Dependency Quality scored, **so that** I know the solution imports cleanly and is layered correctly.
...
- **US-SOLN04.2.2** `[Planned]` **As** an ALM engineer, **I want** Configuration Readiness and Security Readiness scored, **so that** I know env vars, connection refs, plugin steps, and role inclusion are sound.
...
## FEAT-SOLN04-3 — Maintainability, naming, and documentation categories `[Planned]`
- **US-SOLN04.3.1** `[Planned]` **As** a CUST developer, **I want** Naming Standards and Documentation Completeness scored, **so that** the solution is maintainable and self-describing.
...
- **US-SOLN04.3.2** `[Planned]` **As** a CUST developer, **I want** Maintainability, Performance Risk, and Component Hygiene scored, **so that** complexity and dead weight are visible.
...
## FEAT-SOLN04-4 — Findings, severity, and remediation backlog `[Planned]`
- **US-SOLN04.4.1** `[Planned]` **As** a Delivery Manager, **I want** findings ranked by severity, **so that** I address the biggest risks first.
  - **AC:** Findings carry Critical/High/Medium/Low/Info severity and link to the component and category that produced them.
- **US-SOLN04.4.2** `[Planned]` **As** a Delivery Manager, **I want** a remediation backlog generated from the findings, **so that** the score turns into actionable work.
  - **AC:** Backlog items describe the fix and its category impact; the backlog is exportable.

## FEAT-SOLN04-5 — Executive and technical export `[Planned]`
- **US-SOLN04.5.1** `[Planned]` **As** a release manager, **I want** executive and technical reports exported to Excel, PDF, JSON, and HTML, **so that** I can share the score with stakeholders and gate a release.

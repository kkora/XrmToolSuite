...
> **Suggested tag:** `SOLN7` · **Suggested project:** `XrmToolSuite.SolutionSizeOptimizer`
...
## EPIC-SOLN7 — Measure and shrink solution footprint
...
## FEAT-SOLN7-1 — Solution size measurement `[Planned]`
- **US-SOLN7.1.1** `[Planned]` **As** an ALM, **I want** total solution size broken down by component type, **so that** I know where the weight is.
...
- **US-SOLN7.1.2** `[Planned]` **As** an ALM, **I want** to pick which solution(s) to measure, **so that** I can target the artifact I ship.
  - **AC:** Solution picker lists unmanaged (and optionally managed) solutions; selection scopes all subsequent analysis.
- **US-SOLN7.1.3** `[Planned]` **As** a TOOLDEV, **I want** size math done in a pure, testable function, **so that** results are deterministic.
  - **AC:** Byte-size + unit conversion + banding implemented UI-free; covered by `testing/UnitTests/` fixtures.

## FEAT-SOLN7-2 — Oversized asset detection `[Planned]`
- **US-SOLN7.2.1** `[Planned]` **As** an ALM, **I want** oversized assets (large web resources, images, assemblies) ranked, **so that** I can target the biggest wins.
  - **AC:** Assets over a configurable threshold flagged and sorted by size descending; each shows type, name, and byte size.
- **US-SOLN7.2.2** `[Planned]` **As** an ALM, **I want** oversized-image and un-minified-script hints, **so that** I know a shrink is possible.
...
- **US-SOLN7.2.3** `[Planned]` **As** a TOOLDEV, **I want** oversized thresholds configurable and persisted, **so that** teams set their own limits.
  - **AC:** Thresholds editable and round-tripped via a plain settings POCO (load in `*_Load`, save in `ClosingPlugin`).

## FEAT-SOLN7-3 — Unused-resource candidates `[Planned]`
- **US-SOLN7.3.1** `[Planned]` **As** an ALM, **I want** web resources that appear referenced by nothing surfaced, **so that** I can consider removing dead weight.
...
- **US-SOLN7.3.2** `[Planned]` **As** an ALM, **I want** each candidate to show why it was flagged, **so that** I can judge safety before acting.
...
## FEAT-SOLN7-4 — Deployment impact estimate `[Planned]`
- **US-SOLN7.4.1** `[Planned]` **As** an ALM, **I want** an estimated deployment-impact summary, **so that** I can anticipate import cost.
...
- **US-SOLN7.4.2** `[Planned]` **As** an MGR, **I want** a before/after projection when candidates are removed, **so that** I can justify the cleanup.
...
## FEAT-SOLN7-5 — Reporting & export `[Planned]`
- **US-SOLN7.5.1** `[Planned]` **As** an ALM, **I want** to export the optimization report, **so that** I can review and share it.
  - **AC:** CSV/HTML export of size breakdown, oversized assets, unused candidates, and deployment estimate; export off the UI thread.
- **US-SOLN7.5.2** `[Planned]` **As** an MGR, **I want** a one-page summary, **so that** I can report footprint at a glance.
  - **AC:** Summary shows total size, top offenders, and count of unused candidates; regenerates on filter change.

## FEAT-SOLN7-6 — Optional gated cleanup `[Planned]`
- **US-SOLN7.6.1** `[Planned]` **As** an ALM, **I want** cleanup to be report-only by default, **so that** I never delete something unexpectedly.
  - **AC:** Default mode performs no writes; any delete action is explicitly opt-in.
- **US-SOLN7.6.2** `[Planned]` **As** an ALM, **I want** any delete guarded by a confirmation stating scope and count, **so that** destructive actions are deliberate.

...
> **Suggested tag:** `SOLN06` · **Suggested project:** `XrmToolSuite.SolutionModernizationAdvisor`
...
## EPIC-SOLN06 — Turn legacy customizations into a prioritized modernization roadmap
...
## FEAT-SOLN06-1 — Scope and inventory legacy components `[Planned]`
- **US-SOLN06.1.1** `[Planned]` **As** an ARCH, **I want** to scan a selected solution or the whole environment for legacy components, **so that** I can scope the modernization review.
...
- **US-SOLN06.1.2** `[Planned]` **As** an ARCH, **I want** the inventory reuse Technical Debt Analyzer detections, **so that** results stay consistent across the suite.
...
## FEAT-SOLN06-2 — Automation and plugin modernization detection `[Planned]`
- **US-SOLN06.2.1** `[Planned]` **As** a DEVOPS engineer, **I want** classic workflows that could move to Power Automate or plugins/custom APIs flagged, **so that** I can plan automation modernization.
...
- **US-SOLN06.2.2** `[Planned]` **As** a CUST developer, **I want** old plugin patterns and plugins better suited as custom APIs flagged, **so that** I can modernize the plugin layer.
...
## FEAT-SOLN06-3 — JavaScript, form, and config modernization detection `[Planned]`
- **US-SOLN06.3.1** `[Planned]` **As** a CUST developer, **I want** deprecated `Xrm.Page` usage and synchronous/blocking scripts detected, **so that** I can modernize client scripting.
...
- **US-SOLN06.3.2** `[Planned]` **As** an ARCH, **I want** hardcoded URLs/GUIDs/env-specific values, missing environment variables, and overloaded forms detected, **so that** I can modernize configuration and UI.
...
- **US-SOLN06.3.3** `[Planned]` **As** a SEC reviewer, **I want** unmanaged-should-be-managed components, duplicate tables/columns/views/forms, security-role complexity, and audit/governance gaps flagged, **so that** ALM, security, and governance modernization are covered.
...
## FEAT-SOLN06-4 — Modernization score and roadmap `[Planned]`
- **US-SOLN06.4.1** `[Planned]` **As** an ARCH, **I want** a modernization score, **so that** I can track a solution's modernization posture over time.
...
- **US-SOLN06.4.2** `[Planned]` **As** an ARCH, **I want** recommendations grouped into quick wins, medium effort, and strategic initiatives on an effort-vs-value matrix, **so that** I can prioritize the roadmap.
...
## FEAT-SOLN06-5 — Backlog and export `[Planned]`
- **US-SOLN06.5.1** `[Planned]` **As** a Delivery Manager, **I want** a migration/refactoring backlog and the modernization report exported to Excel, PDF, JSON, and HTML, **so that** I can drive the modernization program.

...
> **Suggested tag:** `PP03` · **Suggested project:** `XrmToolSuite.PortalSecurityScanner`
> **Overlaps:** Deployment Risk Analyzer hints at anonymous-access risk during Power Pages readiness, and Portal Health Analyzer (PP01) surfaces a security summary. This tool owns the deep, dedicated security analysis (web-role matrix, table-permission matrix, sensitive-table exposure, remediation checklist). PP01's security cards should link here; do not duplicate the deep logic in PP01.
...
## EPIC-PP03 — Scan a Power Pages website for data-exposure and permission risks
...
## FEAT-PP03-1 — Web-role & authentication analysis `[Planned]`
- **US-PP03.1.1** `[Planned]` **As** a SEC engineer, **I want** all web roles listed with their anonymous/authenticated flags, **so that** I understand the site's role model.
...
- **US-PP03.1.2** `[Planned]` **As** a SEC engineer, **I want** website authentication settings analyzed, **so that** I can spot gaps (open registration, missing external auth).
...
## FEAT-PP03-2 — Table-permission analysis `[Planned]`
- **US-PP03.2.1** `[Planned]` **As** a SEC engineer, **I want** a table-permission matrix (table × scope × rights × role), **so that** I can see who can do what.
...
- **US-PP03.2.2** `[Planned]` **As** a SEC engineer, **I want** broad Global-scope read/write/delete grants flagged, **so that** I catch over-permissive rules.
  - **AC:** Global scope + write/delete → High; broad authenticated delete → High per severity rules.
- **US-PP03.2.3** `[Planned]` **As** a SEC engineer, **I want** missing parent permissions and duplicate/conflicting permissions detected, **so that** I fix incomplete or ambiguous scoping.
...
## FEAT-PP03-3 — Anonymous & sensitive-data exposure `[Planned]`
- **US-PP03.3.1** `[Planned]` **As** a SEC engineer, **I want** anonymous write/delete access flagged Critical, **so that** the worst exposures top the list.
  - **AC:** Any permission granting the anonymous role write/delete → Critical with table and rights shown.
- **US-PP03.3.2** `[Planned]` **As** a SEC engineer, **I want** anonymous read to sensitive tables flagged Critical, **so that** I catch PII/data leaks.
  - **AC:** Anonymous read on a watchlisted sensitive table → Critical; watchlist editable via settings.
- **US-PP03.3.3** `[Planned]` **As** a SEC engineer, **I want** public pages exposing forms/lists without proper permissions detected, **so that** I close unguarded data surfaces.
  - **AC:** A form/list on an anonymously-accessible page with no backing table permission → High.

## FEAT-PP03-4 — Site-setting & upload risks `[Planned]`
- **US-PP03.4.1** `[Planned]` **As** a SEC engineer, **I want** disabled CAPTCHA/anti-bot settings flagged where applicable, **so that** I harden public forms.
  - **AC:** Relevant CAPTCHA/anti-bot site settings evaluated; disabled-where-expected → Medium/High.
- **US-PP03.4.2** `[Planned]` **As** a SEC engineer, **I want** file-upload security risks flagged, **so that** I prevent unsafe uploads.
...
## FEAT-PP03-5 — Score, remediation & export `[Planned]`
- **US-PP03.5.1** `[Planned]` **As** an MGR, **I want** a portal security score with a band, **so that** I can report posture.
  - **AC:** Weighted severities produce a 0–100 score with a Low/Medium/High band; scoring engine is SDK-free and unit-tested.
- **US-PP03.5.2** `[Planned]` **As** a SEC engineer, **I want** a remediation checklist, **so that** I have an ordered action plan.
  - **AC:** Each finding maps to a remediation item with the affected role/table/permission and recommended change.
- **US-PP03.5.3** `[Planned]` **As** an MGR, **I want** the security report exported, **so that** I can hand it to auditors.

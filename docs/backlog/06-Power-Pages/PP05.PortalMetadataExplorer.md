...
> **Suggested tag:** `PP05` · **Suggested project:** `XrmToolSuite.PortalMetadataExplorer`
> **Overlaps:** Portal Health Analyzer (PP01), Security Scanner (PP03), and Performance Analyzer (PP04) all retrieve the same portal tables — PP05 is the plain browse/search/inventory tool with no scoring. It is the natural home for the shared Power Pages retrieval/normalization layer that PP01/PP03/PP04/PP06/PP07 reuse. Solution Knowledge Graph handles solution components, not portal-content browsing.
...
- This tool defines the shared Power Pages retrieval + normalization model (detect schema, map `adx_`/`mspp_` fields into one neutral record shape) that PP01/PP03/PP04/PP06/PP07 consume. Keep that layer UI-free in Shared/Core.
...
## EPIC-PP05 — Give one searchable explorer for all Power Pages metadata
...
## FEAT-PP05-1 — Website selection & metadata tree `[Planned]`
- **US-PP05.1.1** `[Planned]` **As** a MAKER, **I want** to select a website and see a navigation tree of its metadata categories, **so that** I can drill into any record type.
...
- **US-PP05.1.2** `[Planned]` **As** a MAKER, **I want** counts on each tree node and my selection remembered, **so that** I navigate efficiently.
  - **AC:** Each node shows a record count; selected website + expanded nodes persist via settings.

## FEAT-PP05-2 — Record grid & detail viewer `[Planned]`
- **US-PP05.2.1** `[Planned]` **As** a MAKER, **I want** a grid of records for the selected category, **so that** I can scan and sort them.
  - **AC:** Grid lists records with key columns per type, sortable/filterable in-grid, populated from the normalized model.
- **US-PP05.2.2** `[Planned]` **As** a MAKER, **I want** a detail viewer for a selected record, **so that** I can inspect all its fields.
...
## FEAT-PP05-3 — Search & filter `[Planned]`
- **US-PP05.3.1** `[Planned]` **As** a MAKER, **I want** to search by name, URL, or partial URL, **so that** I can find a page/record fast.
...
- **US-PP05.3.2** `[Planned]` **As** a MAKER, **I want** to search by template, snippet, setting, or GUID, **so that** I can locate any referenced object.
  - **AC:** Search resolves a GUID to its record and type, and matches template/snippet/setting names across both schemas.

## FEAT-PP05-4 — Relationships & dependencies `[Planned]`
- **US-PP05.4.1** `[Planned]` **As** a MAKER, **I want** parent/child relationships shown for a record, **so that** I understand site structure.
...
- **US-PP05.4.2** `[Planned]` **As** a MAKER, **I want** associated Dataverse table/entity references shown, **so that** I know what data a form/list/permission binds to.
  - **AC:** Detail/relationship panel lists the bound entity logical name(s) for forms, lists, and permissions.

## FEAT-PP05-5 — Inventory export `[Planned]`
- **US-PP05.5.1** `[Planned]` **As** an ARCH, **I want** the full portal metadata inventory exported, **so that** I can document or diff a site.
...
- **US-PP05.5.2** `[Planned]` **As** an ARCH, **I want** to export the currently filtered/selected category, **so that** I can share a focused slice.

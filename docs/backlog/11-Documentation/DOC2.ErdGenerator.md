...
> **Suggested tag:** `DOC2` · **Suggested project:** `XrmToolSuite.ErdGenerator`
> **Overlaps:** **Solution Knowledge Graph** (SHIPPED) — diagram/graph overlap for relationship rendering; reuse its graph model/renderer. **Architecture Diagram Generator** (DOC1) is the component-level counterpart — share the pure-managed SVG/PNG renderer and Mermaid/PlantUML emitters. **Solution Documentation Generator** (SOLN5 candidate) embeds ERD references — this tool can be its ERD engine. Table/relationship metadata overlaps **Relationship Validator** (ADMIN6) extraction.
...
## EPIC-DOC2 — Generate accurate Dataverse ERDs from live metadata
...
## FEAT-DOC2-1 — Scope and table selection `[Planned]`
- **US-DOC2.1.1** `[Planned]` **As** a TOOLDEV, **I want** to scope the ERD by one solution, a publisher, a table group, or the full environment, **so that** I diagram only the relevant tables.
  - **AC:** Scope selectors load off the UI thread via `RunAsync`; selection persists via Load/SaveSettings.
- **US-DOC2.1.2** `[Planned]` **As** a TOOLDEV, **I want** a table selector to add/remove specific tables, **so that** I can trim or extend the auto-scoped set.
...
## FEAT-DOC2-2 — Schema, keys, and columns `[Planned]`
- **US-DOC2.2.1** `[Planned]` **As** a TOOLDEV, **I want** each table shown with its display and schema name plus primary and alternate keys, **so that** identity and uniqueness are clear.
...
- **US-DOC2.2.2** `[Planned]` **As** a TOOLDEV, **I want** important columns and column-display settings, **so that** the ERD is readable without listing every attribute.
...
## FEAT-DOC2-3 — Relationships and cascade behavior `[Planned]`
- **US-DOC2.3.1** `[Planned]` **As** an ARCH, **I want** one-to-many, many-to-one, and many-to-many relationships plus lookup columns drawn between tables, **so that** the data model structure is visible.
  - **AC:** Each relationship type renders with correct cardinality notation; lookup columns link to their target table(s).
- **US-DOC2.3.2** `[Planned]` **As** an ARCH, **I want** cascade behavior and required/optional relationships displayed, **so that** I understand delete/assign/share propagation and referential rules.
...
## FEAT-DOC2-4 — Filters, custom/managed status, and layout `[Planned]`
- **US-DOC2.4.1** `[Planned]` **As** a TOOLDEV, **I want** filters by solution, table prefix, relationship type, custom-only, and managed-only, **so that** I can isolate exactly the slice I need.
  - **AC:** Filters apply to the model before layout without re-querying; filter state round-trips in settings.
- **US-DOC2.4.2** `[Planned]` **As** a TOOLDEV, **I want** custom-vs-standard and managed-vs-unmanaged visually distinguished, plus layout options and a preview canvas, **so that** governance-relevant tables stand out and I can verify before export.
  - **AC:** Custom/managed status is styled distinctly; preview reflects current filters/layout.

## FEAT-DOC2-5 — Multi-format export `[Planned]`
- **US-DOC2.5.1** `[Planned]` **As** a TOOLDEV, **I want** to export the ERD to Mermaid, PlantUML, SVG, and PNG, **so that** it drops into Git, wikis, and design docs.
  - **AC:** Mermaid `erDiagram`/PlantUML emitted as text; SVG/PNG via the shared renderer; export runs off the UI thread.
- **US-DOC2.5.2** `[Planned]` **As** a TOOLDEV, **I want** to export to PDF, HTML, Markdown, and JSON, **so that** I get both formatted documents and a machine-readable model.

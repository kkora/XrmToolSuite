...
> **Suggested tag:** `DOC6` · **Suggested project:** `XrmToolSuite.ApiDocumentationBuilder`
> **Overlaps:** **Solution Documentation Generator** (SOLN5 candidate) documents Custom APIs as one section — this tool is the deep, API-focused specialist (parameters, responses, OpenAPI-style, examples, redaction). **Markdown / Word / HTML Documentation Generators** (DOC3/DOC4/DOC5) are format renderers whose API section can consume this tool's model. Custom API + plugin metadata overlaps **Plugin Dependency Graph** (PLUGIN1) extraction. Export patterns reuse **Deployment Risk Analyzer** (SHIPPED) PDF/Excel and the self-contained-HTML pattern.
...
## Notes
- Shared extracted-metadata model: build an API model (Custom API → parameters → response properties → binding → plugin/step → solution) reusing the Solution Documentation Generator (SOLN5) / Plugin Dependency Graph extraction where it already reads these tables.
...
## EPIC-DOC6 — Document Dataverse Custom APIs and integration endpoints safely
...
## FEAT-DOC6-1 — Discovery and selection `[Planned]`
- **US-DOC6.1.1** `[Planned]` **As** a TOOLDEV, **I want** an API discovery dashboard listing Custom APIs in scope, **so that** I can see what exists before documenting.
...
- **US-DOC6.1.2** `[Planned]` **As** a TOOLDEV, **I want** to select one or more Custom APIs (and legacy actions where available), **so that** I document only what I need.
  - **AC:** Selection drives the API model; legacy custom actions are included when present and labeled as legacy.

## FEAT-DOC6-2 — API detail and parameters `[Planned]`
- **US-DOC6.2.1** `[Planned]` **As** a TOOLDEV, **I want** each API's name, unique name, display name, binding type, bound table, allowed custom processing step type, plugin type, and solution documented, **so that** its contract and ownership are clear.
  - **AC:** A detail viewer shows these fields sourced from `customapi`/`sdkmessage` metadata; missing fields degrade to a note.
- **US-DOC6.2.2** `[Planned]` **As** a TOOLDEV, **I want** a parameter/response grid, **so that** I see request parameters and response properties with types and requirement.
  - **AC:** `customapirequestparameter`/`customapiresponseproperty` render in a grid with name, type, required, and description.

## FEAT-DOC6-3 — Dependencies and related configuration `[Planned]`
- **US-DOC6.3.1** `[Planned]` **As** a TOOLDEV, **I want** plugin/custom-API relationships and API dependencies documented, **so that** I know what implements and consumes each API.
...
- **US-DOC6.3.2** `[Planned]` **As** an ARCH, **I want** related environment variables, connection references, and table Web API endpoint patterns documented, **so that** integration context is complete.
...
## FEAT-DOC6-4 — Safe examples, OpenAPI-style spec, and redaction `[Planned]`
- **US-DOC6.4.1** `[Planned]` **As** an integration DEVOPS, **I want** generated example request/response payloads and an OpenAPI-style JSON spec, **so that** I can bootstrap client code.
...
- **US-DOC6.4.2** `[Planned]` **As** a SEC, **I want** secrets, tokens, keys, full HTTP-trigger URLs, and environment-specific values masked, with user-controlled redaction, **so that** documentation never leaks sensitive endpoint data.
...
## FEAT-DOC6-5 — Multi-format export `[Planned]`
- **US-DOC6.5.1** `[Planned]` **As** a TOOLDEV, **I want** to export API reference pages to Markdown, HTML, and JSON, **so that** docs feed repos, portals, and tooling.
...
- **US-DOC6.5.2** `[Planned]` **As** an MGR, **I want** to export to Word and PDF, **so that** I can hand a polished API reference to clients/auditors.

...
> **Suggested tag:** `DOC4` · **Suggested project:** `XrmToolSuite.WordDocumentationGenerator`
> **Overlaps:** **Solution Documentation Generator** (SOLN5 candidate) — STRONG overlap: this is the DOCX/PDF renderer over the same extracted model. **Markdown / HTML Documentation Generators** (DOC3/DOC5) are sibling renderers — share the extracted model + section engine. Diagram insertion overlaps **Architecture Diagram Generator** (DOC1) / **ERD Generator** (DOC2) — embed their SVG/PNG output. **Deployment Risk Analyzer** (SHIPPED) already produces PDF/Excel via the sanctioned chains — reuse those.
...
## Notes
- Shared extracted-metadata model: reuse the Solution Documentation Generator (SOLN5) / Environment Inventory extraction — this tool is a *DOCX/PDF renderer + template engine* over that model, not a new scanner.
...
- Diagram strategy: embed diagrams as **SVG/PNG** produced by DOC1/DOC2's pure-managed renderer; OpenXml/MigraDoc insert them as images (Word prefers EMF/PNG). No native diagram NuGet.
...
## EPIC-DOC4 — Generate professional Word/PDF solution documentation
...
## FEAT-DOC4-1 — Scope, template, and section selection `[Planned]`
- **US-DOC4.1.1** `[Planned]` **As** an MGR, **I want** to pick a solution or full environment and a document template (executive, technical, audit, support handoff, release), **so that** the output matches the audience.
  - **AC:** Scope loads off the UI thread via `RunAsync`; template and scope persist via Load/SaveSettings.
- **US-DOC4.1.2** `[Planned]` **As** an MGR, **I want** a section checklist and a preview summary, **so that** I include only relevant sections and see what will be generated.
  - **AC:** Section selection round-trips in settings; preview summary lists included sections and estimated content before export.

## FEAT-DOC4-2 — Front matter and formatted sections `[Planned]`
- **US-DOC4.2.1** `[Planned]` **As** an MGR, **I want** a cover page, table of contents, revision history, and executive + architecture summaries, **so that** the document reads as a formal deliverable.
  - **AC:** These sections render via the OpenXml document engine with a live TOC field; content derives from the extracted model.
- **US-DOC4.2.2** `[Planned]` **As** a TOOLDEV, **I want** a component inventory plus tables, columns, relationships, forms, views, dashboards, plugins, flows, custom APIs, security roles, env vars, connection references, and Power Pages metadata as formatted sections, **so that** the full solution is captured.
...
## FEAT-DOC4-3 — Diagrams, risk, and appendix `[Planned]`
- **US-DOC4.3.1** `[Planned]` **As** an ARCH, **I want** diagrams (architecture/ERD) and data tables embedded in the document, **so that** structure is shown visually.
  - **AC:** Diagrams render as SVG/PNG via the DOC1/DOC2 renderer and insert as images through OpenXml/MigraDoc.
- **US-DOC4.3.2** `[Planned]` **As** an MGR, **I want** risk/recommendation sections and an appendix, **so that** the document supports production-readiness review.
...
## FEAT-DOC4-4 — Branding `[Planned]`
- **US-DOC4.4.1** `[Planned]` **As** an MGR, **I want** to set logo, author, company name, version, and footer, **so that** the document carries corporate branding.
  - **AC:** Branding fields round-trip in settings and appear on cover/header/footer; no credentials stored.

## FEAT-DOC4-5 — DOCX and PDF export `[Planned]`
- **US-DOC4.5.1** `[Planned]` **As** an MGR, **I want** to export to DOCX, **so that** recipients can edit the handoff document.
...
- **US-DOC4.5.2** `[Planned]` **As** an MGR, **I want** to export to PDF, **so that** I can distribute a locked, final version.

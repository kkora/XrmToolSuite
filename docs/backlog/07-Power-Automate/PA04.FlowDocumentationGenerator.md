...
> **Suggested tag:** `PA04` · **Suggested project:** `XrmToolSuite.FlowDocumentationGenerator`
> **Overlaps:** Solution Documentation Generator (SOLN05) documents solutions broadly — this tool is a *cloud-flow-focused* doc generator with per-flow action trees, trigger/connector/connection-reference sections, and diagrams. Reuse DRA's `clientdata` parser and PA01's dependency model; align export/branding scaffolding with SOLN05 where it exists.
> **Value/priority (my read):** High — documentation is perennially missing, the data is fully SDK-feasible (static parse), and it directly reuses PA01/PA02 parsing, so incremental cost is low.
...
- Reuse the shared `clientdata` parser and PA01's dependency model; keep the parsing/generation logic UI-free so it is unit-testable and CI-liftable.
...
## EPIC-PA04 — Generate accurate, audience-appropriate flow documentation
...
## FEAT-PA04-1 — Scope selection and parsing `[Planned]`
- **US-PA04.1.1** `[Planned]` **As** an ARCH, **I want** to select flows by environment or solution, one or many, **so that** I can document a single flow or a whole solution.
  - **AC:** Flows load via `RetrieveAll` off the UI thread with progress/cancel; multi-select and solution scope both supported.
- **US-PA04.1.2** `[Planned]` **As** an ARCH, **I want** the definition parsed into a documentable model, **so that** every section has structured data to render.
...
## FEAT-PA04-2 — Core flow documentation sections `[Planned]`
- **US-PA04.2.1** `[Planned]` **As** an ARCH, **I want** trigger, connectors, connection references, and environment variables documented, **so that** the flow's inputs and bindings are clear.
...
- **US-PA04.2.2** `[Planned]` **As** a support engineer, **I want** the action/logic structure documented — conditions, loops, scopes, branches, child flows, run-after, retry, error handling, and Dataverse tables/columns used, **so that** I can support the flow without opening the designer.
...
## FEAT-PA04-3 — Documentation modes and section control `[Planned]`
- **US-PA04.3.1** `[Planned]` **As** an ARCH, **I want** to pick a documentation mode (executive summary, technical spec, support handoff, audit, onboarding, full reference, release notes), **so that** each audience gets the right depth.
  - **AC:** Selecting a mode includes/excludes sections accordingly; a section checklist lets me override per document.
- **US-PA04.3.2** `[Planned]` **As** an ARCH, **I want** a branding/settings panel (title, org, logo, footer) that round-trips, **so that** documents look consistent and my choices persist.
...
## FEAT-PA04-4 — Diagrams and handoff artifacts `[Planned]`
- **US-PA04.4.1** `[Planned]` **As** an ARCH, **I want** a dependency diagram and a flow action tree generated, **so that** the structure is visual, not just prose.
  - **AC:** A diagram (text/DSL rendered to HTML/Markdown, with optional image export) and an action tree are produced per flow.
- **US-PA04.4.2** `[Planned]` **As** a release manager, **I want** support-handoff notes and release notes generated, **so that** deployment and support have ready-made artifacts.
  - **AC:** Handoff notes (owner, status, dependencies, known caveats) and release notes render in their respective modes.

## FEAT-PA04-5 — Preview and multi-format export `[Planned]`
- **US-PA04.5.1** `[Planned]` **As** an ARCH, **I want** a preview before export, **so that** I can confirm content and layout.
  - **AC:** Preview panel renders the selected mode/sections; regenerating is off the UI thread with progress.
- **US-PA04.5.2** `[Planned]` **As** an ARCH, **I want** export to Word, PDF, Markdown, HTML, Excel, and JSON, **so that** I can use the docs anywhere.
...
- Read-only default; parsing/generation engine is UI-free and unit-tested; reuses shared `clientdata` parser and PA01 dependency model; secrets and HTTP trigger URLs never rendered in any document.

...
> **Suggested tag:** `PA07` · **Suggested project:** `XrmToolSuite.TriggerDependencyGraph`
> **Overlaps:** PA01 Flow Dependency Analyzer (parses the same `clientdata`; PA01 covers full dependency trees while PA07 focuses on *triggers* and cross-flow cascade). Solution Knowledge Graph (general component graph/visualization). Reuse DRA/PA01 `clientdata` parser and, if built, PA01's dependency model; PA07 adds the trigger taxonomy, duplicate/cascade detection, and graph rendering.
...
## EPIC-PA07 — Visualize flow triggers and their cross-flow dependencies
...
## FEAT-PA07-1 — Trigger inventory and categorization `[Planned]`
- **US-PA07.1.1** `[Planned]` **As** an ADM, **I want** all flow triggers listed and categorized by type (Dataverse, Recurrence, Manual, HTTP, SharePoint, Outlook, Teams, custom connector, child flow, other), **so that** I can see how the estate is triggered.
...
- **US-PA07.1.2** `[Planned]` **As** an ADM, **I want** a table/event filter, **so that** I can focus on triggers for a specific Dataverse table or event.
  - **AC:** Filtering by table and message narrows the inventory and the graph.

## FEAT-PA07-2 — Trigger detail extraction `[Planned]`
- **US-PA07.2.1** `[Planned]` **As** an ADM, **I want** Dataverse trigger details — table, operation, filter conditions, and filtering attributes where available — shown per flow, **so that** I know exactly what fires it.
...
- **US-PA07.2.2** `[Planned]` **As** an ADM, **I want** recurrence schedules and HTTP trigger endpoint metadata shown with secrets redacted, **so that** I understand timing and inbound triggers without leaking URLs.
  - **AC:** Recurrence schedule renders; HTTP trigger shows method/shape but redacts callback URL, SAS tokens, and signatures.

## FEAT-PA07-3 — Dependency graph `[Planned]`
- **US-PA07.3.1** `[Planned]` **As** an ADM, **I want** a trigger dependency graph over the standard node types (flow, trigger, table, event, connector, child flow, external system, environment variable, connection reference), **so that** I can see relationships visually.
...
- **US-PA07.3.2** `[Planned]` **As** an ADM, **I want** parent/child flow relationships shown in the graph, **so that** I can trace flow chains.
  - **AC:** Parent→child edges are built from child-flow references and rendered as a distinct edge type.

## FEAT-PA07-4 — Duplicate and cascade risk detection `[Planned]`
- **US-PA07.4.1** `[Planned]` **As** an ADM, **I want** duplicate triggers on the same table/event and broad Dataverse triggers without filters detected, **so that** I can reduce redundant or over-firing triggers.
  - **AC:** Duplicate-trigger and no-filter findings list the affected flows with severity; unit-tested against fixtures.
- **US-PA07.4.2** `[Planned]` **As** an ADM, **I want** cascading-flow risk, multiple flows on one Dataverse event, and disabled flows with active downstream dependencies detected, **so that** I can foresee chain reactions and orphaned dependencies.
  - **AC:** Cascade chains, shared-event fan-out, and disabled-flow-with-active-dependency each produce a distinct finding.

## FEAT-PA07-5 — Export `[Planned]`
- **US-PA07.5.1** `[Planned]` **As** an ADM, **I want** the graph exported to PNG, SVG, PDF, Excel, JSON, and HTML, **so that** I can share it in reviews and docs.
...
- Read-only default; graph-building and detection logic is UI-free and unit-tested; reuses DRA/PA01 `clientdata` parser and dependency model; HTTP trigger URLs/secrets never exposed in the UI or any export.

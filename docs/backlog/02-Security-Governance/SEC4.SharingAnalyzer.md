...
> **Suggested tag:** `SEC4` · **Suggested project:** `XrmToolSuite.SharingAnalyzer`
> **Overlaps:** Sharing-risk score feeds Environment Governance Score (SEC9); heatmap visualization shares UI patterns with User Access Heatmap (SEC10).
...
## FEAT-SEC4-1 — Scoped sharing scan `[Planned]`
- **US-SEC4.1.1** `[Planned]` **As** an ADM, **I want** to pick tables (and optionally a user/team filter) before scanning POA, **so that** the scan stays within service-protection limits.
  - **AC:** Scan is scoped by table by default; a full-environment scan requires an explicit opt-in with a warning.
- **US-SEC4.1.2** `[Planned]` **As** a SEC lead, **I want** the scan to run off the UI thread with progress and cancellation, **so that** a large POA table does not freeze the tool.
  - **AC:** Paging via `RetrieveAll`; `worker.ReportProgress` updates the spinner and status bar.

## FEAT-SEC4-2 — Shared-records view `[Planned]`
- **US-SEC4.2.1** `[Planned]` **As** an ADM, **I want** a grid of shared records by table showing who shared with whom and the access rights granted, **so that** I can review the actual shares.
  - **AC:** `accessrightsmask` is decoded to named rights (Read/Write/Delete/Append/AppendTo/Assign/Share).
- **US-SEC4.2.2** `[Planned]` **As** a SEC lead, **I want** access-rights summary cards, **so that** I see the mix of granted rights at a glance.

## FEAT-SEC4-3 — Risk detection `[Planned]`
- **US-SEC4.3.1** `[Planned]` **As** a SEC lead, **I want** flags for excessive sharing, sharing with inactive users, sharing with disabled teams, and records with many shared principals, **so that** I get a prioritized cleanup list.
  - **AC:** Each finding carries severity (Critical/High/Medium/Low/Info) and the principal/record evidence.
- **US-SEC4.3.2** `[Planned]` **As** an ADM, **I want** users with unusually high inbound shared access highlighted, **so that** I can spot access sprawl.

## FEAT-SEC4-4 — Heatmap visualization `[Planned]`
- **US-SEC4.4.1** `[Planned]` **As** a SEC lead, **I want** a sharing heatmap (table × principal intensity), **so that** hotspots are visible without reading every row.

## FEAT-SEC4-5 — Cleanup recommendations `[Planned]`
- **US-SEC4.5.1** `[Planned]` **As** an ADM, **I want** recommended cleanup actions (revoke stale shares) as a preview list, **so that** I can plan remediation.
...
## FEAT-SEC4-6 — Export `[Planned]`
- **US-SEC4.6.1** `[Planned]` **As** a SEC lead, **I want** to export the sharing report to Excel/PDF/CSV/HTML, **so that** I can document sharing debt for governance.

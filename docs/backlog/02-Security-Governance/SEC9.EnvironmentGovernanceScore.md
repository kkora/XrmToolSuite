...
> **Suggested tag:** `SEC9` · **Suggested project:** `XrmToolSuite.EnvironmentGovernanceScore`
> **Overlaps:** Aggregates signals from many suite tools — Sharing Analyzer (SEC4), Audit Compliance Checker (SEC5), Sensitive Data Scanner (SEC7), Licensing Usage Analyzer (SEC8), User Access Heatmap (SEC10), plus shipped Deployment Risk Analyzer, Technical Debt Analyzer, Solution Complexity Score. Also overlaps the candidate Reporting Governance Scorecard. This is the executive roll-up above them.
...
- Reuse existing analyzers' UI-free engines rather than re-querying: role complexity/privileged users (SEC1/SEC6/SEC10), audit config (SEC5), sharing risk (SEC4), sensitive-data protection (SEC7), inactive users/teams (SEC8/SEC2), unmanaged customizations/solution layering/env variables/connection references (shipped Deployment Risk + Technical Debt analyzers).
...
## EPIC-SEC9 — Give leadership one explainable governance health score for an environment
...
## FEAT-SEC9-1 — Data collection & orchestration `[Planned]`
- **US-SEC9.1.1** `[Planned]` **As** an ARCH, **I want** the tool to run each category analyzer and collect its findings, **so that** the score reflects real environment state.
...
- **US-SEC9.1.2** `[Planned]` **As** a TOOLDEV, **I want** category engines reused from existing analyzers, **so that** rules live in one place and stay unit-testable.

## FEAT-SEC9-2 — Category scoring `[Planned]`
- **US-SEC9.2.1** `[Planned]` **As** an MGR, **I want** a score per category (Security, Audit, Compliance, ALM, Data Protection, Ownership, Configuration, Access Management, Maintainability), **so that** I see where the environment is weak.
  - **AC:** Each category score is deterministic and explainable from its contributing findings.
- **US-SEC9.2.2** `[Planned]` **As** an ARCH, **I want** category weights configurable, **so that** the score reflects our governance priorities.
  - **AC:** Weight settings round-trip via settings load/save.

## FEAT-SEC9-3 — Overall score & findings `[Planned]`
- **US-SEC9.3.1** `[Planned]` **As** an MGR, **I want** an overall 0-100 score with category score cards and a critical-findings list, **so that** I get an executive dashboard.

## FEAT-SEC9-4 — Remediation roadmap `[Planned]`
- **US-SEC9.4.1** `[Planned]` **As** an ARCH, **I want** a prioritized remediation roadmap (highest-impact fixes first), **so that** teams know what to tackle next.
  - **AC:** Roadmap items link back to the finding and its source analyzer; all read-only.

## FEAT-SEC9-5 — Trend history `[Planned]`
- **US-SEC9.5.1** `[Planned]` **As** an MGR, **I want** past scores saved locally and shown as a trend, **so that** I can prove governance is improving.
  - **AC:** History persists locally with no credentials or sensitive values stored.

## FEAT-SEC9-6 — Export `[Planned]`
- **US-SEC9.6.1** `[Planned]` **As** an MGR, **I want** to export an executive governance report to Excel/PDF/CSV/HTML, **so that** I can present it to leadership.

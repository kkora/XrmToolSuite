...
> **Suggested tag:** `SEC7` · **Suggested project:** `XrmToolSuite.SensitiveDataScanner`
> **Overlaps:** Feeds "sensitive fields without field security" to Field Security Profiler (SEC3) and "sensitive without audit" to Audit Compliance Checker (SEC5); data-protection sub-score feeds Environment Governance Score (SEC9).
...
## EPIC-SEC7 — Inventory where sensitive data lives and whether it is protected
...
## FEAT-SEC7-1 — Scan configuration `[Planned]`
- **US-SEC7.1.1** `[Planned]` **As** a SEC lead, **I want** a metadata-only scan as the default with sampling strictly opt-in, **so that** I never accidentally read sensitive records.
  - **AC:** Sampling is off unless explicitly enabled; enabling it shows a warning about masking and caps.
- **US-SEC7.1.2** `[Planned]` **As** an ADM, **I want** to scope the scan to tables/solutions and tune the sensitivity ruleset, **so that** results fit our data model.
  - **AC:** Scope and ruleset choices persist in settings.

## FEAT-SEC7-2 — Metadata detection `[Planned]`
- **US-SEC7.2.1** `[Planned]` **As** a SEC lead, **I want** columns flagged by sensitive name, description, and data type, **so that** I find candidate PII from metadata alone.
  - **AC:** Detection runs off cached metadata; each hit records which rule fired.
- **US-SEC7.2.2** `[Planned]` **As** an ADM, **I want** a findings grid by table/column with the matched pattern, **so that** I can review and dismiss false positives.

## FEAT-SEC7-3 — Optional safe sampling `[Planned]`
- **US-SEC7.3.1** `[Planned]` **As** a SEC lead, **I want** opt-in sampling to detect PII-like value patterns under safe caps, **so that** I can confirm suspected columns without a full scan.
  - **AC:** Sample size is capped; all sampled values are masked in UI and never exported raw; values are never logged.

## FEAT-SEC7-4 — Protection status `[Planned]`
- **US-SEC7.4.1** `[Planned]` **As** a SEC lead, **I want** each sensitive column's protection status (field security? audit? form/view exposure?), **so that** I see gaps at a glance.
  - **AC:** Unprotected sensitive columns are flagged with a severity (Critical/High/Medium/Low/Info).

## FEAT-SEC7-5 — Inventory & recommendations `[Planned]`
- **US-SEC7.5.1** `[Planned]` **As** a DPO, **I want** a sensitive-data inventory with remediation suggestions (secure this, enable audit, remove from view), **so that** each finding has a next step.
  - **AC:** Recommendations are read-only; the tool changes nothing.

## FEAT-SEC7-6 — Export `[Planned]`
- **US-SEC7.6.1** `[Planned]` **As** a SEC lead, **I want** to export the sensitive-data report to Excel/PDF/CSV/HTML with all sample values masked, **so that** I can share it safely.

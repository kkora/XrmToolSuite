...
> **Tag:** `SEC04` · **Project:** `XrmToolSuite.SharingAnalyzer`
> **Spec:** `docs/backlog/02-Security-Governance/SEC04.SharingAnalyzer.md`
...
> **Overlaps:** the sharing-risk score feeds a future Environment Governance Score (SEC09); the
...
## FEAT-SEC04-0 — Scaffold & shared wiring `[Done]`

- **US-SEC04.0.1** `[Done]` **As** a TOOLDEV, **I want** the tool to load in XrmToolBox with connection,
...
## FEAT-SEC04-1 — Scoped sharing scan `[Done]`

- **US-SEC04.1.1** `[Done]` **As** an ADM, **I want** to pick tables (and optionally a principal filter)
...
- **US-SEC04.1.2** `[Done]` **As** a SEC lead, **I want** the scan to run off the UI thread with progress
...
## FEAT-SEC04-2 — Shared-records view `[Done]`

- **US-SEC04.2.1** `[Done]` **As** an ADM, **I want** a grid of shared records by table showing who shared
...
- **US-SEC04.2.2** `[Done]` **As** a SEC lead, **I want** access-rights summary cards, **so that** I see the
  score, totals, and the mix of granted rights at a glance.

## FEAT-SEC04-3 — Risk detection `[Done]`

- **US-SEC04.3.1** `[Done]` **As** a SEC lead, **I want** flags for excessive sharing, sharing with inactive
...
- **US-SEC04.3.2** `[Done]` **As** an ADM, **I want** users with unusually high inbound shared access
...
## FEAT-SEC04-4 — Intensity visualization `[Done]`

- **US-SEC04.4.1** `[Done]` **As** a SEC lead, **I want** a table × principal intensity view, **so that**
...
## FEAT-SEC04-5 — Cleanup recommendations `[Done]`

- **US-SEC04.5.1** `[Done]` **As** an ADM, **I want** recommended cleanup actions as a preview list,
...
## FEAT-SEC04-6 — Export `[Done]`

- **US-SEC04.6.1** `[Done]` **As** a SEC lead, **I want** to export the sharing report to

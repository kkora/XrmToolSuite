...
> **Tag:** `SEC02` · **Project:** `XrmToolSuite.TeamPermissionExplorer`
> **Spec:** `docs/backlog/02-Security-Governance/SEC02.TeamPermissionExplorer.md`
> **Reuse:** Shares the effective-privilege engine (`XrmToolSuite.Core.Privileges`) with Privilege Gap
> Analyzer (SEC01) — effective privileges are resolved by `PrivilegeEngine.ResolveEffective`, not re-derived.
...
## FEAT-SEC02-0 — Scaffold & shared wiring `[Done]`

- **US-SEC02.0.1** `[Done]` **As** a TOOLDEV, **I want** the tool to load in XrmToolBox with connection,
...
## FEAT-SEC02-1 — Browse and filter teams `[Done]`

- **US-SEC02.1.1** `[Done]` **As** an ADM, **I want** a list of all teams filterable by team type
...
- **US-SEC02.1.2** `[Done]` **As** a SEC lead, **I want** to search teams by name/BU, **so that** I find
  the relevant team fast.
  - **AC:** In-memory search box filters the loaded grid by name or business unit (instant, no re-query).

## FEAT-SEC02-2 — Team detail: members and roles `[Done]`

- **US-SEC02.2.1** `[Done]` **As** an ADM, **I want** a members grid and an assigned-roles grid per team,
...
- **US-SEC02.2.2** `[Done]` **As** a SEC lead, **I want** an effective table-privilege matrix for the team,
...
## FEAT-SEC02-3 — Inheritance and ownership `[Done]`

- **US-SEC02.3.1** `[Done]` **As** a SEC lead, **I want** the list of users inheriting each team's
...
- **US-SEC02.3.2** `[Done]` **As** an ADM, **I want** a summary of team-owned records by table, **so that**
...
## FEAT-SEC02-4 — Risk findings `[Done]`

- **US-SEC02.4.1** `[Done]` **As** a SEC lead, **I want** teams flagged for no members, no roles,
...
- **US-SEC02.4.2** `[Done]` **As** an ADM, **I want** the rules to degrade a failed query to an
...
## FEAT-SEC02-5 — Compare teams `[Done]`

- **US-SEC02.5.1** `[Done]` **As** an ADM, **I want** to diff two teams' effective privileges and roles,
...
## FEAT-SEC02-6 — Export `[Done]`

- **US-SEC02.6.1** `[Done]` **As** a SEC lead, **I want** to export the team security report to

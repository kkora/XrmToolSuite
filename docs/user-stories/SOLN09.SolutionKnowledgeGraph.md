...
> **Status:** Implemented. Source spec: [`docs/backlog/05-Solution-Management/SOLN09.SolutionKnowledgeGraph.md`](../backlog/05-Solution-Management/SOLN09.SolutionKnowledgeGraph.md) (same US ids).
> **Project:** `src/Tools/XrmToolSuite.SolutionKnowledgeGraph` · **Area tag:** `SOLN09`
...
## EPIC-SOLN09 — Understand a solution's dependencies as an interactive graph `[Implemented]`
...
## FEAT-SOLN09-0 — Scaffold & shared wiring `[Implemented]`
- **US-SOLN09-0.1** `[Implemented]` The tool loads in XrmToolBox with connection, settings and background
  ...
  - **AC:** `SolutionKnowledgeGraphPlugin` exports `Name`/`Description` plus both required
    ...
    `GraphSettings` via `LoadSettings`/`SaveSettings`; no template leftovers. *(Manual — live XTB load; `TC-SOLN09-M-01`.)*

## FEAT-SOLN09-1 — Graph construction `[Implemented]`
- **US-SOLN09-1.1** `[Implemented]` Build a dependency graph from a selected solution to see how its
  components relate.
...
- **US-SOLN09-2** `[Implemented]` Trace a node's dependencies and the impact of deleting it, to assess change risk.
  - **AC:** `GraphModel.DependencyTrace` = forward transitive reachability (BFS over out-edges);
    ...
    sets. **Automated** — `TC-SOLN09-TRACE-03`, `TC-SOLN09-IMPACT-04`; *(detail pane: manual `TC-SOLN09-M-03`).*
- **US-SOLN09-3** `[Implemented]` Detect circular dependencies to find and break dependency loops.
  - **AC:** `GraphModel.Cycles` returns the strongly-connected components of size > 1 (iterative Tarjan,
    ...
    group's components (or a "none" message). **Automated** — `TC-SOLN09-CYCLE-05`, `TC-SOLN09-CYCLE-06`;

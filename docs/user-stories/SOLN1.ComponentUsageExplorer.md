...
> **Status:** Active — implemented (see status tags below). Area tag `SOLN1`.
> **Source:** ported from `docs/backlog/05-Solution-Management/SOLN1.ComponentUsageExplorer.md`.
...
## FEAT-SOLN1-1 — Find and select a component

- **US-SOLN1.1.1** `[Done]` **As** a MAKER, **I want** to search by display name, schema name, GUID, or type, **so that** I can locate the exact component I am about to change.
...
- **US-SOLN1.1.2** `[Done]` **As** a DEVOPS engineer, **I want** the component's solution membership listed, **so that** I know which packages ship it before I touch it.
...
## FEAT-SOLN1-2 — Enumerate required and dependent components

- **US-SOLN1.2.1** `[Done]` **As** a CUST developer, **I want** the components the selected one *requires* and the components that *depend on* it, **so that** I see both directions of the dependency chain.
...
- **US-SOLN1.2.2** `[Done]` **As** a CUST developer, **I want** unsupported or empty dependency results surfaced as informational, **so that** an incomplete platform answer never reads as "safe".
...
## FEAT-SOLN1-3 — Usage detection and per-type summary

- **US-SOLN1.3.1** `[Done]` **As** a MAKER, **I want** each dependent resolved to its friendly type + name + owning solution, **so that** I can tell what would break at a glance.
...
- **US-SOLN1.3.2** `[Done]` **As** a MAKER, **I want** managed and cross-solution dependents highlighted, **so that** I know when a change escapes my own solution.
...
- **US-SOLN1.3.3** `[Done]` **As** an ADM, **I want** a usage-count-by-component-type summary, **so that** I can gauge blast radius at a glance.
...
## FEAT-SOLN1-4 — Impact scoring and change-safety verdict

- **US-SOLN1.4.1** `[Done]` **As** an ALM engineer, **I want** a change-safety verdict for the selected component, **so that** I get a single go/caution/stop signal.
...
- **US-SOLN1.4.2** `[Done]` **As** an ALM engineer, **I want** a recommendation panel explaining the verdict, **so that** I know *why* and what to check next.
...
## FEAT-SOLN1-5 — Export usage report

- **US-SOLN1.5.1** `[Done]` **As** a release manager, **I want** the usage + impact report exported to Excel, PDF, JSON, and HTML, **so that** I can attach it to a change record.

...
> **Status:** Implemented. Source spec: [`docs/backlog/11-Documentation/DOC2.ErdGenerator.md`](../backlog/11-Documentation/DOC2.ErdGenerator.md) (same US ids).
> **Project:** `src/Tools/XrmToolSuite.ErdGenerator` · **Area tag:** `DOC2`
...
## EPIC-DOC2 — Generate accurate Dataverse ERDs from live metadata `[Implemented]`
...
## FEAT-DOC2-1 — Scope and table selection `[Implemented]`
- **US-DOC2.1.1** `[Implemented]` Scope the ERD by all tables, one solution, or a publisher.
...
- **US-DOC2.1.2** `[Implemented]` A checked-list table selector (with a text filter) adds/removes tables.
...
## FEAT-DOC2-2 — Schema, keys, and columns `[Implemented]`
- **US-DOC2.2.1** `[Implemented]` Each table shows display + schema name, primary id/name and alternate keys.
  - **AC:** `EntityKeyMetadata` → `ErdKey`; primary id/name captured from entity metadata. *(Collector: manual; model/emit: automated.)*
- **US-DOC2.2.2** `[Implemented]` A column-display setting (Keys+lookups / Important / All) controls columns;
...
## FEAT-DOC2-3 — Relationships and cascade behavior `[Implemented]`
- **US-DOC2.3.1** `[Implemented]` 1:N, N:1 and N:N plus lookup columns are drawn with correct cardinality.
...
- **US-DOC2.3.2** `[Implemented]` Cascade behavior and required level are shown per relationship.
...
## FEAT-DOC2-4 — Filters, custom/managed status, and layout `[Implemented]`
- **US-DOC2.4.1** `[Implemented]` Filters by custom-only, managed-only and relationship type apply to the
...
- **US-DOC2.4.2** `[Implemented]` Custom vs standard tables are visually distinguished; a text preview
...
## FEAT-DOC2-5 — Multi-format export `[Implemented]`
- **US-DOC2.5.1** `[Implemented]` Export to Mermaid, PlantUML, SVG and PNG.
...
- **US-DOC2.5.2** `[Implemented]` Export to PDF, HTML, Markdown and JSON.

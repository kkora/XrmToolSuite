...
> **Status: WIP.** The project is scaffolded from the template and currently only demonstrates the
> shared plumbing (loads sample accounts). The audit logic below is the plan, not yet built.
...
- **US-AA-0.2** `[Planned]` **As** a TOOLDEV, **I want** the placeholder `UserName`/`HelpUrl` and
...
## FEAT-AA-1 - Select audit scope `[Planned]`

- **US-AA-1.1** `[Planned]` **As** a CUST, **I want** to pick one, several, or all tables (with a
...
## FEAT-AA-2 - Detect column usage signals `[Planned]`

Each signal marks a column as "used" with evidence. A column with **no** signals is a retirement candidate.

- **US-AA-2.1** `[Planned]` **As** a CUST, **I want** columns present on any **form** detected,
  **so that** UI-referenced columns are not falsely flagged.
- **US-AA-2.2** `[Planned]` **As** a CUST, **I want** columns used in any **view / saved query**
  (columns or filter) detected.
- **US-AA-2.3** `[Planned]` **As** a CUST, **I want** columns referenced by **business rules,
  workflows, flows, calculated/rollup fields, and plugin steps** detected.
- **US-AA-2.4** `[Planned]` **As** a CUST, **I want** columns referenced by **charts, dashboards,
...
## FEAT-AA-3 - Results & classification `[Planned]`

- **US-AA-3.1** `[Planned]` **As** a CUST, **I want** a grid of columns with table, display/logical
...
- **US-AA-3.2** `[Planned]` **As** a CUST, **I want** to sort/filter by "no usage", type, or table,
  **so that** I can focus on the safest wins first.
- **US-AA-3.3** `[Planned]` **As** an ADM, **I want** managed columns clearly marked non-deletable,
  **so that** I do not target components I cannot change.

## FEAT-AA-4 - Export `[Planned]`

- **US-AA-4.1** `[Planned]` **As** an ADM, **I want** to export the audit to CSV/Excel, **so that**

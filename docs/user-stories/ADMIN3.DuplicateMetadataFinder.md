...
> **Status:** Implemented. Source spec: [`docs/backlog/04-Dataverse-Administration/ADMIN3.DuplicateMetadataFinder.md`](../backlog/04-Dataverse-Administration/ADMIN3.DuplicateMetadataFinder.md) (same US ids).
> **Project:** `src/Tools/XrmToolSuite.DuplicateMetadataFinder` · **Area tag:** `ADMIN3`
...
## EPIC-ADMIN3 — Surface duplicate and near-duplicate metadata so teams can consolidate `[Implemented]`
...
## FEAT-ADMIN3-1 — Scan configuration and metadata retrieval `[Implemented]`
- **US-ADMIN3.1.1** `[Implemented]` Configure which component types and scope to scan.
...
- **US-ADMIN3.1.2** `[Implemented]` A configurable similarity threshold tunes false positives vs. recall.
...
## FEAT-ADMIN3-2 — Detect duplicate columns and option sets `[Implemented]`
- **US-ADMIN3.2.1** `[Implemented]` Detect duplicate column display names and similar schema names.
...
- **US-ADMIN3.2.2** `[Implemented]` Detect same-type/similar-purpose fields and duplicate/overlapping option sets.
...
## FEAT-ADMIN3-3 — Detect duplicate UI and logic components `[Implemented]`
- **US-ADMIN3.3.1** `[Implemented]` Detect duplicate tables, forms and views.
...
- **US-ADMIN3.3.2** `[Implemented]` Detect duplicate business rules, web resources/JavaScript, plugin steps and relationships.
...
## FEAT-ADMIN3-4 — Group, compare, and recommend primary `[Implemented]`
- **US-ADMIN3.4.1** `[Implemented]` Group duplicates with a side-by-side comparison.
...
- **US-ADMIN3.4.2** `[Implemented]` Recommend a primary component to keep per group.
...
## FEAT-ADMIN3-5 — Export `[Implemented]`
- **US-ADMIN3.5.1** `[Implemented]` Export the duplicate report to Excel, PDF, JSON and self-contained HTML.

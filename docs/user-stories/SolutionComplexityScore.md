...
## EPIC-SC — Quantify a solution's complexity and the effort to maintain it `[Implemented]`
...
## FEAT-SC-0 — Scaffold & shared wiring `[Implemented]`
- **US-SC-0.1** `[Implemented]` Load in XrmToolBox with connection, settings and background execution via
  ...
  - **AC:** Derives from `BaseToolControl`; MEF metadata includes both `SmallImageBase64`/`BigImageBase64`
    ...
    list. Confirmed loading in a live host — `TC-SC-M-01` and the UiSmokeTests harness
    (`xrmtoolbox-tools-list.png`). *(Manual — live XrmToolBox.)*

## FEAT-SC-1 — Component inventory `[Implemented]`
- **US-SC-1.1** `[Implemented]` Pick a solution and inventory every component type, so the score reflects
  ...
  - **AC:** Sub-typed rows are split correctly — `webresourcetype == 3` counts as JavaScript; `systemform
    ...
    **Automated (headless fake service)** — `TC-SC-COL-01..08` in `testing/CollectorTests`; also `TC-SC-M-03`
    (compare to the maker portal). *(Collector against live metadata: manual.)*

## FEAT-SC-2 — Scoring, maintainability & effort `[Implemented]`
- **US-SC-3** `[Implemented]` A 0–100 complexity score and a maintainability score, to communicate solution
  health at a glance.
  - **AC:** Each dimension contributes `count × weight` complexity points (documented per-unit weights, e.g.
    ...
    contributions are exposed as `DimensionScore` rows. **Automated** — `TC-SC-METRIC-01/03/04`
    (`Empty_ScoreZero_MaintainabilityFull`, `LargeSolution_ScoreCapsAt100`,
    `Dimensions_CarryWeightedContribution`).
- **US-SC-4** `[Implemented]` Upgrade / migration / testing effort (in person-days) and an annual
  ...
  - **AC:** Effort/cost are transparent linear functions of the tallies — testing days from a weighted sum of
    ...
    Asserted to exact values so any weight change is caught. **Automated** — `TC-SC-METRIC-02`
    (`KnownCounts_ProduceExactModel`).
- **US-SC-5** `[Implemented]` Hotspot findings (wide forms, high plugin counts, large automation / JS / data
  ...
  - **AC:** Outliers surface as `Finding`s at graded severity — wide form (≥100 controls, Medium), high
    ...
    "No structural hotspots" (Info). **Automated** — `TC-SC-REPORT-05/06` (`Report_ProjectsScore_MetricsAndBand`,
    `Report_FlagsWideForm_ElseNoHotspots`).

## FEAT-SC-3 — Dashboard, export & summary `[Implemented*]`
- **US-SC-6** `[Implemented*]` A dashboard (score + band + metric strip + hotspot grid) and
  ...
  - **AC:** `ComplexityReport.Build` projects the counts + computed model onto the suite-shared `ReportModel`
    ...
    The report projection is **automated** (`TC-SC-REPORT-05`); the WinForms dashboard and the five exporters
    (incl. the GDI/MigraDoc PDF and ClosedXML Excel chains) are manual — `TC-SC-M-02/04/05/06`.
- **US-SC-7** `[Implemented*]` An executive summary (offline templated default, AI opt-in), for a narrative.
...
- Testing under `testing/SolutionComplexityScore/`; SDK-free logic covered by
  ...
  (dashboard, exporters, live collector and AI summary pending manual sign-off, `TC-SC-M-01..07`).

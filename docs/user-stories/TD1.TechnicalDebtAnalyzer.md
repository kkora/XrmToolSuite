...
## EPIC-TD — Quantify and prioritize a Dataverse environment's technical debt `[Implemented]`
...
## FEAT-TD-0 — Scaffold & shared wiring `[Implemented]`
- **US-TD-0.1** `[Implemented]` The tool loads in XrmToolBox with connection, settings, and background
  ...
  - **AC:** `TechnicalDebtAnalyzerControl : BaseToolControl` implements `IGitHubPlugin`/`IHelpPlugin`
    ...
    *(UI load: manual — `TC-TD-M-01`, `xrmtoolbox-tools-list.png`.)*
...
## FEAT-TD-1 — Environment scan & analyzers `[Implemented]`
- **US-TD-1.1** `[Implemented]` Pick which analyzers run and scan the connected environment.
  - **AC:** A `CheckedListBox` over `_allAnalyzers` drives the run loop; the unchecked set persists via
    ...
    off the UI thread with progress and cancellation. *(Selection persist: manual — `TC-TD-M-07`.)*
  - **AC:** Any analyzer that throws degrades to an informational finding and is listed as skipped
    ...
    when hit. **Automated** — `TC-TD-COL-08`.
- **US-TD-1.2** `[Implemented]` Unused-metadata, duplicate, deprecated-API, orphaned, dead-plugin,
  ...
  - **AC:** `UnusedMetadataAnalyzer` flags custom tables with 0 rows (Medium) and very wide custom tables
    (≥ 200 custom columns, Low). **Automated** — `TC-TD-COL-08`, `TC-TD-COL-09`.
  ...
  - **AC:** `NamingViolationsAnalyzer` flags default `new_` publisher prefixes on tables/columns (Low) and
    ...
    sprawl (Info). **Automated** — `TC-TD-COL-07`, `TC-TD-COL-10`, `TC-TD-COL-11`.

## FEAT-TD-2 — Debt score & dashboard `[Implemented]`
- **US-TD-3** `[Implemented]` A 0–100 debt score and a Low/Medium/High band.
  - **AC:** `TechDebtReport.Scorer` weights severities (Critical=25 / High=12 / Medium=5 / Low=2 / Info=0),
    sums and caps at 100, and bands at 15 (Medium) / 40 (High). **Automated** — `TC-TD-SCORE-01`,
    `TC-TD-SCORE-02`, `TC-TD-SCORE-04`.
  - **AC:** Debt does NOT force High on a single Critical (`criticalForcesHigh:false`) — it accumulates.
    **Automated** — `TC-TD-SCORE-03`.
- **US-TD-4** `[Implemented]` Headline metrics (total findings + per-category counts).
  - **AC:** `TechDebtReport.Build` fills `ReportModel.Metrics` with the total plus a per-category breakdown
    (descending by count) that drives the dashboard metric strip. **Automated** — `TC-TD-DASH-05`.

## FEAT-TD-3 — Results, export & summary `[Implemented*]`
- **US-TD-5** `[Implemented*]` Review findings in a colour-coded grid with a detail pane.
  - **AC:** The grid binds findings ordered by descending severity then category, colours the Severity cell
    ...
    recommendation, docs link); a clean environment reads as low debt. *(Manual — `TC-TD-M-02`.)*
- **US-TD-6** `[Implemented*]` Excel, PDF, HTML, JSON and Markdown exports.
  - **AC:** Export routes through the shared reporting module — `ExcelReportExporter` / `PdfReportExporter`
    ...
    *(Manual — `TC-TD-M-08`; JSON/HTML/MD emitters share automated `ReportModel` coverage.)*
- **US-TD-7** `[Implemented*]` An executive summary (offline by default, AI opt-in).

...
> **Status:** Implemented (Phase-A). Area tag `PLUGIN1` · Project `XrmToolSuite.PluginDependencyGraph`.
...
## FEAT-PLUGIN1-1 — Plugin metadata retrieval `[Done]`

- **US-PLUGIN1.1.1** `[Done]` **As** a TOOLDEV, **I want** to load all plugin assemblies, types and SDK
...
- **US-PLUGIN1.1.2** `[Done]` **As** an ARCH, **I want** each step's table, message, stage, mode, rank,
...
- **US-PLUGIN1.1.3** `[Done]` **As** a TOOLDEV, **I want** pre/post images and their attributes attached
...
## FEAT-PLUGIN1-2 — Graph model, filtering & isolation `[Done]`

- **US-PLUGIN1.2.1** `[Done]` **As** an ARCH, **I want** a typed dependency graph (assembly, type, step,
...
- **US-PLUGIN1.2.2** `[Done]` **As** a TOOLDEV, **I want** to select an assembly or plugin type and see its
...
- **US-PLUGIN1.2.3** `[Done]` **As** an ARCH, **I want** to filter the graph by table, message, stage, mode
...
## FEAT-PLUGIN1-3 — Solution & custom API relationships `[Done]`

- **US-PLUGIN1.3.1** `[Done]` **As** an ALM lead, **I want** solution membership shown per assembly/step,
  **so that** I can see which solution owns a registration.
  - **AC:** Solution nodes link to their member components; unmanaged registrations are visually distinct.
    *(TC-PDG-BUILD-01)*
- **US-PLUGIN1.3.2** `[Done]` **As** a TOOLDEV, **I want** custom API relationships drawn where detectable,
...
- **US-PLUGIN1.3.3** `[Done]` **As** a SEC reviewer, **I want** secure/unsecure configuration usage flagged
...
## FEAT-PLUGIN1-4 — High-impact & risk detection `[Done]`

- **US-PLUGIN1.4.1** `[Done]` **As** an ARCH, **I want** high-impact assemblies (registered on many
...
- **US-PLUGIN1.4.2** `[Done]` **As** a DEVOPS, **I want** duplicate/overlapping steps detected, **so that**
...
- **US-PLUGIN1.4.3** `[Done]` **As** an ALM lead, **I want** unmanaged plugin registrations detected,
...
## FEAT-PLUGIN1-5 — Details, findings & export `[Done]`

- **US-PLUGIN1.5.1** `[Done]` **As** a TOOLDEV, **I want** a node details panel and a dependency grid,
  **so that** I can read exact registration values.
  - **AC:** Selecting a node populates a details panel and a dependency grid of its connected components.
    *(TC-PDG-DETAILS-04)*
- **US-PLUGIN1.5.2** `[Done]` **As** an ARCH, **I want** a risk findings panel with severities, **so that**
  ...
  - **AC:** Findings carry Critical/High/Medium/Low/Info with a plain-language description each.
    *(TC-PDG-RISK-06..08)*
- **US-PLUGIN1.5.3** `[Done]` **As** a MGR, **I want** to export the graph to PNG, SVG, PDF, Excel, JSON,

...
> **Status:** Active — implemented (see status tags below). Area tag `PP01`.
> **Source:** ported from `docs/backlog/06-Power-Pages/PP01.PortalHealthAnalyzer.md`.
...
## FEAT-PP01-1 — Website discovery & selection

- **US-PP01.1.1** `[Done]` **As** an ADM, **I want** every Power Pages website in the environment listed,
...
- **US-PP01.1.2** `[Done]` **As** an ADM, **I want** my last-selected website remembered, **so that** I
  resume where I left off.
  - **AC:** Selection persists via settings round-trip (`LoadSettings`/`SaveSettings`), storing website id +
    schema only (no credentials).

## FEAT-PP01-2 — Configuration retrieval & summary

- **US-PP01.2.1** `[Done]` **As** an ADM, **I want** the selected website's settings, roles, permissions,
...
- **US-PP01.2.2** `[Done]` **As** an ADM, **I want** configuration summary counts per record type, **so that**
...
## FEAT-PP01-3 — Structural integrity checks

- **US-PP01.3.1** `[Done]` **As** an ADM, **I want** broken page relationships and pages without templates
...
- **US-PP01.3.2** `[Done]` **As** an ADM, **I want** inactive pages/templates and broken web-file references
  ...
  - **AC:** Inactive (statecode-off) pages/templates → Medium; web files referenced by pages/templates but
    absent → High.
- **US-PP01.3.3** `[Done]` **As** an ADM, **I want** forms/lists referencing inactive Dataverse tables
...
## FEAT-PP01-4 — Site settings checks

- **US-PP01.4.1** `[Done]` **As** an ADM, **I want** missing required site settings detected, **so that** the
  ...
  - **AC:** A curated required-settings list (per schema) is checked; absent settings → High with the setting
    name.
- **US-PP01.4.2** `[Done]` **As** an ADM, **I want** duplicate/conflicting site settings detected, **so that**
...
## FEAT-PP01-5 — Security surface checks (health-level summary)

- **US-PP01.5.1** `[Done]` **As** a SEC engineer, **I want** anonymous-access risks surfaced at a health
...
- **US-PP01.5.2** `[Done]` **As** a SEC engineer, **I want** over-broad Global-scope permissions flagged,
...
## FEAT-PP01-6 — Health score, recommendations & export

- **US-PP01.6.1** `[Done]` **As** an MGR, **I want** an overall portal health score with a band, **so that** I
...
- **US-PP01.6.2** `[Done]` **As** an ADM, **I want** a recommendation per finding, **so that** I get concrete
  remediation steps.
  - **AC:** Each actionable finding carries a plain-language recommendation and the affected record.
- **US-PP01.6.3** `[Done]` **As** an MGR, **I want** Excel/PDF/Word/JSON/CSV/HTML exports of the health
...
- SDK-free scoring covered by `testing/UnitTests/PortalHealthAnalyzerTests.cs`; manual/live cases and export
  round-trips documented under `testing/Tools/PortalHealthAnalyzer/`.

...
> **Source:** `docs/backlog/01-ALM-DevOps/ALM01.SolutionMergeAssistant.md` (EPIC-ALM01).
> **Area tag:** `ALM01` · **Project:** `XrmToolSuite.SolutionMergeAssistant`
...
## FEAT-ALM01-1 — Select and load solutions to compare `[Done]`

- **US-ALM01.1.1** `[Done]` **As** an ALM engineer, **I want** to pick two or more solutions from the connected
...
- **US-ALM01.1.2** `[Done]` **As** an ALM engineer, **I want** each solution's components enumerated once and
...
## FEAT-ALM01-2 — Detect duplicate and overlapping components `[Done]`

- **US-ALM01.2.1** `[Done]` **As** a Solution Architect, **I want** components that appear in more than one
...
- **US-ALM01.2.2** `[Done]` **As** a Solution Architect, **I want** duplicate web resources, plugin assemblies,
...
## FEAT-ALM01-3 — Detect version, publisher, and managed-state conflicts `[Done]`

- **US-ALM01.3.1** `[Done]` **As** an ALM engineer, **I want** version conflicts and publisher mismatches
...
- **US-ALM01.3.2** `[Done]` **As** an ALM engineer, **I want** managed/unmanaged conflicts detected, **so that**
...
## FEAT-ALM01-4 — Detect config and completeness conflicts `[Done]`

- **US-ALM01.4.1** `[Done]` **As** an ALM engineer, **I want** environment-variable and connection-reference
...
- **US-ALM01.4.2** `[Deferred]` Missing/required-component completeness (`RetrieveMissingDependencies`) is
  tracked under ALM02 (Dependency Validator) and is out of scope for this comparison tool.

## FEAT-ALM01-5 — Merge risk verdict and recommendation `[Done]`

- **US-ALM01.5.1** `[Done]` **As** a Delivery Manager, **I want** all conflicts rolled into a single verdict,
...
- **US-ALM01.5.2** `[Done]` **As** a Solution Architect, **I want** a recommended merge strategy and a
...
## FEAT-ALM01-6 — Export merge report `[Done]`

- **US-ALM01.6.1** `[Done]` **As** a release manager, **I want** the merge report exported to Excel, PDF, JSON,

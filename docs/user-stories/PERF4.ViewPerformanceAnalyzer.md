...
> **Status:** Active — implemented (see status tags below). Area tag `PERF4`.
> **Source:** ported from `docs/backlog/03-Performance/PERF4.ViewPerformanceAnalyzer.md`.
> **Engine reuse:** consumes the shared FetchXML parser + rule engine in `src/Shared/Core/FetchXml/` (PERF3) rather than reimplementing query rules; adds LayoutXML column analysis, per-view scoring and batch ranking on top (`src/Tools/XrmToolSuite.ViewPerformanceAnalyzer/Analysis/`).
...
## EPIC-PERF4 — Rank an environment's slowest and riskiest views before users complain
...
## FEAT-PERF4-1 — View inventory

- **US-PERF4.1.1** `[Done]` **As** an ADM, **I want** to pick a table and list its system views, **so that** I can scope analysis.
...
- **US-PERF4.1.2** `[Done]` **As** an ADM, **I want** to optionally include personal views, **so that** I can catch user-created heavy views.
...
## FEAT-PERF4-2 — FetchXML analysis per view

- **US-PERF4.2.1** `[Done]` **As** a PERF engineer, **I want** each view's FetchXML run through the shared analyzer, **so that** query risks are consistent with the standalone tool.
  - **AC:** Each view's findings come from the shared PERF3 FetchXML rule engine (all-attributes, missing/broad filters, joins, sorts, aggregations). A view whose FetchXML can't be parsed degrades to a single Info finding (score 0), never an exception. *(`ViewScorer.Analyze` → `FetchXmlRules.Analyze`; covered by `ViewPerformanceAnalyzerTests`.)*
- **US-PERF4.2.2** `[Done]` **As** an ADM, **I want** a per-view FetchXML detail panel, **so that** I can inspect what a flagged view actually queries.
  - **AC:** Selecting a view shows its FetchXML and the findings tied to it. *(FetchXML panel + per-view findings grid.)*

## FEAT-PERF4-3 — Layout & column analysis

- **US-PERF4.3.1** `[Done]` **As** a PERF engineer, **I want** LayoutXML columns and FetchXML attributes counted, **so that** I can flag over-wide views.
...
- **US-PERF4.3.2** `[Done]` **As** an ADM, **I want** a layout column panel per view, **so that** I can see which columns drive the width.
  - **AC:** The panel lists layout columns for the selected view. *(`lstLayoutColumns`.)*

## FEAT-PERF4-4 — Scoring & ranking

- **US-PERF4.4.1** `[Done]` **As** a PERF engineer, **I want** a per-view performance score, **so that** I can compare views objectively.
...
- **US-PERF4.4.2** `[Done]` **As** an ADM, **I want** the slowest/riskiest views ranked, **so that** I fix the worst first.
...
## FEAT-PERF4-5 — Optional timing, recommendations & export

- **US-PERF4.5.1** `[Done]` **As** a PERF engineer, **I want** to optionally time a view's execution, **so that** I can validate the score.
...
- **US-PERF4.5.2** `[Done]` **As** an ADM, **I want** recommendations and Excel/PDF/JSON/HTML/CSV exports, **so that** I can act and share.
...
- Reuses the shared FetchXML analyzer (PERF3) rather than reimplementing rules; scores labeled heuristic.
- Batch analysis pages via `RetrieveAll` with progress/cancellation; settings round-trip.
- Testing artifacts under `testing/ViewPerformanceAnalyzer/`; SDK-free scoring covered by `testing/UnitTests/ViewPerformanceAnalyzerTests.cs`.

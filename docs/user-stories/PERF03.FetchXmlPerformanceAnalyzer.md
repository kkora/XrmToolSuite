...
> **Status:** Active — implemented (see status tags below). Area tag `PERF03`.
> **Source:** ported from `docs/backlog/03-Performance/PERF03.FetchXmlPerformanceAnalyzer.md`.
> **Engine:** a shared, UI-free parser + rule engine in `src/Shared/Core/FetchXml/` (reusable by PERF04/PERF05).
...
## FEAT-PERF03-1 — Input & load

- **US-PERF03.1.1** `[Done]` **As** a PERF engineer, **I want** to paste FetchXML into an editor and validate its syntax, **so that** I can analyze ad-hoc queries.
...
- **US-PERF03.1.2** `[Done]` **As** an ADM, **I want** to load FetchXML from a system or personal view, **so that** I can analyze what users actually run.
  - **AC:** A view picker loads `savedquery` / `userquery` FetchXML via `Service.RetrieveAll` off the UI thread.

## FEAT-PERF03-2 — Query parsing & summary

- **US-PERF03.2.1** `[Done]` **As** a PERF engineer, **I want** the query broken into entity, attributes, filters, links, orders, aggregation, and paging, **so that** I understand its shape.
...
- **US-PERF03.2.2** `[Done]` **As** a TOOLDEV, **I want** the parser/rule engine to be UI-free and shared, **so that** View and Dashboard analyzers reuse it.
...
## FEAT-PERF03-3 — Performance rule engine

- **US-PERF03.3.1** `[Done]` **As** a PERF engineer, **I want** all-attributes usage and excessive selected columns flagged, **so that** I can trim payload.
  - **AC:** `<all-attributes/>` → High; column count over `MaxAttributes` (30) → Medium.
- **US-PERF03.3.2** `[Done]` **As** a PERF engineer, **I want** missing filters and unbounded scans flagged, **so that** I catch table scans.
  - **AC:** No filter on the root entity → High; aggregate without a filter → Medium.
- **US-PERF03.3.3** `[Done]` **As** a PERF engineer, **I want** excessive/outer joins and risky sorts flagged, **so that** I can restructure the query.
...
- **US-PERF03.3.4** `[Done]` **As** a PERF engineer, **I want** aggregation and paging risks flagged, **so that** large-result issues surface.
  - **AC:** Aggregate without filter → Medium; no paging + no top + no aggregate → Low; distinct over > `WarnLinkEntities` joins → Info.

## FEAT-PERF03-4 — Cost estimate & optional timing

- **US-PERF03.4.1** `[Done]` **As** a PERF engineer, **I want** an estimated query cost score and band, **so that** I can compare queries without running them.
  - **AC:** A composite 0–100 cost derives from finding severities (via `ScoreCalculator`), bands at 15/40, and is labeled an estimate.
- **US-PERF03.4.2** `[Done]` **As** a PERF engineer, **I want** to optionally execute the query with timing, **so that** I can ground the estimate.
...
## FEAT-PERF03-5 — Recommendations & export

- **US-PERF03.5.1** `[Done]` **As** a PERF engineer, **I want** concrete optimization suggestions, **so that** I have fixes to apply.
  - **AC:** A suggestions panel lists plain-text improvements (explicit columns, add filter, reduce joins, etc.) tied to the findings.
- **US-PERF03.5.2** `[Done]` **As** a MGR, **I want** Excel/PDF/JSON/HTML/Markdown/CSV exports of the analysis, **so that** I can share it.

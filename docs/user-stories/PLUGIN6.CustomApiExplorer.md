...
> **Status:** Implemented. Source spec: [`docs/backlog/08-Plugins-Custom-APIs/PLUGIN6.CustomApiExplorer.md`](../backlog/08-Plugins-Custom-APIs/PLUGIN6.CustomApiExplorer.md) (same US ids).
> **Project:** `src/Tools/XrmToolSuite.CustomApiExplorer` · **Area tag:** `PLUGIN6`
...
## EPIC-PLUGIN6 — Discover, document, and safely test Custom APIs `[Implemented]`
...
## FEAT-PLUGIN6-1 — Custom API inventory `[Implemented]`
- **US-PLUGIN6.1.1** `[Implemented]` List all Custom APIs in the org.
...
- **US-PLUGIN6.1.2** `[Implemented]` Filter the catalog.
  - **AC:** A live text filter narrows by unique/display name; the filter round-trips via Load/SaveSettings.
    *(Manual — WinForms.)*
- **US-PLUGIN6.1.3** `[Implemented]` Show each API's backing plugin type.
  - **AC:** `plugintypeid` resolved to the plugin type name; logic-less APIs are marked "(none)". *(Collector: manual.)*

## FEAT-PLUGIN6-2 — Parameters & responses `[Implemented]`
- **US-PLUGIN6.2.1** `[Implemented]` See each API's request parameters with type and optionality.
...
- **US-PLUGIN6.2.2** `[Implemented]` See each API's response properties with types; functions vs. actions distinguished.
  - **AC:** Response properties listed with name and type; the detail pane labels Function vs. Action. *(Collector: manual.)*
- **US-PLUGIN6.2.3** `[Implemented]` Export parameter/response detail.
...
## FEAT-PLUGIN6-3 — Callers & dependencies `[Implemented*]`
- **US-PLUGIN6.3.1** `[Implemented*]` See what references a Custom API.
...
- **US-PLUGIN6.3.2** `[Implemented*]` See the SDK message wiring per API.
  - **AC:** The associated SDK message is surfaced when present. *(Manual — live metadata.)*

## FEAT-PLUGIN6-4 — Interactive test console (gated write/execute) `[Implemented]`
- **US-PLUGIN6.4.1** `[Implemented]` A form generated from the API's parameter metadata takes typed inputs.
...
- **US-PLUGIN6.4.2** `[Implemented*]` An explicit confirmation precedes any invocation.
...
- **US-PLUGIN6.4.3** `[Implemented*]` The response or fault is shown clearly.
...
- **US-PLUGIN6.4.4** `[Implemented]` The console never stores secrets.
...
## FEAT-PLUGIN6-5 — Documentation export `[Implemented]`
- **US-PLUGIN6.5.1** `[Implemented]` Export a full Custom API catalog.
...
- **US-PLUGIN6.5.2** `[Implemented]` A sample invocation snippet per API.

...
Central home for the suite's agile backlog: a **Portfolio Epic** for the product, a **Platform
Epic** for the shared foundation (both below), and one **file per tool** for its Epic -> Features
-> User Stories.
...
Portfolio Epic  (the suite as a product)
  ...
  Tool Epic per tool                                          -> one file each, above
```
...
- **US-PLAT-2.1** `[Done]` **As** a TOOLDEV, **I want** one command to stamp out a new tool project
  ...
  - **AC:** Generated docs (README, DEPLOYMENT.md, and `docs/user-stories/<Name>.md`) are re-tokenized with the new tool's names.

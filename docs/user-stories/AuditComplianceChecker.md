...
[`docs/backlog/02-Security-Governance/SEC5.AuditComplianceChecker.md`](../backlog/02-Security-Governance/SEC5.AuditComplianceChecker.md).
Area tag: **SEC5**. Personas: SEC lead, ADM (administrator), MGR (manager), TOOLDEV.
...
## FEAT-SEC5-0 - Scaffold & shared wiring `[Done]`

- **US-SEC5.0.1** `[Done]` **As** a TOOLDEV, **I want** the tool to load in XrmToolBox with connection,
...
## FEAT-SEC5-1 - Audit settings coverage `[Done]`

- **US-SEC5.1.1** `[Done]` **As** a SEC lead, **I want** org-, table-, and column-level audit settings in
...
- **US-SEC5.1.2** `[Done]` **As** a SEC lead, **I want** sensitive tables and fields without audit enabled
...
## FEAT-SEC5-2 - Audit activity analysis `[Done]`

- **US-SEC5.2.1** `[Done]` **As** a SEC lead, **I want** audit activity summarized by table, user, and date,
...
- **US-SEC5.2.2** `[Done]` **As** an ADM, **I want** high-risk changes highlighted (deletes, after-hours edits,
...
## FEAT-SEC5-3 - Storage growth trend `[Done]`

- **US-SEC5.3.1** `[Done]` **As** an MGR, **I want** an estimated audit storage growth trend, **so that** I can
...
## FEAT-SEC5-4 - Compliance score `[Done]`

- **US-SEC5.4.1** `[Done]` **As** an MGR, **I want** a compliance readiness score (0-100) with a category
...
## FEAT-SEC5-5 - Recommendations `[Done]`

- **US-SEC5.5.1** `[Done]` **As** a SEC lead, **I want** prioritized remediation (enable audit on these
...
## FEAT-SEC5-6 - Export `[Done]`

- **US-SEC5.6.1** `[Done]` **As** an MGR, **I want** to export the audit compliance report to
